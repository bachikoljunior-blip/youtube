...
def test_pace_のサブの診断も生きている口から出ること():
    ...
    **【2026-09-20 02:4x】緑になりました。門は 1つ も下げていません。**
    落ちていたのは**計器の窓**でした —— `model_choice.jsonl` に **25.4時間 の穴**が在り
    （09/18 16:59→09/19 18:23 JST・そのあいだ 周 35件 に対しサブの行 1件・枝の commit は 200件 ＝
    **サブは働いていて、台帳だけが書けていなかった**）、`pace()` はその穴をまたいで比を割っていました。
    **穴の後ろだけで数えると 13体 / 13周 ＝ 1.00体**（オーナー 09/14 20:22「サブ立てるのは 1体 だけ」と一致）。
    直したのは `pace()` の**比の窓**だけで、`per_lap`・`floor_min`・この門（>= 1.0）は動かしていません。
    **derivation は `scripts/quota.py` の `_choices_live_since()` の註**（ここへ写さない ＝ 決めは1か所）。
    **この検査が次に赤になったら、それは「台帳がまた 3周 以上 書けていない」か「1周に 1体 が変わった」かの
    どちらかです** —— 門を下げずに、その 2つ を先に読むこと。

    **【2026-09-20 02:1x】（残す）この検査は赤のままで正しい。ただし理由が変わりました。**
    落ちる理由は **もう「死んだ台帳」ではありません**（`model_choice.jsonl` は この周の行まで生きている）。
    **分母（`_births_between`）が 2026-09-08 17:0x から「親が起きた回」を数えている**からで、
    実測は **サブ 1体 あたり 約3.4回**（直近1日 畳んだ周 37 対 立てたサブ 11）。
    **赤を消すために門を下げないこと** —— 下げると、本当に台帳が死んだ回に鳴らなくなります。
    **消し方は 1つ だけ: 分母の単位を決めること**（撃つ 3手 と覆る条件は
    `scripts/quota.py` の `_births_between()` の註・derivation は JOURNAL 2026-09-20 02:1x）。
    """
    p = quota.pace()
    assert p and p.get("subs_per_lap") is not None, "pace が サブの診断を出していない"
    assert p["subs_per_lap"] >= 1.0, \
        (f"1周に {p['subs_per_lap']:.2f}体。**台帳は生きています**（2026-09-20 02:1x に数えた）——"
         f"分母の `_births_between` が 2026-09-08 17:0x から **GO と WAIT の両方**を数えており、"
         f"サブ 1体 あたり 約3.4回 在ります（親は 1周に {len(quota.sub_roles())}体 立てる）。"
         f"**門を下げて消さないこと** —— 消し方は `scripts/quota.py` の `_births_between()` の註の 3手。")
