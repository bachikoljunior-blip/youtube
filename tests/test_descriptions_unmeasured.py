...
## **同じ穴を、同じ日に2つの回が別々に見つけました**

`origin/claude/youtube-auto-post-revenue-ggedij` 側（hourly）が 2026-08-30 夜に
`report()` の 1) 2) を「**測っていません**」に直しており、この回はそれを知らずに
同じ所へ着きました。**併合では相手の文言を採り**（`quotaExceeded` と
JST 16:00 まで書いてあるので、そちらのほうが厚い）、
**こちらは相手に無いほうだけを残しています** —— `_coverage()`、すなわち
**「0本」ではなく「途中まで取れた」回**の穴です。相手の枝は `not recs` しか見ないので、
50/735本 で止まった回は「0 / 50本」とだけ出て、**735本ぜんぶ綺麗**に読めます。
...
def test_1本も取れていない回は_0件と印字しない(tmp_path):
    ...
    assert "解除条件1・2 の根拠にしないこと" in out
