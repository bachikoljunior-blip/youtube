...
# ---------------------------------------------------------------------------
# **2つ目のチャンネル**（2026-09-14 09:4x・optimizer・Opus）
# `yt.channel()` は `channels.list(mine=True)` ＝ **いまのトークンが指すチャンネル**を返す。
# オーナー 09/14 09:3x「上書き後のはクッキーストラテジャーというチャンネルの方のトークン」で、
# **同じ台帳に 2つ目の id が入り得る口が開いた**（台帳はまだ 1つ・243件）。
# ---------------------------------------------------------------------------

def _row2(at: str, subs: int, views: int) -> dict:
    """**別のチャンネル**の行（id が違うだけ・行そのものは正しい ＝ 赤は出ない）。"""
    return {"at": at, "id": "UCyyy", "event": "channel",
            "subs": subs, "views": views, "videos": 4}


def test_別のチャンネルの行は混ぜないこと():
    rows = [_row("2026-09-14T03:00:00+09:00", 28, 88000),
            _row("2026-09-14T09:00:00+09:00", 28, 88344),
            _row2("2026-09-14T09:45:00+09:00", 3, 12)]
    assert trend.channel_ids(rows) == ["UCxxx", "UCyyy"]
    g = trend.channel_growth(rows)
    assert g["n"] == 1 and g["subs"] == 3 and g["views"] == 12   # 新しい id の行だけ
    assert g["d_subs"] is None and g["d_views"] is None          # 跨いだ差は出さない


def test_positive_control_絞りを外すと登録だけが化け_総再生は包絡に隠れること(monkeypatch):
    """**この検査が守っている当のもの**、と **この回に撃って分かった形**。

    id で絞らずに両端を取ると、**登録は 28 → 3 ＝ `-25` として読まれます**
    （§7 の収益の節の覆る条件 (1) の登録率の分子）。
    **ところが総再生は `+344` のまま**です —— 両端は**包絡**で読むので
    （`_pt_row`・2026-09-11 04:0x）、低いほうの 12 は「遅れた複製」として上げられます。
    ＝ **混ざりは半分しか表に出ません**: 総再生は**混ざっても平らに見える**ので、
    §7 (m)（チャンネルが止まっているか）は**混ざりを止まりと読みます**。
    **だから `!!` が要ります** —— 数の側だけを見ていると、片方は化け、片方は黙ります。
    """
    rows = [_row("2026-09-14T03:00:00+09:00", 28, 88000),
            _row("2026-09-14T09:00:00+09:00", 28, 88344),
            _row2("2026-09-14T09:45:00+09:00", 3, 12)]
    monkeypatch.setattr(trend, "channel_ids", lambda rs: [])     # 絞りを潰す
    g = trend.channel_growth(rows)
    assert g["n"] == 3 and g["d_subs"] == -25          # 登録は化ける
    assert g["d_views"] == 344                          # 総再生は包絡に隠れる


def test_混ざった回は行の頭で言うこと():
    rows = [_row("2026-09-14T03:00:00+09:00", 28, 88000),
            _row("2026-09-14T09:00:00+09:00", 28, 88344),
            _row2("2026-09-14T09:45:00+09:00", 3, 12)]
    line = trend.channel_line(rows)
    assert line.startswith("!!") and "UCyyy" in line and "UCxxx" in line


def test_positive_control_id_が1つの回は何も言わないこと():
    """いまの台帳（243件・id 1つ）で `!!` が出たら、この口は毎周 鳴る側です。"""
    rows = [_row("2026-09-14T03:00:00+09:00", 28, 88000),
            _row("2026-09-14T09:00:00+09:00", 28, 88344)]
    assert trend.channel_ids(rows) == ["UCxxx"]
    assert not trend.channel_line(rows).startswith("!!")


def test_id_の無い古い行は落とさないこと():
    """`id` を持たない `channel` の行（在れば）は、絞りで消さないこと ——
    消すと、口が足される前の窓が丸ごと読めなくなります。"""
    old = {"at": "2026-09-14T03:00:00+09:00", "event": "channel", "subs": 28, "views": 88000}
    rows = [old, _row("2026-09-14T09:00:00+09:00", 28, 88344)]
    g = trend.channel_growth(rows)
    assert g["n"] == 2 and g["d_views"] == 344
