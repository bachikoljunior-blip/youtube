...
def test_しきいは実測で置いた値のまま() -> None:
    """**2 は勘ではありません**（2026-09-01 夕に 4 から下げた）。

    ## なぜ 4 をやめたか

    前の版の理由は「連の長さは 中央 2・平均 3.8。しきい 4 なら普通の連は触らない」
    ——**連の分布から決めていて、比のほうを見ていませんでした。**

    **連のしきい `N` は、`fix` 比の天井を `N/(N+1)` に固定します。**
    `N=4` の天井は **80%** で、**門を入れる前の実測 72.8% より高い** ——
    **完全に効いても、直そうとしていた比を下げられない設計**でした。
    実測: 門の後の `fix` 比は **83.2%**（107件中 89件・`rm.fix_share()`）。

    **この検査は「値」を止めています。「値の正しさ」を止めているのは
    `tests/test_fix_run_cap_share.py`** —— しきいを、その時点の `fix` 比より
    高い天井へ戻すと、あちらが赤くなります。**上げるなら、先に `N/(N+1)` を
    出して、いまの比（`rm.fix_share_line()`）と比べること。**
    """
    assert rm.FIX_RUN_CAP == 2
