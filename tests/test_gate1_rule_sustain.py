...
def test_規則の外の本数で作られた再生には断りが出る():
    ...
    # **場を規則から作ること**（2026-09-05 に書き替えた）——
    #     ここは `views_per_day = 2724.1` をべた書きし、規則の下で続く数
    #     `942.125 × 1本/日 = 942` との比 2.89倍 で断りを出させていました。
    #     規則が 10本/日 になると続く数は 9,421 になり、**2,724 は規則の中へ収まって**
    #     この検査は「断りが出ない場面」を測る形に化けます（実際に赤くなりました）。
    #     **比を固定して、場のほうを規則から作ります**（見張る物は同じ）。
    over = 2.89
    a = {"per_video_now": 942.125, "views_per_day": 942.125 * rule * over,
         "subs_per_day": 0.86}
    lines = eta.rule_sustain_lines(a)
    assert lines, (
        f"いまの再生／日 が、規則の下で続く数の {over}倍 なのに、断りが1行も出ません。"
