...
def test_末尾の沈黙も分母に入る():
    """**公開が止まったら、出口の実測は下がること。**（2026-08-27・最適化の回）

    `published_rate()` は `max(days)`（＝最後に1本でも公開した日）で割っていました。
    **内側の 0本 の日は分母に入るのに、末尾は落ちます** ——
    docstring 自身が「落とすと**止まっていた日を無かったことにします**」と
    書いているのに、末尾についてはそうなっていませんでした。

    害は「数がずれる」ではありません。**公開が今日ぱったり止まっても、この数は
    1ミリも下がらない**ことです。`state()` はこれを「1日 続けられる速さ」として
    `density_month` に渡すので、**投稿が途切れた回ほど供給の見通しが正しく見えます。**
    `CLAUDE.md`「投稿が途切れるのが最大の損失」の、ちょうどそのときに黙る計器でした。

    **覆る条件**: 「意図して出さない日」を持つようになったら（例: 対照日を空ける実験）、
    その日を分母から除く欄が要ります。いまはそういう日が1つも無いので、
    **沈黙は全部 事故**として数えます。
    """
    # 08/16・08/17 に 4本ずつ → その2日だけ見れば 4.0本/日。
    # ところが今日は 08/22 で、**08/18〜08/21 の4日は1本も出ていません。**
    rows = [{"at": "2026-08-16T10:00:00Z"}] * 4 + [{"at": "2026-08-17T10:00:00Z"}] * 4
    pr = supply.published_rate(rows, today=date(2026, 8, 22))
    assert pr["n"] == 8
    assert pr["days"] == 6, f"末尾の沈黙 4日 が分母に入っていません（{pr['days']}日）"
    assert pr["per_day"] == pytest.approx(8 / 6), (
        f"止まっているのに {pr['per_day']:.2f}本/日 と名乗っています")
