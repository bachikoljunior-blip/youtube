...
def test_M11が却下の見出しの下に出る(capsys):
    """**「出ている」では足りません。どの見出しの下かを見ます。**（2026-08-27 に足した）

    2026-08-27 に `print_means()` へ **動いている／終わった** の2つを足した回、
    枝を `却下・待ち` より**前**に置いて **M11 が「終わった」へ落ちました** ——
    状態の一行が

        M14 の8の段待ち（2026-08-15 16:0x に「却下」を**取り消し済み**…

    で、**「取り消し済み」の「済」に部分一致した**からです。
    M11 は同じ関数の註が「**律速そのもの**」と書いている項で、
    落ちると**覆る条件（8本の段に14日 乗る）が毎回どこにも出ません。**

    **上の2つの検査はどちらも通ります** —— 片方は `MEANS.md` の本文しか見ず、
    もう片方は `"M11" in out` なので、**どの見出しの下でも当たります。**
    この帳面が何度も踏んでいる「見えているつもりで見えていない」形なので、
    **見出しとの関係で見ます。**
    """
    status.print_means()
    out = capsys.readouterr().out
    ...
    for line in out.splitlines():
        if line.lstrip().startswith("--- "):
            section = line
        elif line.strip().startswith("M11 "):
            break
    else:                                            # pragma: no cover
        raise AssertionError(f"M11 の行が印字にありません:\n{out}")

    assert section is not None and "却下・待ち" in section, (
        f"M11 が「却下・待ち」ではなく {section!r} の下に出ています。"
        "**律速が語の部分一致で別の札へ落ちています**")
