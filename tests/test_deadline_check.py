...
def test_暦の日の押さえが_実物の前提に効いていること(_real_world):
    ...
    # **数えるのは「1.00 と印字されたか」ではなく「1.00 を越えた率が在るか」**
    # （2026-09-19 18:xx・optimizer・Opus 5・ultracode）。
    # 09-04 22:3x に置いたときの字は `"1.00/日" in why`（＝ **押さえが効いた跡**）でしたが、
    # これは **自然の伸び率が 1.00 を越えている間しか真になりません**。
    # 実測 2026-09-19: 標本が 17日で 14件 ＝ **0.82/日** ＝ 押さえに当たらない ＝
    # 「1.00/日」の字が出ない ＝ **押さえが正しく働いているのに赤**（この検査は 09/19 まで
    # 赤いまま申し送られていました・`docs/JOURNAL.md` 09/19 17:0x の 4.）。
    # **この検査が守りたいのは「1日1つより速い率を名乗らないこと」**なので、
    # 印字された率を全部 読んで、**1.00 を越えるものが 1つ も無い**ことを見ます
    # （押さえが外れれば 6.00/日 のような率がそのまま字に出るので、この形でも捕まります）。
    # **覆る条件**: 率の印字の形（`0.82/日`）が変わったら、この正規表現を直すこと。
    # 印字から率が消えたら（率を出さなくなったら）、この検査は仕事を終えています。
    rates = [float(x) for x in re.findall(r"(\d+(?:\.\d+)?)/日", why)]
    assert rates, f"率が 1つ も印字されていません: {why}"
    assert max(rates) <= 1.0, (
        f"暦の日を数える式が 1日1つ より速い伸び率を名乗っています（{max(rates)}/日）: {why}")
