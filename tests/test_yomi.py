...
@pytest.mark.parametrize("word", ["控除額", "差額", "総額", "年額", "月額", "全額", "賃金日額"])
def test_複合語は触らない(word):
    """複合語は Google TTS が実測で全部「がく」と読む。触ると逆に壊れる。

    **`額面` は 2026-09-05 に この一覧から外しました。**
    上の1文は「実測で全部 がく」と言っていましたが、**読み照合の輪が本番の音で
    反証しました** —— `data/yomi_ledger.json`::

        額面  correct ガクメン  heard **ラクメン**  verdict misread  by hear

    ＝ `額面` は そのままでは正しく読まれないので、`yomi_gate` の台帳がかなへ直します。
    ここに残すと、**その直しのほうが赤くなります**（赤いのは台帳ではなく、この主張のほう）。

    **`額` の Fix は いまも `額面` を触りません** —— `pattern` が
    「前後どちらも漢字でない `額`」なので当たりません。外したのは検査の主張だけです。

    **覆る条件**: 本番エンジンが `額面` を ガクメン と読むようになったら、
    台帳の側（`data/yomi_ledger.json`）から落として、ここへ戻すこと。
    """
