...
def test_終わる前に流れている() -> None:
    ...
    #
    # **行の頭には `[+MM:SS]` が付きます**（2026-09-04 17:1x に足した。
    #     `_run_out` の docstring が 09-03 から「どの段が遅いかは誰も測れていません」と
    #     書いていた、その時刻です）。**素の `[sweep]   さき` で索くとここが落ちます** ——
    #     この検査が見たいのは「子が終わる前に流れているか」であって、字の形ではないので、
    #     時刻を跨いで索きます。
    import re as _re
    assert _re.search(r"\[sweep\]\s+\[\+\d+:\d\d\] さき", seen[0]), \
        f"子が終わる前に、最初の行が刷られていない: {seen[0]!r}"
    assert not _re.search(r"\[sweep\]\s+(\[\+\d+:\d\d\] )?あと", seen[0]), \
        "この時点でまだ出ていないはずの行が出ている"
