...
def test_バーストの速さを入れると13倍が埋まってしまう():
    """**直す前の姿**。3.3時間で +5本 ＝ 1日 36.5本 を入れると、
        ...
        density_month = min(計画の密度, 36.5) = **計画の密度**  ← 上限がそのまま戻る
        天井 = 923回 × その本数 ＝ 要る 500,000回/月 を上回る

    ——**伸び率を1つも触らずに、13倍の開きが埋まります。**

    ## **`25` を焼き込まないこと**（2026-08-30 に踏んだ）

    ここは長らく `pytest.approx(25.0)` と書いてありました。
    `PLAN_PUBLISH_PER_DAY` が **2026-08-30 に 25 → 13 へ落ちた**
    （`density_verdict` が「1時間より詰めても落ちない」を falsified にした）ので、
    **このテストだけが赤くなりました。** 落ちたのは計画の密度で、
    **このテストが見ている性質（バーストの速さを入れると上限が戻る）ではありません。**

    **だから定数を参照します。** ここが見たいのは
    「`density_month` が**供給ではなく計画の上限で決まってしまう**」ことで、
    その上限がいくつかは**この性質と関係ありません。**

    **覆る条件**: `density_month` が `min(計画の密度, 供給)` でなくなったら、
    この参照ごと書き直すこと。
    """
    m = _metrics()
    a = eta.analyse(m)
    pl = eta.plan(m, a, supply=_supply(36.53, 36.53))
    # **供給 36.53本/日 は計画の密度より大きい** —— でなければ min が供給側で決まり、
    # このテストが見たい「上限が戻る」が起きません。
    assert 36.53 > eta.PLAN_PUBLISH_PER_DAY
    assert pl["density_month"] == pytest.approx(eta.PLAN_PUBLISH_PER_DAY)

    # **ここから下は、2026-08-30 に外しました。理由を残します。**
    #
    # もとはこう書いてありました:
    #
    ...
    #
    # つまり「バーストの速さを入れると、**収益に届いてしまう**」まで固定していました。
    # **これは `PLAN_PUBLISH_PER_DAY` が 25 だったから成り立っていた帰結**で、
    # 25 → 13 に落ちた時点で成り立たなくなります（天井 923回 × 13本 ＝ 月 359,970回
    # < 要る 500,000回）。**バグが直ったからではなく、別の定数が下がったからです。**
    #
    # **このテストが見張っている穴は「速さの読み替え」のほう** ——
    # 3.3時間の実績を1日ぶんに引き伸ばすと、`density_month` が
    # **供給ではなく計画の上限で決まってしまう**。その機構は上の行で固定してあります。
    # **届くかどうかは、その機構とは別の問い**なので、ここでは主張しません。
    #
    # **代わりに、機構の効き目そのものを相対で固定します** ——
    # バーストを入れた天井は、1日続く速さの天井より**必ず高い**。
    # ここが崩れたら、読み替えの穴は塞がっていません。
    # （絶対値の対応は `test_1日続く速さを入れると同じ伸び率のまま届かなくなる` が持っています）
    sustained = eta.plan(m, a, supply=_supply(36.53, 3.5))
    assert pl["ceiling_day"] > sustained["ceiling_day"]
    assert pl["growth"]["g"] == pytest.approx(sustained["growth"]["g"]), \
        "伸び率は動かしていないこと（動かすと、天井の話ではなくなる）"
