...
def test_窓を見ろと言う(capsys):
    ...
    **詰めろとだけ言うと、測定を壊しにいかせます。**
    ...
    ## この検査は 2026-08-31 から 09-01 まで赤で立っていました

    08/31 にオーナーの規則（**1日1本・作り置きなし**）が入った日、
    `status._print_per_day` の薄い日の枝は
    **「これは規則どおりです。詰めないこと」だけ**になり、
    **`src/measure_window.py` への道しるべが消えました。**

    **消してよかったのは「詰めろ」のほうだけです。**
    「薄い日は空けてあるかもしれない」は規則と何の関係もありません ——
    規則の下でも、測定の窓は同じように空いています。

    **この検査を消して緑にしないこと。** 消すと、次に来た側は
    薄い日を見て `--compact` を撃ちにいきます（それがこの検査の由来です）。
    **覆る条件**: `src/measure_window.py` が無くなったとき。
    """
...
def test_穴の先の作り置きを名指しし埋め方を1つに絞る(capsys):
    """**08-31 に入った「詰めないこと」は、結論は合っていたが理由が空でした。**

    規則（1日1本・作り置きなし）が入った日、この枝は
    「先が空いているのは そういう作りです。詰めないこと」へ倒れました。
    **`gap` は `max(per)`（＝必ず1本以上ある日）より手前の0本の日しか拾わない**ので、
    穴の先には必ず作り置きが居ます（`test_最後の予約より先は穴と呼ばない`）。
    つまり「そういう作りです」は、この節では一度も成り立ちません。

    詰めてはいけない理由は別の所にありました ——
    `--compact` が動かすのは `at` だけで `uploaded_at` は残るので、
    詰めた本は `house_rule.is_stockpile()` の下で作り置きのまま、
    **`pool_drain` が同じ本を同じ日枠で外します**
    （実測 2026-09-01: 詰める 12本 のうち 11本 が池化の 267本 の中）。

    だからこの行は3つを言うこと:
      1. 穴の先に**何本**の作り置きが在るか（＝規則2がどれだけ破れているか）
      2. **詰めるな**（理由は上の `uploaded_at`）
      3. **埋める道は「その日の回が `upload` を1本 出す」1つだけ**

    **覆る条件**: オーナーが規則を外す／`is_stockpile` が `at` も見るようになったら、
    2 の理由が消えるのでこの検査ごと書き直すこと。
    """
    st._print_per_day(_ahead({"2026-08-20": 1, "2026-08-25": 3}), today=TODAY)
    out = capsys.readouterr().out
    assert "予約が1本も無い日が 4日" in out
    # **薄い日の節と混ぜないこと** —— あちらの「詰めないこと」は別の文です。
    hole = out.split("予約が1本も無い日")[1].split("1日2本以下の日")[0]
    assert "その穴の先に、もう作ってある予約が 3本 あります" in hole
    assert "詰めないこと" in hole and "is_stockpile" in hole      # 2（理由つき）
    assert "穴を埋める道は1つだけ" in hole and "upload" in hole   # 3
    # **「そういう作りです」に戻さないこと**（穴の定義の下では成り立たない）
