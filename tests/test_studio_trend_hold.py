...
def test_ledger_now_answers_the_question_section7_asked():
    ...
    **2026-09-14 11:0x に下端を 0 まで開けました**（optimizer・Opus）——
    `0 < pct` は**きょうの状態を不変条件として書いた側**で（§5 の教訓の形 6つ目）、
    **齢 6h に 1回 も付いていない本が出た周に赤くなります**。実際に出ました:
    5本目 `2YZ_4FXC-XI` は 09/14 11:0x の測りで **齢 95.5〜97.1h に 1回目** が付き、
    分母（いまの再生）が 0 → 1 になって `hold` に入り、**6h は 0%**（本当のこと）。
    **0% は「割合が出ていない」ではありません** —— 出ていない側は `at[6.0]` が空で、
    その本はこの `six` に入りません。**見張りたいのは「空でない」ことのほうです。**
    """
    got = tr.hold(tr.ledger_rows())
    six = [r for r in got if r["at"][6.0]]
    assert len(six) >= 4
    assert all(0 <= r["at"][6.0]["pct"] <= 100 for r in six)
    assert any(r["at"][6.0]["pct"] > 0 for r in six), "6h の割合が 1本も出ていない"
...
# ---------------------------------------------------------------------------
# いま 0回 の本は、落とすが黙っては落とさない（2026-09-11 06:0x・optimizer・Opus）
#   `hold` は分母（いまの再生）が 0 の本を落とす（0除算になる）。
#   落ちた本は「数えた N本」にも並びにも出ないので、**名指しする行が要る**。
# ---------------------------------------------------------------------------

def test_いま0回の本は落ちるが名指しされる():
    zero_rows = _rows([(0.3, 0), (10.0, 0), (19.3, 0)], vid="ZERO")
    got = tr.hold(zero_rows)
    assert got == []                                  # 分母が作れないので並びには出ない
    zero: list = []
    tr.hold(zero_rows, zero=zero)
    assert [z["id"] for z in zero] == ["ZERO"] and zero[0]["age_h"] == 19.3
    # 他の本と一緒でも、数えた本数と 0回 の本は別に出る
    both = zero_rows + _rows([(1.0, 10), (6.0, 40), (30.0, 100)], vid="LIVE")
    line = "\n".join(tr.hold_lines(both))
    assert "数えた 1本" in line
    assert "いま 0回 の本 1本（ZERO 齢19.3h）" in line


def test_0回の本が無ければ_その行は出ない():
    """**陽性対照**: 落とした本が 0本 の回に、この行を印字しないこと（毎周の字を増やさない）。"""
    line = "\n".join(tr.hold_lines(_rows([(1.0, 10), (6.0, 40), (30.0, 100)], vid="LIVE")))
    assert "いま 0回 の本" not in line
