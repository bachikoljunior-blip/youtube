...
def test_焼き直しの案内と同じ所に出ている() -> None:
    ...
    # **註の中の同じ字を拾わないこと**（2026-09-03 15:5x に赤かった）—— 09/03 13:50 の直しが
    #     同じ文を**コメント**（`#      「→ 焼き直すのが …」と`）で引いたので、`index()` は
    #     そちらを先に見つけ、実物との差が 1,095字 になってこの検査が落ちていました。
    #     見たいのは**印字する行**なので、`out.append(` ごと索くこと。
    i = src.index('out.append("  → **焼き直すのが `improve` の1手です**')
