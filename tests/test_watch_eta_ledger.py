...
def test_実物の待ちが台帳の期限を指している() -> None:
    ...
    2026-08-26 の実測。4件とも文面のほうが古く、**そちらが読まれていました。**

        登録の依頼-30000再生         文面 2026-09-14 ／ 台帳 2026-10-11（27日 古い）
        長尺-1000再生                文面 2026-09-15 ／ 台帳 2026-11-22（68日 古い）
        族べつ登録率-15000再生       文面 2026-09-20 ／ 台帳 2026-09-17
        冒頭のstatの割り方-両群16本  文面 2026-09-20 ／ 台帳 2026-10-06（16日 古い）

    ## **台帳の日付をべた書きしないこと**（2026-08-26 夕に直した）

    ここには上の「台帳」側の4つの日付が `want` としてべた書きしてあり、
    **台帳を正しく動かした回に落ちました** ——
    `scripts/deadline_check.py --shrink` が `冒頭のstatの割り方` を
    10-06 → **09-05** へ縮めた回です（データはもう揃うのに期限が 31日 先だった）。

    **この検査が守りたいのは日付ではなく「どちらを読むか」**です。
    日付を写した瞬間に、**この検査自身が3か所目の写し**になり、
    台帳を直すたびに赤くなって、直す側に「台帳のほうを戻せ」と言い始めます。
    だから見るのは**関係だけ** —— 文面に期限が無いこと、
    そして `deadline_of()` が台帳の値を返すこと。
    """
