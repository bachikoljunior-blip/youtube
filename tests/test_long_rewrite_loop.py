...
def test_耳で言った数の直し方が書いてあること():
    """**検査を足したら、直し方も足すこと**（2026-08-26）。

    同じ回に `_check_narrated_shown` を長尺へ広げました（それまで
    `if portrait:` の中にあり、**長尺は1本も通っていなかった** ——
    実測で長尺 7/33本 = 21%）。指摘の本文は
    「棒か行か見出しに足すこと」としか言っておらず、
    **この文書の他の2行と引っぱり合います**:

        「尺は縮めないこと」            → その文を消す道が塞がっている
        「計算出力に出ている桁で書け」  → 数を勝手に作る道も塞がっている

    **逃げ道が書いていないと、書き直しの3回が別の指摘に化けます**
    （消す → 4分を割って落ちる／作る → 計算出力に無いと言われる）。
    だから**出口を2つとも名前で**書いてあること。
    """
    long_txt = script_writer.LONG_FIX_GUIDANCE
    assert "画面のどこにも出ていません" in long_txt, (
        "`_check_narrated_shown` の指摘に、直し方が書いてありません"
    )
    # 出口(1) 画面に出す —— **棒は `display`**（`value` は棒の長さで、字は出ない）
    assert "display" in long_txt
    # 出口(2) 言わない —— **ただし尺は保つ**
    assert "同じくらいの長さの別の文に差し替える" in long_txt
