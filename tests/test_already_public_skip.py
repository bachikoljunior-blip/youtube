...
def _枠の番人を外す(monkeypatch):
    """**日枠の番人を、この検査ファイルのあいだだけ黙らせる。**（2026-08-31）

    ## なぜ要るか（**赤の意味が薄れていました**）

    下の3件は `videos.update` まで届く道を通るので、`reschedule._update` の
    **`upload_cap.reserve_hold()`（実物の日枠の控えを読む番人）**に当たって
    `SystemExit` で落ちていました ——

        [reschedule] **この窓の単位は、計測のぶんを残して止めています**
        （使った 9,400 ／ 実測の枠 9,050 ／ 残す 400）

    **コードは壊れていません。** `data/day_quota.jsonl` が
    「きょうの枠は尽きた」と言っているだけで、**枠が尽きるたびに赤くなる作り**でした
    （同じ枝の別の回が同日に数えた: `reschedule` 系の赤 **10件**は全部これ）。

    **これは「止める仕掛け」を外すのではありません。** 番人は本番のまま
    （`scripts/reschedule.py` は1文字も変えていません）。
    外しているのは**検査が実物の可変な状態を読んでいること**のほうです ——
    この3件が守っているのは「**公開済みの関門が、ふつうの入れ替えを止めていないこと**」で、
    **日枠の番人はその対象ではありません。**

    ## これを直す値打ち

    **枠が尽きた日に 10件 が赤くなると、赤の意味が薄れます。**
    次の回は赤を見て「またこれか」と読み、**本物の壊れを同じ顔で見逃します。**
    検査は「いま測れない」と「壊れている」を別の顔で言うこと。

    ## 覆る条件

    - **番人そのものを検査したくなったら**、その検査はこのファイルの外に置くこと
      （ここで外しているので、ここには書けません）
    - `reserve_hold` / `move_hold` の名前が変わったら、この fixture は
      `raising=False` で黙って効かなくなります。**そのときは下の3件がまた
      枠の状態で赤くなる**ので、名前で気づけます
    """
