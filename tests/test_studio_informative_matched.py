...
def test_本物の台帳では門は齢と長さの側で読む():
    ...
    **見張る先を次の門へ移します** —— いまの見張りは 2つ:
      ...
      (a) ~~読む側が `齢＋長さ` であること~~ → **(6) の「幅」が 3回 続けて門の下に入らないこと**
      (b) その側の比が **0.5倍 を切っていない**こと（＝ 門 (2) が本当に引かれる日。判定は `hourly`・§5）

    **【2026-09-12 03:1x に (a) を次の門へ移しました】** (a) は 09/12 03:0x に**狙いどおり赤になって
    教えました** —— `side_hi` が 0.1614 → **0.0747** に落ち、側が `齢だけ` へ戻った
    （`gate_span` の覆る条件 (5) ＝ この回に初めて引かれた）。**越えたあとも同じ assert を残すと
    毎周 赤のまま**なので、09/10 12:3x と同じ手で見張る先を次の門へ移します:
    **(6)**（幅が門の下で `SIDE_RUN_LAPS` 回 続いたら、側は点で読んでよい ＝ (5)(6) を畳む）。
    **側そのものは assert しません** —— **「きょうの状態」を不変条件に書かない**
    （§5 の「教訓の形」6つ目）。側は `trend` が毎周 印字します（`_side_verdict_line`）。
    ...
    **鋸の歯のどこで撃っても答えが変わりません。**
    ...
    assert int(v["narrow_run"]) < trend.SIDE_RUN_LAPS, (
        f"差の幅 {float(v['hi']) - float(v['lo']):.3f} が 門 {trend.GAP_SPLIT} を"
        f"測り {v['narrow_run']}回 続けて下回った ＝ 側の鋸の歯は消えている。"
        "`gate_span` の覆る条件 (6) を引くこと（側は点で読んでよい ＝ (5)(6) と "
        "`side_verdict` を畳む）")
