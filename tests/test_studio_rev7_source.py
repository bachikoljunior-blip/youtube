...
def test_同じ日までを引き直した回は_1つに畳む():
    """**空引きは刻も値も進めない**（`rev7_draws` と同じ規則 ＝ **最初の読みが残る**）。

    2026-09-14 02:5x に `hourly` が直しました（01:0x の optimizer の申し送り）——
    それまでは `kept[-1] = d` で**あとの読みが勝ち**、空引きのたびに「最後の引き」の刻だけが進んでいました。
    """
    ...
    assert s["draws"][0]["studio"] == 400        # 最初の読みが残る
    assert s["draws"][0]["at"].strftime("%m/%d %H:%M") == "09/10 16:07"


def test_空引きは_見かけの段を_old_runに入れない():
    """**陽性対照**（§5 教訓の形 3つ目）—— 直す前の規則（あとの読みが勝つ）だと、
    **引き直した日だけが熟した読み**になり、その日で上がって次の日で下がる段が立ちます。

    下の並びは `old` が 1000 → 1000 → 900 で、**本当は 1度も上がっていません**。
    あとの読みが勝つと 09/07 だけが 1200 になり、`old_run` に 1 が立ちます。
    """
    rows = (_draw("10T16:07:00", "2026-09-07", "2026-08-31", {"n": 468}, {"o": 1000})
            + _draw("10T16:18:00", "2026-09-07", "2026-08-31", {"n": 468}, {"o": 1200})
            + _draw("11T12:18:00", "2026-09-08", "2026-09-01", {"n": 468}, {"o": 1000})
            + _draw("12T08:25:00", "2026-09-09", "2026-09-02", {"n": 468}, {"o": 900}))
    s = trend.rev7_source(rows)
    assert [d["old"] for d in s["draws"]] == [1000, 1000, 900]
    assert s["old_run"] == 0
