...
CHECKABLE = {
    ...
    # **2026-09-02 04:0x に、この行は閉じて外しました**（`verdict`・survived）。
    # 立てたのも閉じたのも同じ日で、`earliest_now` は**このために足した床**です。
    # **関数は残してあります** —— `needs: kind: now` の前提は今後も立つので、
    # 次に立った1件はここへ `earliest_now` で1行 足せば済みます。
    #   閉じた根拠: `python -m src.form_tail`（Data API 0単位）の2本
    #     (1) 本数の差では説明できない（間引き 2,000回・p < 0.0005）
    #     (2) **その日の出来事では説明できない**（`form_tail.shelf()`:
    #         上端 1,891回 の 75% 以上に 10本・初観測日 7日・幅 17日 ＝ 棚）
    #   `config/hypotheses.yaml` の `confirmed:` の先頭に全文があります。
    #
    # **同じ回が、その続きを1件 立て**（`premise`）、
    # **2026-09-02 07:0x の回が期限どおり閉じました**（`verdict`・survived）。
    # だからここからも外してあります —— **閉じた前提を CHECKABLE に残すと、
    # `deadline_of()` が「開いている前提が 0件」で赤くなります**
    # （閉じた回がここを直さず、次の回が拾う形を3回 繰り返しています）。
    #   閉じた根拠: `python -m src.form_tail`（Data API 0単位）——
    #     日ごとの**最大**の弾力性 b=+0.059（95% [-0.144, +0.262]）は、
    #     日ごとの**平均**の b=-0.364 を**含まない** ＝ 本数を減らして上がるのは
    #     平均だけで、上端は動かない。天井 4,101回 は外挿ぶんだけ ×1.49 の上振れ。
    #   `config/hypotheses.yaml` のその前提に `verdict` の全文があります。
    #
    # **`earliest_now` は残してあります** —— `needs: kind: now` の前提は今後も
    # 立つので、次に立った1件はここへ1行 足せば済みます。
    #
    # **2026-09-02 11:5x に1件 足し**（`premise`・`needs: kind: now`）、
    # **同じ日の 16:5x に閉じました**（`verdict`・**falsified**・最適化の回
    # agent-a00f32febfccf7639）。だからここからも**同じ回で外しました。**
    #
    # **上の註が「閉じた回がここを直さず、次の回が拾う形を3回 繰り返しています」と
    # 書いています。4回目にしないための1行です。**
    #   閉じた根拠: `python -m src.clarity`（**API 0単位**・n=113・門 0.185）——
    #     再生の側は **7つとも門に届かず**（`文字/コマ` は -0.002 ＝ 門の 1%）、
    #     維持率の側は **-0.200 ★**（門を越える）＝ 計器も標本も生きている。
    #     ＝ 維持率で門を越えた物差しは、**再生数のほうには効かない**。
    #   n を待たなかった理由: `scripts/retention.py` をこの回に2度 撃って
    #     **2度とも 0本**（63本 は上流にカーブがまだ無い）。規則1 の下で
    #     `clarity_books()` が 135本 に届くのは **約460日 先**です。
    #   `config/hypotheses.yaml` のその前提に `verdict` と `next_done` の全文があります。
    #
    # **`earliest_clarity_views` は残してあります** —— `needs: kind: now` で
    # `data/retention.json` × `data/views.jsonl` を読む前提は今後も立つので、
    # 次に立った1件はここへ1行 足せば済みます。
}

#: 当てられない件と、その理由。**docstring と同じ並び。**
UNCHECKABLE = {
    # 2026-09-03 21:0x（最適化の回）に足した（`improve`）。床は
    # **「片群 30,000再生」＝ 将来の再生の合計**なので、この file の冒頭が
    # 名指ししている「台帳の数え上げに落ちないもの」そのものです
    # （すぐ上に在る `ショートの最後で登録を直接1回頼む` と**同じ形**）。
    # `earliest_*` は「予約に既に在る本から床を数える」道具で、
    # **予約に在る本が将来いくつ再生を積むかは、予約表から出ません。**
    #
    # **判定日 2027-03-04 は、勘ではなく測った数です**（立てた回が数えた）:
    #   2026-08-26 以降 公開 228本 → 合計 2,636再生 ＝ **片群 164.8再生/日**
    #   → 片群 30,000再生 に **182日**
    #   （直近5日だけで数えると 片群 10.7〜16.0/日 ＝ **1,875〜2,812日**。
    #     上は**速い側**を採っています —— 遅い側が続くなら期限を延ばすこと）
    #
    # **`CHECKABLE` へ移す条件**: 移しません。**床が再生の合計である限り、
    # 予約表からは当たりません。** 数で鳴らすなら `config/watches.yaml` の
    # 待ち（`kind: accrual`）の側で、この検査ではありません。
    "登録の依頼を画面（全時間）にも置くと":
        "床が『片群 30,000再生』（将来の再生の合計）。予約表から出ないので earliest_* が当たらない",
    # 2026-09-03 02:xx（最適化の回）に足した（`premise`）。床は `kind: after`（09-07）＝
    # 「**外の作りを写した長尺が1本 公開されて 48時間**」で、**その本はまだ作られていない**
    # （予約にも池にも無い。作るのは主実行の「その日の1本」の決め）。
    # `earliest_*` は「予約に既に在る本から床を数える」道具なので当てられません。
    # 判定日は `deadline: 2026-09-09`（09/05 の枠 ＋ 48h ＋ 余白 2日）が持ちます。
    #
    # **`CHECKABLE` へ移す条件**: その本が `--draft` で上がって `[次の枠]` に入ったら、
    # 床は「その本の公開 ＋ 48h」の引き算になるので `earliest_*` の形で移せます。
    # **09-09 までに作られていなければ、前提の外れではなく「主実行が作り方を変えなかった」**
    # （`falsified_if` の註）—— その回が理由を日誌に書いて延ばすこと。
    "外の今年の上位の作り（尺 20分以上":
        "床は『まだ作っていない長尺 1本 ＋ 48h』。予約に無い本なので earliest_* が当たらない",
    # 2026-09-04 20:2x（毎時の回）に足した（`premise`）。**すぐ上の長尺の対で、形がショート**。
    # 床は「**まだ作っていないショート 1本 ＋ 48h**」＝ 予約に無い本なので `earliest_*` が
    # 当たりません（長尺の側とまったく同じ理由）。判定日は `deadline: 2026-09-08` が持ちます
    # —— 09/05 は長尺（09-07 の判定用）で埋まっているので、最短で 09/06 の枠 → 48h が 09/08。
    # **余白は持たせません**（持たせると `test_遅すぎる期限が残っていないこと` が赤）。
    # **枠の取り合いにしないこと**（規則1 ＝ 1日1本）。
    #
    # **`CHECKABLE` へ移す条件**: その本が `--draft` で上がって `[次の枠]` に入ったら、
    # 床は「その本の公開 ＋ 48h」の引き算になるので `earliest_*` の形で移せます。
    # **09-14 までに1本も公開されていなければ、前提の外れではなく
    # 「主実行がショートの枠をこの作りに回さなかった」** —— その回が理由を日誌に書いて延ばすこと。
    "外の帯の上位の**ショート**の作り（尺 100〜180秒":
        "床は『まだ作っていないショート 1本 ＋ 48h』。予約に無い本なので earliest_* が当たらない",
    # 2026-09-03 16:2x（定期の回）に足した（`premise`）。床は「**上限 2周 で焼いた本と
    # 4周 で焼いた本が、それぞれ 2本以上 公開されて 48時間**」＝ **まだ1本も焼いていない本**
    # （2周の群は 0本。上限を下げる前なので、そもそも存在しない）。
    # `earliest_*` は「予約に既に在る本から床を数える」道具なので当てられません。
    # 判定日は `deadline: 2026-09-17`（規則 1日1本 で片側 2本ずつ ＝ 最短 4日 ＋ 余白）が持ちます。
    #
    # **`CHECKABLE` へ移す条件**: 2周 で焼いた本が `--draft` で上がって `[次の枠]` に
    # 入ったら、床は「その本の公開 ＋ 48h」の引き算になるので移せます。
    # **09-17 までに 2周 の本が1本も焼かれていなければ、前提の外れではなく
    # 「上限を下げてみた回が無かった」** —— その回が理由を日誌に書いて延ばすこと。
    "分かりやすさの輪の上限を 4周 → 2周 にしても":
        "床は『上限 2周 で焼いた本 2本 ＋ 4周 の本 2本 の 48h』。2周 の本はまだ 0本 なので earliest_* が当たらない",
    # 2026-09-04 15:2x（定期の回）に足した（`premise`）。床は「**門の後に焼いた
    # 外の作りの長尺が 3本 公開されて 48時間**」＝ **まだ焼いていない本**
    # （門を入れたのが同じ回なので、置いた群は 09/05 の1本 から始まります）。
    # `earliest_*` は「予約に既に在る本から床を数える」道具なので当てられません。
    # 判定日は `deadline: 2026-09-15`（規則 1日1本 で 3本 ＝ 最短 3日 ＋ 48h ＋ 遅れ ＋ 余白）が持ちます。
    #
    # **`CHECKABLE` へ移す条件**: 置いた群の本が `--draft` で上がって `[次の枠]` に
    # 入ったら、床は「その本の公開 ＋ 48h」の引き算になるので移せます。
    # **09-15 までに 3本 に届かなければ、前提の外れではなく「その日の1本が長尺で
    # なかった日が続いた」** —— その回が理由を日誌に書いて延ばすこと。
    "冒頭 120文字以内に「知らない側がどうなるか」の1文を置くと":
        "床は『門の後に焼いた外の作りの長尺 3本 の 48h』。まだ 0本 なので earliest_* が当たらない",
    # 2026-09-02 夕（最適化の回）に足した（`premise`）。
    # 床は「**`scripts/niche_ceiling.py` を別の日に撃ち直す**」だけで、
    # **貯まるのを待つ本も日もありません**（`needs:` を持たない前提です）。
    # `earliest_*` は「予約に既に在る本から床を数える」道具なので、
    # 予約と無関係なこの前提には当てられません。
    # 縛っているのは `search.list` の**日枠**（実測 2026-09-02 は4語のうち3語が 429）で、
    # 枠は毎日 戻るので `deadline: 2026-10-02` は十分に届きます。
    #
    # **`CHECKABLE` へ移す条件**: 移しません。**床が無いので、いつでも判定できます** ——
    # 期限より前に撃てた回が、そこで閉じること。
    "`per_video` の天井 4,229回 は帯（このニッチ）の天井ではなく":
        "床が無い（別の日に niche_ceiling.py を撃ち直すだけ）。予約の本数と無関係なので earliest_* が当たらない",
    # 2026-09-02 07:4x に足した（`premise`）。床は `kind: accrual` ＝
    # 「**伸びきった日**が 25日 → 45日 に増えたら」で、増えるのは
    # **1日1本の規則の下で実際に公開された日**だけです。
    # `earliest_*` は「予約に既に在る本から床を数える」道具ですが、
    # ここで数えるのは本ではなく**日**で、しかも暦に穴があると増えません
    # （`run_marker.py --write` の `[暦]`）。だから当てられません。
    # 判定日は `kind: accrual`（`deadline_check` が伸び率から解く）が持ちます。
    #
    # **`CHECKABLE` へ移す条件**: 暦の穴が埋まって「1日1点ずつ増える」が
    # 実物で成り立つようになったら、床は日数の引き算になるので移せます。
    "1本あたり再生の上端（棚・記録 1,891回）は**チャンネルの大きさで決まって**おり":
        "床が『伸びきった日 45日』（kind: accrual）。予約の本数ではなく公開された日の数なので earliest_* が当たらない",
    # 2026-09-01 夜（最適化の回・別のサブ）に足した前提。**登録が漏れていて、
    # 09/01 16:3x から 09/02 04:0x まで この検査を赤にしていました**
    # （立てた回が走っている最中だったので、当時の回は手を出さずに日誌へ残した）。
    #
    # 床は `kind: accrual` ＝「`data/video_forms.json` の札が 196本 → **215本** に
    # 増えたら」で、**増やすのは Analytics の上流**です（`python -m src.rpm_mix --forms`
    # を撃っても、向こうがまだ作っていなければ 0行 増えません）。
    # `earliest_*` は「**予約に既に在る本**から床を数える」道具なので、
    # 予約を1本も動かさずに動くこの数には当てられません。
    # 判定日は `kind: accrual`（`deadline_check` が伸び率から解く）が持ちます。
    #
    # **`CHECKABLE` へ移す条件**: 札の付かない本が 0本 になったら
    # （＝ 標本が完全になったら）、床は「きょう」になるので `earliest_now` で足ります。
    "いまの天井 4,101回 は、チャンネルの天井ではなく標本の縁である":
        "床が Analytics の札の到着（196→215本・kind: accrual）。予約の本数ではないので earliest_* が当たらない",
    ...
    # **同じ形です**（2026-09-03 19:5x に足した `sub_rate` の1件）。
    # 床は「09/05 以降に公開した本の再生の合計 15,000」で、
    # **予約の本数でも控えの件数でもない**ので `earliest_*` が当たりません。
    "登録の依頼を説明欄の先頭":
        "条件が「計15,000再生」＝これから積む再生の合計（『チャンネルのホームに紹介動画とバナーを置く』と同じ形）",
    ...
    # 2026-09-01 夜（最適化の回）に足した。床は「09/03〜09/23 に **1日1本 で連続公開した本**
    # 10本以上」＝ **これから起きる暦**で、しかもその暦はまだ存在しません
    # （同じ回の実測: 09/05〜09/23 の 19日が空。`src/next_slot.calendar()`）。
    # `earliest_*` は「予約に**既に在る**本から床を数える」道具なので、
    # **予約が空の日は数えようがありません。** 判定日は `kind: after`（10-01 ＋ 遅れ）が持ちます。
    #
    # **`CHECKABLE` へ移す条件**: `scripts/reschedule.py --compact --apply` が
    # 撃たれて 09/03〜09/23 が 1日1本 で埋まったら、そこは「予約に在る本」になるので
    # `earliest_*` と同じ形で床を数えられます。**そのとき移すこと。**
    "前後も規則の密度だった日": "床が『これから 1日1本 で連続公開する本 10本』で、その暦がまだ予約に無い（19日 空）",
    # 2026-09-02 10:2x に足した（`premise`・オーナー固定その3「動画内の説明は人間に
    # わかるようにして」の側）。床は `kind: accrual` ＝ `clarity_books()` が
    # **135本**（＝ 控えと維持率カーブが**両方ある**本）に届いたら。
    #
    # `earliest_*` は「予約に**既に在る**本から床を数える」道具ですが、
    # ここで数えるのは**予約ではなく、カーブを取り終えた本**です ——
    # 予約に在っても `data/retention.json` に入るまでは 1本も数えません
    # （入れるのは `python scripts/retention.py`。Analytics の枠で、Data API とは別）。
    # だから当てられません。判定日は `kind: accrual` が持ちます
    # （`deadline_check` が `clarity_books()` の伸び率から解く。
    #  **規則（1日1本）で押さえてあります** —— 貯めの井戸は 2026-09-02 に尽きており、
    #  ここから増えるのは公開した本のぶんだけ。`_counts_videos()` の註）。
    #
    # **`CHECKABLE` へ移す条件**: 公開した本のカーブが**その日のうちに**
    # `data/retention.json` へ入る作りになったら（＝ 予約 ＝ 標本になったら）、
    # 床は予約の本数の引き算になるので移せます。
    "一息に載せる数の個数が多いほど":
        "床が『控えと維持率カーブが両方ある本 135本』（kind: accrual）。予約に在る本ではなく、カーブを取り終えた本を数えるので earliest_* が当たらない",
}
