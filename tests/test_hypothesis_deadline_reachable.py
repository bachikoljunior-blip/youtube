...
UNCHECKABLE = {
    ...
    # 2026-08-28 に足した（前提は同じ日の別の回が置き、ここが空のままだった）。
    # 条件は「08/29〜09/04 に公開した帯内ショートの、日ごと中央値の中央が
    # **341 以上に戻っているか**」＝ **将来の絶対値**です。
    # `earliest_*` は「予約に既に在る本から床を数える」道具なので、
    # 「戻ったか」には当てられません。判定日は `kind: after`（09-04 ＋ 遅れ）と
    # `watch: 配信抑制-0824` が持ちます。
    "1本あたり再生が 08/24 に落ちたのは": "条件が将来の絶対値（帯内の日ごと中央値が 341 へ戻るか）",
}
