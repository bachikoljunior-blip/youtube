...
def test_期限は判定できる最短の日より後にあること(key: str):
    ...
    ## **閉じた前提の鍵は飛ばします**（2026-09-02 16:2x に構造で直した）

    ここは長らく、前提が閉じるたびに `CHECKABLE` から**手で1行 消す**必要が
    ありました。上の註がその履歴です —— **「閉じた回がここを直さず、次の回が
    拾う形を3回 繰り返しています」「同じ所を数えて5回目です」。**
    **5回 同じ所で赤くなったなら、直すのは回ではなく形のほうです**
    （`docs/GOAL.md`「同じ種類の失敗を繰り返したら、文書に書き足すのではなく
    機械にすること」）。

    **鍵が残っていても害はありません** —— 開いている前提が無ければ、
    この検査には当てるものがないだけです。**同じ形の前提が次に立った回は、
    その1行がそのまま効きます**（登録し忘れは
    `test_開いている前提は全部どちらかに入っていること` が別に捕まえます）。

    **緩めていません**: 開いている前提が1件でも当たれば、下の2つの assert は
    そのまま当たります。飛ばすのは **0件のときだけ**です。
    """
    if not [h for h in open_with_deadline() if key in str(h.get("claim", ""))]:
        pytest.skip(f"「{key}」に当たる開いている前提はありません（閉じた鍵）")
