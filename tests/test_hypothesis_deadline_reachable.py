...
UNCHECKABLE = {
    # 2026-09-05 01:5x（毎時の回）に足した。**00:3x の回が立てた前提が、どちらにも
    # 入っていないまま赤で残っていました**（`test_開いている前提は全部どちらかに入っていること`）。
    #
    # 床は「**台帳の門が、譲る側の実測の上に置き直されること**」＝ 動くのは
    # `config/hypotheses.yaml` の**書き方**で、予約表の本ではありません。
    # `earliest_*` は「予約に既に在る本から床を数える」道具なので、当てられません。
    # 判定日は `deadline: 2026-09-12` が持ちます。
    #
    # **同じ回（01:5x）が、この前提の中身は先に実装しています** ——
    # `src/slot_cost.win_band()` が当たりを2帯（`unpaid` / `paid`）に割り、
    # `config/hypotheses.yaml` の3か所（長尺の前提の `falsified_if` 追記・
    # `next_if_false` 3.・姉妹のショートの前提の覆る条件）を書き直しました。
    # **実装は前提の判定ではありません** —— 判定は「置き直した門で、
    # 実際に選ぶ実験が変わったか」で、09-12 にその回が数えること。
    #
    # **`CHECKABLE` へ移す条件**: 移せません。**床が「台帳の書き方」である限り、
    # 予約表からは出ません。** 数で鳴らすなら、`data/daily_pick.jsonl` の
    # `anyway` が付いた決めの件数（＝ 門を越えて枠を取った回）を数える側で、
    # この検査ではありません。
    "枠を1本 使う実験は、その当たりの門が":
        "床は『台帳の門の置き直し』。予約に在る本から出ないので earliest_* が当たらない",
