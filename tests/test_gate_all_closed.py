...
条件そのものを字下げして書く形に `AUTOMATION_PAUSED.md` が変わったら、
...
def test_正本の門はいまも6件():
    """**この数が動いたら、まず `AUTOMATION_PAUSED.md` の書き足しを疑うこと。**"""
...
def test_門が全部閉じても解除は別の1手だと印字する():
    """**閉じた ≠ 解除**。停止は人の手で入っているので、そう書いてあること。"""
    import importlib

    eta = importlib.import_module("eta")
    body = "\n".join(eta.gate_lines("###"))
    if len(resume_gate.open_items()) == 0:
        assert "解除はまだ別の1手" in body
