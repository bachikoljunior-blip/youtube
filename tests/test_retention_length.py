...
def test_導出の旗は全部の本に立っていて_真偽で読める():
    """秒の議論に使う回が、**±0.5÷(割合/100) 秒**の幅を書けるように。

    ## **2026-09-05 05:0x に書き直しました**（それまで赤で止まっていた）
    ...
    **その「いまの走査」は、もう「いま」ではありません。** 走査が `尺` を
    自分で持つようになったので、**導出で埋める本は 0本 が正しい状態**です。
    ところが検査は「1本も導出が無い」で落ちるので、**走査が良くなるほど赤くなる**
    形になっていました（実測 2026-09-05: 導出 0本 で赤）。

    **旗そのものは、無くなっていません。** 幅を書く回に要るのは
    「この本の尺は導出か」で、**導出が 0本 なら『幅は要らない』が答え**です。
    だから見るものを「1本は導出であること」から
    **「旗が全部の本に在って、真偽で読めること」**へ移します
    （上の `test_走査から尺が拾える` が `"尺_導出" in v` を見ているので、
    ここは**値が真偽であること**と、**導出の本には幅が引けること**を見ます）。

    **覆る条件**: 走査が `尺` を落とすようになったら（API の欄が変わる等）、
    導出の本が 0本 でなくなります。**そのときも、この検査は緑のままでよい** ——
    赤にすべきなのは `test_走査から尺が拾える` の `v["尺"] > 0` のほうです。
    """
    vs = retention.videos()
    for v in vs:
        assert isinstance(v["尺_導出"], bool), (
            f"`尺_導出` が真偽ではありません: {v['尺_導出']!r}（{v.get('id')}）")
    derived = [v for v in vs if v["尺_導出"]]
    # 導出の本が在るなら、幅（±0.5 ÷ (割合/100) 秒）が引けること。
    for v in derived:
        assert v["尺"] > 0, f"導出なのに尺が 0 以下です（{v.get('id')}）"
