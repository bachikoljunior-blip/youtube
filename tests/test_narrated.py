...
KNOWN_UNSHOWN = {
    ...
    "plmBPZqqP1U",
    # --- 2026-08-26 06:0x に増えたぶん（4本とも長尺。中身を見ました）---
    #
    # **4本とも「支えの数」です。** 表の欄でも棒でもなく、
    # **その数を正当化するために足した1つ**が、画面に出ていません:
    #
    #   jYoDuz0S9z4  `81万6000` —— 3級の最低保障 61万2000円 を
    #                「障害基礎年金2級の年81万6000円の4分の3」と説明する。
    #                **割られる数のほうが絵に無い**（型1）
    #   xQ2EzmkHRjw  `37万2500` —— 「控除前の所得税額」。表は総合 65万222円 と
    #                分離 70万9902円 の2列しか出しておらず、**引く前の数が声だけ**（型2）
    #   a63FzIUV2wI  `1544万8761` —— 3つ読み上げた行の**まん中だけ**絵に無い（型2）
    #   jeRBnxQjBvY  `330万` `110万` —— 公的年金等控除の適用条件（型1）
    #
    # **ここが増え続けるのは、`src/verify.py` の門が長尺に掛かっていないからです**
    # （`_check_narrated_shown` の docstring:「**ショートだけに掛けます**」）。
    ...
    #
    # --- 2026-08-26 夕（最適化の回）に増えた1本。**門の穴ではありません** ---
    #
    #   nTLSU-dvuhw  `210万2千` `240万5千` `270万7千` `301万` —— **4件**。
    #                絵は各コマ「67歳0か月の差 −36,329円 ／ 差 ＝ 275,184円 − 238,855円」
    #                しか出さず、**声だけが額面（210万2千円）と2つの率
    #                （89.3% / 79.0%）を言います。** 型1（割られる数が絵に無い）が
    #                **4コマ ぶん繰り返された形** ＝ その calc の長尺の**型**のほうです。
    #
    # **この本は、門が長尺へ広がった 33分 後に作られています**
    # （門 `fc757cb` 2026-08-25T22:06:23Z ／ この本 22:39:53Z）。
    # **門をすり抜けたのではなく、作った側にまだ届いていなかった**ぶんです ——
    # 実測（`data/critique_queue` の長尺を、門の時刻で割った）:
    #
    #     門より前  長尺 33本 → 漏れ **7本（21.2%）**
    #     門より後  長尺 12本 → 漏れ **1本（8.3%）** ← **それがこの1本だけ**
    #                          この本より後に作った長尺は **11本とも漏れ 0**
    #
    # **つまり門は効いています。** ここに足すのは「見逃し」ではなく
    # **伝播の遅れで1本 取り残された**という記録です。
    # **この本はまだ予約の中です（公開 2026-10-03）。** 作り直せば消えます ——
    # 枠が戻った回が、`nenkin-fueta-bun-no-teduke-767` を1本 撃ち直すこと。
    # **そのときは、この行も一緒に外すこと**（`bad` と一致しなくなって赤になります）。
    "nTLSU-dvuhw",
    #
