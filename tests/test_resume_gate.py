...
def test_a_same_day_burst_of_closes_is_not_a_measured_rate(tmp_path):
    ...
    **測っているのは閉じる速さではなく、停止が始まってからの経過**です。
    ...
    # **ここの数は 2026-08-30 に変わりました**（同じ日に、別の回が同じ穴を
    # 別の側から塞いでいます）。窓の門はそのまま残っていますが、**窓が開いた後に
    # 何で割るか**が「件数 ÷ 経過」から**間隔**に替わりました
    # （`src/resume_gate.days_per_close()` に実測の表）。
    #
    #     直す前  3件 ÷ 3日 ＝ 1.00件/日 → 残り3件は **3.0日**
    #     いま    間隔は 0日（同じ日に固まっている）／最後に閉じてから **3日**
    #             → 1件あたり 3日 → 残り3件は **9.0日**
    #
    # **同じ日にまとめて閉じた3件は、間隔について何も言っていません**（この
    # 検査の docstring がそう言っています）。窓が開いた瞬間にその3件を
    # 速さへ繰り入れると、**同じ壊れ方が3日 遅れて出るだけ**でした。
    assert resume_gate.rate_per_day(text, led, ok) == pytest.approx(1 / 3)
    assert resume_gate.days_to_close(text, led, ok) == pytest.approx(9.0)
