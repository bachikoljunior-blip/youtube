...
REVIEWED_UNBACKED = {
    ...
    # 2026-08-30 に増えたぶん（型1。**中身を実際に見て足しています** —— 最適化の回）。
    # 鳴っていたのは **`10,420`** の1件だけ。angle の原文は
    # 「合計40,420円。よく見る計算の30,000円より **10,420円** 多い」——
    # **書いた側が、両端を同じ文の中に書いています。**
    #
    #     40,420 − 30,000 = 10,420   ← 表は両端を印字し、差は印字しない
    #     両端 40,420（合計）/ 30,000（よく見る計算）は、どちらも
    #     `src.calc.aoiro` の「e-Taxで増える10万円…」の 5,000,000円 の行に在る
    #
    # **すぐ上の `aoiro-65man-jikkouritsu-nanadan` と同じ表・同じ形**です
    # （あちらは 65万円控除の段、こちらは e-Tax の 10万円の段）。理由は上を読むこと。
    "aoiro-etax-jikkouritsu",
    # 同じ日。鳴っていたのは **`60,000`** の1件だけ。angle の原文は
    # 「社会保険料は **60,000円ずつ** 動いていることを、5行を上から下まで
    #  突き合わせて確かめる」—— **きざみ**です（`ribo-300000-payment-step` と同じ形）。
    #
    #     840,000 − 780,000 = 60,000   ← 表は5行の実額を印字し、きざみは印字しない
    #     5行（780,000 / 840,000 / 900,000 / 960,000 / 1,020,000）は
    #     `src.calc.furusato` の「同じ年収600万で、社会保険料率だけを動かす」に在る
    #
    # [!] **`_checks.near_candidates` は「60,600 を丸めたのでは」と勧めますが、
    ...
    #
    # 2026-08-30 05:5x に増えたぶん（`topic_forge --new-family` の 11件のうち 2件。
    # **中身を実際に見て足しています** —— 上の註が言うとおり、
    # 「全部 型1」で流さず、1件ずつ引き算を当て直しました）。
    #
    #   `jutaku-torikoboshi-zandaka-5dan`  鳴ったのは 3件（7,150,000 / 24,290,000 /
    #     32,860,000）。**全部 隣り合う行の差**で、両端はどれも表に在ります:
    #         28,570,000 − 21,420,000 = **7,150,000**
    #         67,140,000 − 42,850,000 = **24,290,000**
    #        100,000,000 − 67,140,000 = **32,860,000**
    #     題そのものが「上へ行くほど1段の伸びが大きくなる」なので、**差が主題**です。
    #
    #   `izoku-matsushi-18sai-gake-4dan`  鳴ったのは `59.3` の1件。
    #     表に **59.31305751258783** が在り、**その丸めた形**です
    #     （`_checks.near_candidates` が正しく名指ししました）。
    "jutaku-torikoboshi-zandaka-5dan",
    "izoku-matsushi-18sai-gake-4dan",
    # 2026-08-30 07:xx に増えたぶん（`--new-family` の残り2件。
    # **中身を実際に見て足しています** —— 上の註のとおり「全部 型1」で流さず、
    # 1件ずつ引き算を当て直しました）。
    #
    #   `gassan-nokoru-waku-12400`  鳴ったのは `2,500` の1件。angle の原文は
    #     「区分ウ（残る枠30,100円…）から区分エ（残る枠27,600円）へ1段だけ動かすと、
    #      枠は **2,500円** しか縮まない」——**書いた側が両端を同じ文に書いています**:
    #         30,100 − 27,600 = **2,500**（両端とも `backing("gassan")` に在る）
    #     表の主題は「1パーセント加算の有無で届く総医療費が出るかどうか」で、
    #     隣り合う区分の差ではありません。**だから表は直さず、ここへ足します。**
    #
    #   `jikangai-hanboki-goukei-uwanori`  鳴ったのは `19` の1件。
    #     差の列 0 / 38 / 57 / 76 / 95 / 114（6行とも表に在る）の**きざみ**で、
    #         57 − 38 = 76 − 57 = 95 − 76 = 114 − 95 = **19**
    #     ＝ 足し算の 99時間 と 上限の 80時間 の差。`ribo-300000-payment-step` と同じ形。
    #     [!] **同じ回に、angle の文のほうも直しています** ——
    #     原文は「0時間・38時間・…・114時間と **19時間ずつ** 開いていく」でしたが、
    #     **最初の1段だけは 38時間**（0 → 38）で、19時間ずつになるのは 2か月目から先です。
    #     **裏取りに通らない数を見にきて、言っている中身のほうの誤りが出ました。**
    #     この検査は「表に無い数」を探す道具ですが、**鳴った文を読むと、
    #     表と合っていない言い回しのほうが見つかることがあります。**
    #   `kakyu-nenkin-shinsei-teikibin-ni-nai`  鳴ったのは `25` の1件
    #     （2026-09-04 20:4x に見た。**09/02 18:10 から 2日 赤のままでした**）。
    #     angle の原文::
    #
    #         外の同じ帯で今年2番目に見られている長尺（**25分**・題は【相手】＋
    #         「申請しないと」＋「定期便に載らない年金」）の形を写す。
    #
    #     **これは `src/calc/kakyu.py` の出力ではありません** —— `style: outside_long` の
    #     題材が angle に書く「**写す先の外の本の寸法**」です
    #     （`data/niche_ceiling.jsonl` の `D9BI69GFWvs`・4,415,973回・
    #      題「【50歳以上必須】申請しないと大損！ねんきん定期便に載らない年金4選！」）。
    #     題の主役の金額 `423,700`（加給年金の特別加算込み）は `backing("kakyu")` に
    #     在ります（＝ `calc:` の貼りまちがいではない。上の「見分け方」で引いた）。
    #
    #     [!] **この形は、これから増えます。** `style: outside_long` の題材は
    #     「外の帯の本の作りを写す」と決まっているので、angle には
    #     **外の本の尺・順位・再生数**が必ず書かれます。どれも calc の表には在りません。
    #     **覆る条件**: 同じ理由で3件目が鳴ったら、1件ずつ足すのをやめて、
    #     `numbers_backed` に渡す前に angle の「外の帯を説明している節」を落とすこと
    #     （そのときは、落とす節の見分け方を先に測ってから書くこと）。
    "gassan-nokoru-waku-12400",
    "jikangai-hanboki-goukei-uwanori",
    "kakyu-nenkin-shinsei-teikibin-ni-nai",
}
