...
def test_順番で決まる回は_順番を名指しする(monkeypatch):
    ...
    #   2026-08-29 に文言が「いまの予約のままでは間に合いません」へ変わったので、
    #   **こちらも一緒に動かすこと** —— 古い字だけを見張ると、
    #   この見張りは**何も見ていない**状態で緑になります。
    assert "いまの予約のままでは間に合いません" not in out
...
def test_単独でも越える群には_はっきり越えると言う(monkeypatch):
    ...
    assert "いまの予約のままでは間に合いません" in out
    assert "期限を 2日 越えます" in out          # 小さい群を積んだ日ではない
    # **「材料を足しても埋まりません」を、**裸で**言わないこと**（2026-08-29 に消した）。
    #   実測で偽でした —— 足りないのは材料ではなく**枠の中身の並べ替え**で、
    #   `request_form` は死に枠を **13本** どければ間に合いました
    #   （`queue_lag.relief()` の docstring に表）。
    #
    # > ### **【2026-09-05 03:1x】この行は 08-29 から 7日 赤のまま置かれていました**
    # >
    # > 同じ日（08-29）の `558eeb25` が、**その字を条件つきの枝へ割り直しました** ——
    # > 死に枠が **0本**（`relief()` が `None` かつ `dead_slots()` が 0本 ＝
    # > **並べ替える対象が無い**）ときだけ言う、という枝です。そのとき
    # > **きょうだいの `tests/test_queue_lag_supply.py` は「意味」を見る形へ広げられ**、
    # > **この file だけが「字」を見たまま残りました**（commit 本文が
    # > 「きょうだいの残した赤」と書いているのと、**向きが逆の同じ事故**です）。
    # >
    # > この検査が守りたいのは「**裸の断定を出さないこと**」で、
    # > 「その字が1度も出ないこと」ではありません。**だから意味で見ます** ——
    # > 出てもよいのは、**理由（並べ替える対象が無い）が同じ行に在るとき**だけ。
    # >
    # > **覆る条件**: `else` の枝が消えて、その字がどこからも出なくなったら、
    # > この2行は `not in` へ戻してよい。
    _bare = "材料を足しても、この床は期限内に埋まりません"
    if _bare in out:
        assert "並べ替える対象が無い" in out, \
            "**裸の断定です。** 『材料を足しても埋まりません』を、理由なしに出しています"
        assert "relief()" in out and "dead_slots()" in out, out
