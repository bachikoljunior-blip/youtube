...
@pytest.mark.parametrize("narration", [
    ...
    "私は10年間、経理を担当してきました。",
    "専門家として断言します。",
    # **2026-08-30 夜に足した8件。** `python -m src.descriptions` を書いた回に
    # 言い換えを10通り当てたら、当たったのは旧 persona の原文 1件だけでした
    # （しかも `元[・]?<職業>` ではなく「実務で回してきた」のほうで）。
    # **`元・<会社>の<職業>` は、解除条件1・2 を閉じたときから素通り**でした。
    ...
    "人事部にいたころの話をします。",
    # docstring の「覆る条件」2 が**例に挙げていた語**（一覧に無いまま閉じていた）
    "元・国税調査官の視点で見ていきます。",
])
...
@pytest.mark.parametrize("narration", [
    ...
    # **網を広げた回に足した7件**（同 2026-08-30 夜）。
    # 広げる側だけを検査すると、次の回が「もっと広く」しか進めません。
    "元の経理処理に戻します。",
    "税務署に提出してください。",
    "年金事務所で確認できます。",
    "ハローワークに申請します。",
