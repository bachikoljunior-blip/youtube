...
def test_どの腕も_伸び幅が桁外れの1件に乗っ取られていない():
    """**`gain` は「当たりだけの中央値」なので、当たりが1件の腕はその1件そのものになる。**

    ## この検査の前身（2026-08-26 → 2026-09-01 に発火して、役目を終えた）

    前身は `test_rpm_が_MIN_N_に達したら_256_を置き直させる` でした。
    `rpm` の閉じた前提が `MIN_N` に達した瞬間に落ちる時限式で、docstring に
    「**いまは無害。無害でなくなる日が来る。その日にこの検査が落ちる**」と
    書いてありました。**2026-09-01 に、予告どおり落ちました** ——
    同じ日に `rpm` の3件目（「WATCH の伸びは複利で、M3 の門である1日30人に届く」）が
    閉じ、`rpm` が「全体で代用」を抜けて自前の中央値に切り替わったからです。
    そのとき `rpm` の当たりは **×256 の1件だけ**で、**RPM の伸び幅が 256倍**に
    なるところでした。直し方も docstring に2つ書いてあり、そのうちの
    「`lever` を `per_video` へ移す」を採りました（`effect_note` が
    「**1本あたり再生**が2桁 動いた」と書いており、`config/hypotheses.yaml` 冒頭の表
    「1本あたり再生を数える → per_video」と字面が一致するため）。

    **時限式は、発火したら消さずに一般化すること。** 前身は `rpm` と 256 を
    名指ししていたので、**同じ欠陥が別の腕で起きても黙ります。**
    2026-08-26 の `test_rpm_の前提が1本あたり再生を数えていない` と合わせて
    **同じ形が2件**出ているので、ここは腕を名指しせずに書き直しました。

    ## 何を見ているか

    `arm()` の `focus = p · log(gain) · θ` に入る `gain` は、
    **`hit`（`effect > 1.0`）だけの中央値**です。当たりが1件しかない腕では
    中央値＝その1件で、**単位のずれた `effect` がそのまま伸び率になります。**
    `factor_at()` の天井は**総倍率**を頭打ちにするだけで、**速さ `rate` は素通り**です。

    ## 覆る条件

    **ある腕で、¥/1000再生 や登録率が本当に 100倍 動いた回**が来たら、この線
    （`gain >= 100`）は誤検知になります。**そのときは線を上げるのではなく、
    `effect` の単位を欄で申告させること**（いまは `effect_note` の散文にしか
    書かれていないので、機械はどの腕でも単位を確かめられません）。
    """
    import yaml
    ...
    # (1) 前身が直した1件が、`rpm` へ戻っていないこと（戻すと 256 が RPM の伸び幅になる）
    back = [r["claim"][:32] for r in rows
            if r["lever"] == "rpm" and r["effect"] >= 100]
    assert not back, (
        f"`lever: rpm` に桁外れの `effect` が戻っています: {back}。"
        "あれは **1本あたり再生**の倍率で、¥/1000再生 ではありません"
        "（`config/hypotheses.yaml` 冒頭の表）。`per_video` が置き場所です。"
    )

    # (2) どの腕でも、**当たりの中央値**が桁外れの1件に乗っ取られていないこと
    loud = {}
    for lever in arm_speed.ARMS:
        hits = [r["effect"] for r in rows if r["lever"] == lever and r["hit"]]
        if not hits:
            continue
        g = sorted(hits)[len(hits) // 2] if len(hits) % 2 else (
            (sorted(hits)[len(hits) // 2 - 1] + sorted(hits)[len(hits) // 2]) / 2)
        if g >= 100:
            loud[lever] = (round(g, 1), len(hits))
    assert not loud, (
        f"腕の伸び幅が桁外れです: {loud}（腕: (中央値, 当たりの件数)）。"
        "**`gain` は当たりだけの中央値**なので、当たりが1件の腕ではその1件が"
        "そのまま `focus = p·log(gain)·θ` に入ります。"
        "`effect` の単位がその腕の数え方（`config/hypotheses.yaml` 冒頭の表）と"
        "合っているか確かめて、合っていなければ `lever` を移すこと。"
    )
