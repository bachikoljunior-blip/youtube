...
def test_天井が足りていれば逆算は出ない(monkeypatch):
    ...
    **2026-08-21 16:2x に標本を上げました**（`views_per_video` 952 → 2,400）。
    この検査の註が自分で言っていたとおり ——「この標本の前提が変わりました
    （検査のほうを直すこと）」—— 変わったのは**天井の作り方**です:

        天井 ＝ 1本あたり再生 × `density_month`
        `density_month` は 25 → **10**（`src/day_cap.py`。08/20 は 25本 公開して
        #11から先の15本が 0〜3再生。出しても再生が付かない）

    952 のままだと `ceiling_short` が **2.24** になり、**天井が足りている側の
