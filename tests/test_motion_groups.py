...
def test_満杯の日へは送らない() -> None:
    """**11本目は配信されません**（`src/day_cap.py`。08/20〜08/22 の3日とも10本ちょうど）。

    そこへ群を揃えるために1本足すと、その本は「動きが無かったから伸びなかった」ではなく
    **「その日の11本目だったから伸びなかった」**ことになります。動かした本を殺す形なので弾く。
    """
    at = {
        "OFF1": "2026-09-24T00:30:00Z",     # 対照だけの日
        "ON1": "2026-09-23T03:00:00Z",      # 動きありだけの日（**ここが満杯**）
    }
    # 09/23 を per_day ぶんまで埋める（A/B 以外の予約も枠を取る）
    for i in range(3):
        at[f"X{i}"] = f"2026-09-23T0{i}:15:00Z"
    assert retime_plan(["OFF1"], ["ON1"], at, bar=1, per_day=4) == []
    # 枠が1つ空いていれば送れる
    assert retime_plan(["OFF1"], ["ON1"], at, bar=1, per_day=5) == [
        ("OFF1", "2026-09-24", "2026-09-23")]
