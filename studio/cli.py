...
def over_ledger(vid: str, live: int, rows: list[dict] | None = None) -> int | None:
    ...
    **覆る条件**: (1) この印が出た次の `measure` が 3周 続けてその数に届かず、
    **かつ そのあいだに台帳が 1度でも動いていれば**、高い側は複製ではなく別の口 ＝
    そのときは `settle_stats` の `reads` ではなく **どの読みが高いか**（時刻・順番）を数えること。
    **「かつ」は 2026-09-14 08:2x に足しました**（optimizer・Opus）——
    09/14 05:2x に立った 1件 は **台帳が印の時点の 807 から 1度も動いていない**まま門を過ぎ、
    **次の周に 819 へ動いて 4回目・166分 で拾われました** ＝ この条件は **偽陽性で引かれていました**。
    `measures_since` は時間で積もるので、**本が平らなあいだ、どの印もいつか必ずここへ落ちます**
    （§5 教訓の形 12つ目）。分けるのは `trend.over_lag` の `ledger_moved`
    （動いていない印は `pending`＝「まだ言えない」へ）。derivation は JOURNAL 09/14 07:5x／08:2x。
...
def channel_switch_line(ch: dict, rows: list[dict]) -> str:
    ...
    （後に呼ぶと、比べる相手が自分になる）。
    **覆る条件**: (1) この行が出た周の判定は立ったサブ（いま 1体）とオーナー（GOAL (4-f-2)）。
...
def cmd_status(a):
    ...
    # 距離は「件/回（成約率）」で、動かすのは **台本と説明欄** ＝ うちが毎日 作っている物です。
    # **固定2 に「できる」と答えられる道のうち、腕が届くのは この 1つ だけ。**
    # 決めと derivation は `trend.perf_need_rate` の註（**API 0単位**・台帳だけ）。
    pl = trend.perf_short(ledger_rows())
    if pl:
        print("  " + pl)
...
def cmd_hear(a):
    ...
    for (e, g), ii in reps:
        ...
        q = hear.near_quiet(rows, e)
        quiets[f"{e}>{g}"] = q
        if q:   # 反証（hear.near_quiet の註）。**名指しは外しません** —— 読む側の材料を並べるだけ
            print(f"      ただし 予定に「{e}」を持ちながら崩れなかったコマが {len(q)}つ"
                  f"（コマ{'・'.join(str(i) for i in q)}）＝ **「同じ語が出るたびに同じだけ崩れる」は、この本では立ちません**"
                  "（TTS 側なら その字は全部 崩れる）。**通すなら、この数を理由としてこの本の節に書くこと**"
                  "（`near_repeats` にも `_WHISPER_ISMS` にも例外を足さないこと・hear.near_quiet の註）")
        elif e:
            print(f"      予定に「{e}」を持つコマは、崩れた {len(ii)}つ で全部です ＝ **反証はありません**（TTS 側の側）")
        else:
            print("      差し込み型（予定の側が空）なので、この反証は測れません（hear.near_quiet の註）")
    if bad:
    ...
    ledger("heard", a.id, mismatched=len(bad), model="medium" if a.medium else "small", mode="kana",
           ...
           near_quiet=quiets,   # 名指しの組ごとに「その字を持ちながら崩れなかったコマ」（hear.near_quiet の覆る条件を本ごとに数えるため）
...
#
# **【2026-09-19 18:xx・optimizer・Opus 5・ultracode・1周 1体】この枠は「ショートが足りない日」の枠です。**
#   **数**: `SHORT_SLOTS` は **6枠**・`day_upload_cap()` も **6本**（日枠 10,000 ÷ 1,600）
#   ＝ **ショートが 6本 在る日は、この 19:00 に座らせると ショート 1本 を追い出します。**
#   追い出したときに失う額（`trend.slot_value` の**門の外**の 円/枠・帯の中段）:
#       short **¥2,771/枠** − long **¥129/枠** ＝ **¥2,642/日**（21.4倍）
#   作る側の分母（周）で見ると **short ¥2,540/周 対 long ¥25/周（102倍）**（`trend.lap_value`）。
#   **長尺そのものを捨てる話ではありません** —— 門の内（YPP の 円・扉(b) の時間）では long が先です。
#   ただし **扉(b) が開くのは期限の 498日 後**（登録 42人 → 1,000人・`trend.rev_deadline`）＝
#   **期限の中では門の内の 3つ の通貨は 1円も現金になりません**（GOAL (4-w)・METHOD §5）。
#   **＝ 順は「ショートで 6枠 を埋める → 埋まらない日だけ 19:00 に長尺」です。**
#   09/19 18:2x の実物はショートの在庫が **5本** しか無く、6本目 が長尺で埋まる所でした
#   （その周が `studio/series_kurisage.py`（繰下げ 66〜70歳・5本）を書いて置きかえた）。
#   **覆る条件**（**門は 1か所**: 判定は `trend.slot_value` の `perf_yen`）:
#    (a) ある長尺 1本 の 24〜96h の再生が **ショートの中央の半分以上**になったら、この段は畳んでよい。
#    (b) `perf_yen` が畳まれたら（オーナーが「YouTubeの収益 ＝ YouTube から振り込まれる金だけ」と読む ＝
#        `trend.PERF_CLICK_BAND` の覆る条件 (3)）、**門の内が正本に戻り、長尺が先になります**。
#    (c) `day_upload_cap()` が 7本 以上になったら（増枠が通ったら）、**追い出しは起きません** ＝
#        19:00 は素直に長尺の枠に戻ります。**この段が効くのは「枠 ＝ ショートの枠数」のときだけ。**
LONG_SLOTS = ("19:00",)
...
#   **その比を訊く口は `trend.same_day_spread` の `last_ratio`**（2026-09-19 19:xx に足した ——
#   それまで この (6) を訊く口はどこにもありませんでした）。
#   **同じ回に出た実測**: 09/18 の 4本 は **出した順** に 07:00 1,165・10:00 1,076・12:00 847・15:00 802
#   ＝ **刻が後ろへ行くほど単調に減っています**（1.45倍）。**「21:00 が薄い」とはまだ言えません**
#   （族も題も同じ日の中で動いている）が、**21:00 を足した日の `last_ratio` は必ず読むこと**。
...
def gone_comments(rows: list[dict], live: list[dict]) -> list[dict]:
    ...
    **覆る条件**（2026-09-11 12:1x・optimizer・Opus に §7 の「次に見る所」(n) から移した ——
    あちらは「何を見るか」と口の名だけを置く側で、条件の正本は関数の註）:
    (1) 消えた件数が **7本 のうちに 3件 を越えたら**、1人の投稿者の話ではなく**チャンネルの側**
    （§7 (m) と一緒に見ること）。
    (2) 消えたのが**返信ずみの物**だったら、**返信の側も消えているか**を見ること
    （＝ 消したのが投稿者なら返信は残り、YouTube 側なら束で消える ＝ 上の「分けられない」が
    1度だけ分けられる形）。
    """
...
#: **2026-09-18 00:3x**: 字の正本は `common.BRAND_NAME` へ移した（本の中の名乗り ＝ 札と出口の声 と同じ 1か所）。
RENAME_TARGET = common.BRAND_NAME
...
#: 実測は **2回**、どちらも同じ本（`2026-09-20-nenkin-tedori-hayamihyou`・182コマ・`viz` 65コマ・
#: 絵 2,001枚・1,169.9秒 の本）で、**1,180秒**（14:43:28→15:03:08）と **1,358秒**（15:03:42→15:26:21）。
#: **採っているのは長いほう**（1,358秒 ＝ 92秒 の **14.8倍**）—— 短いほうは**焼きの途中で CPU を
#: 分け合っていなかった**回で、うちの周はたいてい何かと重なります。**見積もりは長い側へ倒すこと**
#: （外して困るのは「入ると思って入らなかった」側だけで、逆は困りません）。
...
#: (3) `short` の 92秒 は**この周が測り直していません** —— 長尺だけ 2回 測った数です。
BUILD_SECONDS = {"short": 92, "long": 1358}
...
def form_priority_line(ok: list[str], forms: "dict[str, str]") -> str:
    ...
    **数を出さずに「そろっていない」と言うこと**（`form_yield` が倍率を伏せる回と同じ答え）。
    ...
    # **【2026-09-19 18:xx・optimizer・Opus 5・ultracode・1周 1体】「長尺でしか埋まりません」を外した。**
    # **その字は嘘でした** —— 足りない口は **2通り** で埋まります: (a) 長尺を座らせる ／
    # (b) **その周でショートを書く**（`series_*` の生成器 ＝ 1.1周/本）。
    # **同じ行の下半分（`_form_gap_phrase`）が、その 2つ の値段を既に出しています**
    # （short ¥2,540/周 対 long ¥25/周）。**選択肢を 1つ しか名指ししない行の下に、
    # 102倍 の差を印字していた**ので、読む側は (b) を数えずに (a) を選べました
    # （実測: 09/19 17:4x の周が 09/21 19:00 に長尺を決め、18:xx の周が
    #  `studio/series_kurisage.py`（繰下げ 5本）を書いて置きかえた）。
    # **命令には戻しません**（決めるのは立ったサブ・オーナー 09/06 14:0x）——
    # **足りない口の埋め方を 2つ とも名指しするだけ**です。
    # **覆る条件**: 生成器が 1つ も無い題材しか残っていない周（新しい連作を 1周 で書けない）は
    # (b) の 1.1周/本 が効きません ＝ そのときは `trend.lap_value` の `周/本` のほうを疑うこと。
    return (f"    **形の順**: 上げられるのは **1日 {cap}本**・ショートの在庫は **{len(short)}本** "
            f"＝ 足りない **{cap - len(short)}本** の埋め方は **2つ**: "
            f"**(a) 長尺を座らせる** ／ **(b) この周でショートを {cap - len(short)}本 書く**"
            f"（`studio/series_*.py`）。**どちらを選ぶかは下の値段で**（{_form_gap_phrase()}・"
            "`docs/METHOD.md` §5 2026-09-19 02:3x／18:xx）")
