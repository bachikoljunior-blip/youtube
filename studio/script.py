...
#   **(1) は 2026-09-19 14:1x に撃って、「まだ閉じられない」と分かりました。組は 14組 たまり、
#   14組 とも 振れ 0.00%（90.40→90.40・523.90→523.90 …）。この 0 を「焼きは決まった値を返す」と
#   読まないこと** —— `studio/tts.py` は**本文の sha1 で wav を憶えます**（鍵は `[text, voice, rate, yomi]`・
#   `if not wav.exists()`）＝ **本文が同じなら音を焼き直しません。** 14組 は全部その憶えに当たった行で、
#   **合成そのものの振れは 1度も測れていません**（`BUILD_SIG_VERSION` を上げても、この鍵は動きません）。
#   **＝ 計器が死んでいて、死んだ計器は「振れ 0」と同じ字で出ます。`BUILD_JITTER` 3% は未測。下げないこと。**
#   測るには `tts` の憶えを外して同じ本文を 3回 焼くこと（**焼きの時間は増えます**）。JOURNAL 2026-09-19 13:2x。
...
#   4 … **数字の並びを全部 拾えたときだけ当たりにする**（2026-09-20 00:5x・`studio/narration.find_cue`
#         の覆る条件 (a)(b)(c)）。前の版は 2組 以上 の数のうち **1組目 だけ**の当たりを返しており、
#         在庫の台本 56本 で **28歩・10本 が別の数の所で動いて**いました
#         （実物: 図の「13万7602円」が声の「13万**2398**円」の `13` に当たっていた）。
#         **変わるのは、その 10本 と、2組 以上 の数を持つ図の在る本だけ**です。
BUILD_SIG_VERSION = 4
...
# **2026-09-18 22:xx に「コメント」を足しました**（optimizer・Opus 5・ultracode・JOURNAL 同刻）。
# **なぜ**: 20:3x が足した「説明欄」は、**この チャンネルの配りの 93.9% を占める Shorts のフィードでは
# 開かれない面です**（`trend` の `ショートのフィード 93.9%`）—— Shorts は題を叩いて初めて説明欄が出ます。
# 同じリンクを **コメント欄**（件数つきのボタンが画面に出ていて 1タップ）にも置く口を
# `cli cta`（`studio/asp.comment_block`・`commentThreads.insert` **50単位/本** ＝ 1本 出す 1,650 の **1/33**）で
# 作ったので、**書き手が声で指せる先を 2つ にします**。門が守っているのは変わらず
# 「最後のコマに、外へ出る一手が在ること」だけで、どの面を指すかは書き手が決めます。
# **声で「コメント欄」と言う本は、公開後に `cli cta` を撃つこと** —— 言った先に物が無いのは、
# 説明欄に塊が無いのと同じ穴です（`cli cta --check` が 1単位 で数えます）。
# **覆る条件**: (1) コメント欄を指した本の 24h 中位が、説明欄を指した本の半分 未満なら、
# この語を外して説明欄へ戻すこと。(2) `cli cta --check` に「消えた」が立ったら、
# YouTube がリンクを落としています ＝ `studio/asp.py` の覆る条件 (2) と同じ 1か所。
# **2026-09-19 08:xx に「プロフィール」「リンク」を足しました**（optimizer・Fable 5.1・ultracode・JOURNAL 同刻）。
# **なぜ**: Shorts では説明欄・コメント欄の URL が押せません（YouTube 2023-08-31・`studio/asp.py` の
# `PROFILE_NOTE` の註）。押せる面はチャンネルページのリンクだけなので、ショートの出口はそこを指します
# （`asp.CTA_SEG`）。**「説明欄」「コメント」は残します** —— 長尺の説明欄は押せる面なので、長尺の型はそのまま。
CTA_RE = re.compile(r"登録|説明欄|コメント|プロフィール|リンク")


def has_cta(seg) -> bool:
    """最後のコマが、外へ出る一手を持っているか（`say` か `sub`）。登録／説明欄／コメント欄のリンク。"""
