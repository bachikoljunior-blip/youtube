...
def tier_of(text: str, path=None) -> dict | None:
    ...
    **頭の語で錨を打つ理由**（2026-09-17 17:5x・この回に踏んで直した）:
    はじめは「重なった語の数」だけで選んでいて、`【退職金2000万円】…の手取り` が
    **`退職金 税金 いくら` ではなく `年金 手取り いくら`** に当たりました
    （どちらも 1語 の重なりで、中央の大きいほうが勝つ）。
    `q` の頭は 11語 とも題材の名（`年金`／`退職金`／`医療費控除`…）なので、そこを錨にすると割れます。

    **`None` は「段が低い」ではなく「この corpus では測っていない」**です
    —— 頭の語が字のまま無いだけのことが在ります（実測: `医療費が1年で10万円をこえたら` は
    `医療費控除` を字として持たないので `None`）。**低い段と混ぜて読まないこと。**
    """
    best, best_key = None, ()
    for t in by_query(path):
        ...
        if not words or words[0] not in text:
            continue
        key = (sum(1 for w in words[1:] if w in text), t["median"])
