...
def num_to_kana(n: str) -> str:
    ...
    **数字を1つも持たない字は空で返します**（2026-09-14 01:3x・optimizer・Opus。**実物で踏んだ**）——
    whisper は ASCII の `.` や `,` を単独の語で書くことが在り、`_kana_by_janome` の数の buf は
    `[0-9.,]+` で集めるので、**句読点だけの buf** が `int('')` に落ちて
    **`hear` がその場で止まりました**（§4 (2) の出口が丸ごと落ちる ＝ 読みの輪が回らない）。
    **覆る条件**: 数字を持たない buf が**意味のある語**だった回が出たら、ここで捨てずに
    `_kana_by_janome` 側で仮名へ落とすこと（いまは `.` `,` しか来ません）。
    """
    n = n.replace(",", "")
...
_PRE = [
    ...
    (r"([1-9])\s*つ", lambda m: _COUNT_TSU[m.group(1)]),   # 2つ → ふたつ。whisper は「3 つ」と空白を挟む（09/15 05:xx 実測: 「3つの道」が さんつ に落ちて `!!`）
