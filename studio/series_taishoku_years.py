...
def y15():
    ...
    segs = [
        # **フックは `critique` を 4周 回して、いちばん傷の少ない形に戻してあります**
        # （2026-09-19 15:3x〜15:5x・1周 ＝ build＋hear＋read＋critique で約7分）。
        # **[real] の数で数えました**（同じ本文・同じ判じる側・4周 とも別の形）:
        #   1周目  "20年はたらいた人とくらべます" ＋ 枠だけの表       → **[real] 2件**
        #   2周目  同じ宣言 ＋ 表に手取りを足した                     → **[real] 3件**（足した数の導出が無い）
        #   3周目  "20年の人と枠をくらべます" ＋ 枠だけの表 ＝ **いまの形** → **[real] 1件** ←最小
        #   4周目  宣言をやめて "あと5年で枠は200万円ふえます"        → **[real] 5件**（「誰があと5年？」）
        # ＝ **宣言を外すと傷が増えます。** フックの数は、前提（誰の話か）より先には置けません。
        # **残る [real] 1件 は「20年の人の手取りが最後まで出ない」**で、これは 450字 に
        # **2人ぶんの計算が入らない**という、この連作の形そのものの傷です（`critic.not_converging` の側ではなく、
        # 4周 とも立ち方が変わっているので判じる側は動いています）。
        # **覆る条件**: 次に立った側が 5周目 を回して **1番目が コマ1 以外**に立ったら、
        # 傷は形ではなく本文の側 ＝ そこを直すこと。**また コマ1 に立ったら、直す先は本の中ではなく
        # 連作の組み方です**（1本 に 1人 しか入らないなら、くらべるのは**本と本のあいだ** ＝
        # 題・説明欄の年数の表・同じ日に出る 20年 の本の仕事）。**5周目 にフックを触らないこと** ——
        # 上の 4つ は測った数で、触ると 1周目〜4周目 のどれかに戻ります。
        hook_seg(y, t, "20年の人と枠をくらべます。"),
        prem_seg(y, ""), RULE_SEG, frame_seg(y), taxable_seg(t), tax_seg(t), result_seg(t),
        ...
        dict(say=f"20年の手前は1年40万円です。あと5年はたらくと枠は{man(up15)}ふえます。"
             ...
             tag="しくみ", board=["20年の手前 1年40万円", f"15年→20年 ＋{man(up15)}", f"20年→25年 ＋{man(up20)}",
                                "同じ5年でも ちがう"],
             # **表に出すのは 枠 だけです**（`critique` 2周目 で 1周目 の直しを戻した・2026-09-19 15:4x）。
             # 1周目 の [real]「で、結局20年の人はいくらなの？」に、手取りの段を足して答えました。
             # **2周目 は、その足した数そのものを [real] で突きました** ——
             # 「20年と25年の**計算過程が一切ない**まま数字だけ出される」。
             # **どちらも正しい**ので、直す先は表ではなく**宣言**のほうでした:
             # フックは「20年の人と**枠**をくらべます」と言い、この表は**枠だけ**をくらべます ＝
             # **約束した物と出す物が同じ**になり、出す数は 1つ 残らずこの本の中で導けています
             # （枠 200万円 ＝ コマ4 の「1年40万円」× 5年・350万円 ＝ コマ3 の「こえた分 1年70万円」× 5年）。
             # **手取りの側は、同じ日に出る 20年・25年 の本と説明欄の年数の表が持っています**（`YEARS_TABLE`）。
             # 題の「同じ2000万円」は 1周目 の [real] 2件目（くらべる前提が画面に無い）＝ **0字 で返せる側**なので残します。
             viz={"kind": "table", "title": f"同じ{A}万円 年数だけ変える", "head": ["はたらいた年数", "枠", "5年でふえた分"],
