...
def y20():
    ...
    segs = [
        ...
        prem_seg(y, "枠は20年が境目。21年とくらべます。"), RULE_SEG, frame_seg(y), taxable_seg(t), tax_seg(t), result_seg(t),
        # **途中式を声に入れる**（`critique` 1周目 の [real] (4)）—— 枠 +70万 → かかる金額 −35万 → 税金 −10万6500円。
        # **「線」は 3周目 に [real]**（「初めて聞く人には抽象的」）＝ 「境目」へ。
        dict(say=f"20年が境目です。21年目からは枠が70万円ふえます。半分にすると税金のかかる金額が35万円へり、税金は{man(diff)}へります。",
