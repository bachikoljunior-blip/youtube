...
#:      → **画面の表**（速算表の 3段・率と引く額）で渡した。**声には入れていません** ——
#:        1周目 は声に入れましたが、METHOD §34 が同じ所で 3周 回して「95秒 の声には入らない」と
#:        決めていたので、その決めへ戻しました（`tax_seg` の註・3周目 の [nitpick] も同じ向き）
...
RULE_SEG = dict(
    say="決まりでは、退職金には税金のかからない枠があり、年数で決まります。何年分もためたものなので、はみ出た分は半分にします。",
...
def taxable_seg(t: dict) -> dict:
    ...
    return dict(say=f"枠{man(d)}を引くと{man(over)}がはみ出ます。半分の{man(half)}にだけ税金がかかります。",
...
def tax_seg(t: dict) -> dict:
    ...
    # **速算表の引き算は、声ではなく画面に置きます**（`viz` の表・下）。
    # **METHOD §34 の決めをそのまま継ぎます** —— あの連作は `critique` を 3周 回して
    # 「2周目 は省けと言い、3周目 は見せろと言った」＝ **95秒 の声には入らない側**と決めました。
    # この回の `critique` も 2周 とも同じ所に [real] を立てましたが、**同じ答えを声で 2度 試す意味はありません** ——
    # 足したのは**画面の表**（率と引く額の 3段）で、これで「表のとおり」が言葉ではなく**見て確かめられる**形になります。
    # **覆る条件**: 次の回の `critique` が、表を足したこの形でも同じ [real] を立てたら、
    # 足りないのは置き場所ではなく**尺**です ＝ そのときは 450字 の連作ではなく長尺で扱うこと（§34 の同じ結論）。
    head = f"所得税は税率{pct}%の段で、表のとおり{man(t['income_tax'])}。"
    return dict(
        ...
        viz={"kind": "table", "title": "所得税の速算表（この人の段）",
             "head": ["税金のかかる金額", "率", "引く額"],
             "rows": [["195万円まで", "5%", "0円"], ["330万円まで", "10%", "9万7500円"],
                      ["695万円まで", "20%", "42万7500円"]], "widths": [5, 2, 4]})
...
def y20():
    ...
    segs = [
        ...
        hook_seg(y, t, f"あと1年で税金が{man(diff)}へります。"),
        prem_seg(y, "21年の人とくらべます。"), RULE_SEG, frame_seg(y), taxable_seg(t), tax_seg(t), result_seg(t),
        # **途中式を声に入れる**（`critique` 1周目 の [real] (4)）—— 枠 +70万 → かかる金額 −35万 → 税金 −10万6500円。
        # **「線」は 3周目 に [real]**（「初めて聞く人には抽象的」）＝ 「境目」へ。
        dict(say=f"20年が境目です。21年目からは枠が70万円ふえます。税金のかかる金額は35万円へり、税金は{man(diff)}へります。",
