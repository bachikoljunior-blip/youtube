...
#:
#: **【2026-09-19 12:xx】09/20 の枠は 6つ になりました**（`cli.SHORT_SLOTS` が正本・21:00 が増えた・
#: `budget.UPLOAD_UNITS_SHORT` の註）。**この連作は 5本 なので、21:00 が 1枠 空きます。**
#: 何を座らせるかは立った側が決めること（`series_kuriage.ALL` の 1本 を繰り上げる／長尺 3本 の 1本／新しい 1本）。
#: **この一覧に 6つ目 を字で足さないこと** —— 本が 5本 しか無いので、足すと空の刻を持つだけです。
