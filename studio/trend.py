...
def recounts(rows: list[dict]) -> list[dict]:
    ...
    **数え直しは周と周のあいだに起き、同じ周の 3回 読みは 3回 とも新しい値を返します**
    ＝ **`n_values` は必ず 1** で、その道では構造的に見えません（実測: 台帳の `n_values > 1` は
    **8行 とも 伸び中の本 2冊**・48h 超は 0行。一方で数え直しは 5本 に出ている）。
    **数え直しはここで数えること。**
    """
    out: list[dict] = []
    ...
    if rc:
        out.append(
            ...
            + "。**`n_values` では見えません** —— 数え直しは周と周のあいだに起き、同じ周の 3回 読みは"
              "3回 とも新しい値を返す（§7 (h)・`trend.recounts` の註）。")
    else:
        out.append(
