...
**この回の実測（2026-09-10 16:1x・窓 2026-09-05〜09-07）**:
  ...
  ＝ **維持率（%）では 新しい作りは旧作りの下**（42.1〜45.5% 対 47.8〜53.6%）だが、
    ...
    **差は出ており、向きは 2つ です。判定は立ったサブ（いま 1体）（§5）。**
...
**覆る条件**:
  (1) `lag_days()` が **3日 より短い**回が出たら、この註の「3日」を書き直すこと
      ...
      **2026-09-14 01:0x に、逆向き（長いほう）で 1回目 が出ました** —— 00:52 の引きは **遅れ 4日**。
      **API が遅くなったのではなく、こちらが API の日替わりより手前へ歩いた側**です
      （門 20時間 ＝ 引きの刻が 1日 4時間 ずつ前へ歩く。09/10 16:07 → 09/14 00:52）。
      **＝ この註の「3日」は API の遅れで、印字の「遅れ N日」は それ ＋ こちらの歩き**の 2つ が入ります。
      **数える口は `trend.analytics_draws`**（空引き ＝ 新しい日が 1日も入らなかった引き・
      **連も刻も毎周 印字する ＝ ここへ写さない**）。**2回目が出たら門を刻で留めること**（同 覆る条件 (1)）。
