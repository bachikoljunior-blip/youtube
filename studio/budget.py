...
**覆る条件**:
 ...
 (6) **6本目 の 24h 中央が、同じ日の 1〜5本目 の中央の 半分 未満なら**、枠が薄まった側
     ...
     **この比を訊く口は `trend.same_day_spread` の `last_ratio`**（門 `trend.SPREAD_TAIL_HALF` ＝
     「半分」の 1か所。毎周 `trend` が「出した順」と一緒に印字します）。
     **2026-09-19 19:xx まで、この (6) を訊く口はどこにもありませんでした** ——
     `same_day_spread` が再生の小さい順に並べ替えて返しており、**最大が何本目かを言わなかった**
     （＝ 覆る条件が飾りになる形・METHOD §39 の 2.）。
"""
