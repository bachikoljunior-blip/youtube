...
```json
{
  ...
  "prompt": "【定期の回】`docs/trigger_main.md` を読んで、そのとおりに1周してください。\n**最低1件は出してから終わること**（同 §4）。\n\n**あなたはサブエージェントです**（2026-08-25 夜に子セッションから移しました。理由は\n`docs/trigger_parent.md` の第1節 ——`create_session` が人のタップを待つため、\n夜のあいだ鎖が止まっていました）。だから次の3つは**やらないこと**:\n\n    archive_session / set_session_title / relay.py --next\n      → どれもセッションの都合の話で、あなたには当てはまりません。\n        `set_session_title` は実測77分待って拒否され、8/25 だけで1周を3回殺しています。\n\n**あなたは独立した作業コピー（worktree）にいます。** 他が同じファイルを触っても衝突しません。\n**仕事が終わったら、必ず commit して push すること**（親はあなたの中身を判断しません。\n押さなければ、その回の成果はどこにも残りません）。\n\n    git fetch origin claude/youtube-auto-post-revenue-ggedij\n    git merge --no-edit origin/claude/youtube-auto-post-revenue-ggedij\n    git push -u origin claude/youtube-auto-post-revenue-ggedij\n\n**競合したら merge で相手の作業を残すこと。捨てないこと。**\n`docs/JOURNAL.md` は複数が末尾に足すのでよく衝突します。**両方残すこと**\n（`git checkout --ours/--theirs` を日誌に使わないこと。片側を捨てます）。\n\n**検査**: `python -m pytest tests/ -q -p no:randomly --ignore=tests/test_section_sweep.py`\n（`test_section_sweep.py` は240秒でも終わりません）。**いまの赤は3件**\n（`test_forge_floor_fallback` / `test_narrated` / `test_watches`）で前からのもの。**増やさないこと。**\n\n**触らないこと**: 08/27 の `day_cap` の切り分け（その日に19本・うち4本が 05:00〜08:00 JST）。\n**08/27 以外の日の本数を増やさないこと**（対照が壊れます）。答えは 09/01 ごろ。\n"
}
```
