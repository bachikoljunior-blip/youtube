...
| 声が機械的なまま | `GOOGLE_TTS_API_KEY` が無いか、TTS が失敗して open-jtalk に切り替わっています。実行ログの `[tts] google が失敗しました:` の行に理由が出ます（多くは請求先の紐付け漏れか、キーの制限ミス） |
...
> 請求先の紐付けが正しいかどうかは、**動かせば分かります**。TTS が失敗しても
> 投稿は止まらず、自動で open-jtalk に切り替わって最後まで走ります。
> 実行ログに理由が出るので、それを見てから直せば済みます。
