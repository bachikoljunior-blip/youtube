親の周です。

    cd /home/user/youtube && git pull --no-rebase origin <<branch>>
    cat docs/OWNER_INSTRUCTION_GATE.md
    cat docs/OWNER_MODEL_BUDGET.md
    timeout 120 python scripts/next_round_owner.py

## **新しいオーナー指示の取り込みゲート**（2026-09-02）

**毎周、サブを立てる前に上の2ファイルを読むこと。**
新しい指示を、数字や字面だけでそのままコードへ直結させない。原文を残し、
**目標／固定条件／公式仕様／上限・下限／既定値／実験／一回限り／判断委任**のどれかへ分類し、
数値の対象と分母、意図、誤読の危険、別案、成功条件、戻し方を決めてから実装する。

### Fableの50%についての訂正

**Anthropicの公式仕様**は「対象プラン内のFable利用が、通常の全モデル週間上限の50%分まで」です。
画面の「Fable のみ」は、その50%分を0〜100%で表す内訳メーターです。

- **「Fable のみ」は100%まで使ってよい。50%で止めない。**
- **「すべてのモデル」の現在値50%をFable停止条件にもしない。** その中身がFableとは限らない。
- Fableの利用可能性は「Fable のみ」が100%に達したかで見る。
- これはオーナー独自の50%制限ではなく、公式の枠構造。

既存 `quota.sub_model()` は専用目盛り50%で止め、全モデル速度で専用目盛りを外挿するため、そのまま使わない。
`next_round_owner.py` が専用目盛り100%を正しく利用可能性の門へ適用する。

ただし、枠が残っていることはFableを使える条件であって採用理由ではない。模型は仕事ごとの不確実性、失敗損失、将来の動画へ効く範囲、使用量を比べ、**目標到達を早める期待効果／使用量**で選ぶこと。

## **止めないこと。ただし速さの上限は守る**（オーナー 2026-08-31「何で止まってんだよ！」／09-02「二分の一の速度でやって」／09-03「サブで判断して」）

**`next_round_owner.py` の印字どおりに動くこと。** 0体 でも間隔（`quota.py` の「持続できる間隔」）は
守ります —— 09/03 00:04〜06:39 に「0体 なら即 立てる」で 2体 を 17〜30分 ごとに立て、
週の枠が上限の 4倍 で減り（22:01→06:21 JST で すべてのモデル 19→45%）、**このままなら 23:58 JST に尽きて
土曜 07:00 まで 31時間 止まる**線でした。判断は optimizer の回（`scripts/next_round.py decide()` の節）。

    走っているサブが在る       → 何もしない（二重に立てない）
    0体・GO                    → 立てる（下）
    0体・WAIT                  → **立てない。印字された `send_later` を1回 撃って、「（0体・次 HH:MM）」だけ出す**
                                 （起こしは即返り・承認待ちにならない。起きたら間隔が明けている ＝ GO）

**08-31 に叱られたのは「起こしを置かずに 191分 待った」ことです。** 起こしを置けば、空きは間隔ぶんだけ。
**間隔を無視して立てるのは、止まらないためではなく、31時間 止める側です。**

**GO のときは** `python scripts/spawn_prompt.py` で hourly と optimizer を出し、
**一字一句そのまま** Agent へ。**2件 並列**で必ず:

    isolation: "worktree"
    run_in_background: true
    model: <`next_round_owner.py` が GO と一緒に印字する模型>
        ← Fableの利用可能性は「Fable のみ」100%で判定する
        ← 100%は通常の全モデル週間上限の50%分というAnthropic公式仕様
        ← 「Fable のみ」50%でも「すべてのモデル」50%でも切り替えない
        ← この模型表示は利用可能性の暫定ガード。仕事別の目標効果／使用量の判断は別に残す
        ← オーナー指示 2026-09-02
          「Fable5.1のウルトラコードにした。全てこれに置き換えて」
          「やっぱり最大にしたので全てこれでお願いします。」
          「一応だけど、Fableは全体週間使用量の50%まで。opus sonnet haikuで十分なところはそれ使ってもいいよ。ただし目標を目指してね」
          「Fableはそれ以外含めた全体週間使用量の50%だってことを言ってないなら直して。fableのみは100％使っていい」
          「全モデルの50%っていう公式仕様だわ！」

**cwd が repo の中でないと worktree が作れません。**「Stage 2 classifier error」は撃ち直すこと。
立てたら `python scripts/next_round_owner.py --record hourly,optimizer` → `data/` を commit して push。

## **固定の与件**（オーナー原文。**変えない・要約しない**）

> **「言ったこと全部目標と同じ固定指示ね。」**

    1. 目標 —— YouTube の収益で月20万を最短で
    2. > **「動画は1日一本作り置きはなしにして。次の投稿予定までにそこで投稿する動画を
       >    改善し続ける。それは固定にして。その上で目標を目指す」**
    3. > **「前からやってたサブ二台体制でやれ。明日の投稿から一本のみだよ。今日はもう投稿しない」**
       —— 08/31 は0本。**09/01 から 1日1本のみ**
    4. > **「消さなくて良いよ時間かかるならわざわざ」**
       > **「使わなければ良いだけ前提にも再利用もしない」**
       —— 作り置きは **使わない**（予約を外して private・**1本も削除しない**）／
          **前提にしない**（予測の供給から外す）／**再利用しない**

床は `src/house_rule.PUBLISH_PER_DAY = 1`、検査は `tests/test_house_rule.py`。**毎周 確かめること。**

## いま進行中

- **09/01 の1本は `UIWHsypOPPg`（09/01 22:00 JST・予約ずみ）。もう入っています。**
  **入れ直しは要りません** —— 撃つと 09/01 が2本になり、その日に規則1 を破ります。
  この行は 2026-09-01 00:5x まで「`J67vEIw_VRE` を 09/05 から 09/01 22:00 へ入れ直す」
  でした。**`J67vEIw_VRE` は控えの `at` が `None`**（＝池へ戻ずみ）で、
  **同じ題の再投稿が `UIWHsypOPPg`** です。**確かめる1手**（API 0単位）:

      python scripts/pool_drain.py     ← 先頭の「残す」行が、次に公開される1本

- **予約の池化 残 267本 ＝ 13,617単位**（`data/inbox.jsonl`）。日枠待ち。
  **規則1 が最初に破れるのは 09/12**（26日ぶん・238本 多い）。
  **最低2日ぶんの枠が要ります。1回では終わりません** ——
  枠の戻った回は、**投稿を済ませてから残りを全部** `pool_drain.py --apply` に流すこと
- **Data API の日枠は 09/01 16:00 JST に戻ります。** それまで外の口は 403。**無駄撃ちしないこと**
- **サムネイルは `--video <ID>` で1本だけ押すこと**（50単位）。
  裸の `--missing` は **158本 ＝ 7,900単位**で、池化と取り合います
  （2026-09-01 に、一覧が「次に公開される本から順」に並ぶよう直してあるので、
  途中で切れても先頭から載ります。**それでも枠は 7,900単位 使います**）

**この節は、書いた瞬間から古くなります。** 数字と ID を信じる前に、
上の1手（`pool_drain.py`・API 0単位）で実物を見ること。

## 止める仕掛けを足さないこと

> **「勝手にそれで止まるのなし。今後そういうことがないようにして」**

止まるのは `.owner-pause` が在るときだけ／印を作るコードは repo に無い／判定は1か所／
検査は `tests/test_pause_needs_owner.py`。**作りに問題を見つけたら、止めるのではなく直すこと。**

**報告を書かない。承認を求めない。サブの中身は判定しない。**
**オーナーはスマホしか持っていません。** ターミナルのコマンドを頼まないこと。
**手順の正本は `docs/trigger_parent.md` 第1節。毎回 読み直すこと。**