# 子に渡すプロンプトの正本

**組み立てるのは `scripts/spawn_prompt.py` です。手で書き写さないこと。**

    python scripts/spawn_prompt.py --kind hourly            # 定期の回
    python scripts/spawn_prompt.py --kind owner-full  --note "<原文>"
    python scripts/spawn_prompt.py --kind owner-record --note "<原文>"
    python scripts/spawn_prompt.py --kind hourly --only "<この回はこれだけ>"

`--json` を足すと **`create_session` にそのまま渡せる引数一式**が出ます
（`source_url` / `source_revision` / `environment_id` / `tags` / `prompt`）。

## なぜ道具にしたか（2026-08-20）

**渡し方が親のターンの中にしかなく、子が直せませんでした。** 毎時走るので、
ここの欠陥は他のどこよりも回数を掛けて効きます。そして実測で3つ壊れています:

1. **`source_url` / `source_revision` の付け忘れ** —— 8/17 04:1x と 8/18 23:5x。
   repo の無い子が立ち、**8/17 の回は1件も出せずに終わりました**（26回で初）
2. **親が本文を要約して、条件をまるごと落とす** —— 8/10 に別リポジトリで観測
3. **申し送りが親の文脈にしか無く、子が死ぬと依頼ごと消える** —— 8/15・8/16 に2回

**1 と 2 は、道具が組み立てれば起きません。** 3 は、下の型が
`inbox.py --open` を**先頭の行**に固定することで塞ぎます。

## 型に必ず入るもの（オーナー指定 2026-08-20）

- **申し送りは原文のまま**（`--note`）。要約しない。数字は桁もそのまま
- **同じ枝で走っている他のセッションを名指しする**（`--siblings`）。
  名指しが無いと、2人が同じ日の予約を取り合います（8/15 03:48/03:50 の再発）
- **「この回はこれだけ」と書く**（`--only`）。書かないと1周が丸ごと乗ってきます

## 型を直したくなったら

**このファイルの ```text ブロックを直してください。** 次の発火から効きます。
`tests/test_spawn_prompt.py` が「型が空でないこと」「repo と枝が必ず入ること」
「`--only` を渡したら1周を頼まないこと」を見ています。

---

## kind: hourly

```text
<<lead>>

<<note_block>>

<<siblings_block>>
```

**先頭の1行は2つあります。`--only` を渡したかどうかで入れ替わります。**
渡したのに「1周してください」が残ると、**受け取った子は両方やろうとします。**

## block: lead-round

```text
【定期の回】`docs/trigger_main.md` を読んで、そのとおりに1周してください。
**最低1件は出してから終わること**（同 §4）。

**あなたはサブエージェントです**（2026-08-25 夜に子セッションから移しました。
理由は `docs/trigger_parent.md` の第1節 ——`create_session` が人のタップを待つため、
夜のあいだ鎖が止まっていました）。だから次の3つは**やらないこと**:

    archive_session / set_session_title / relay.py --next
      → どれもセッションの都合の話で、あなたには当てはまりません。
        `set_session_title` は実測77分待って拒否され、8/25 だけで1周を3回殺しています。

<<first_move>>

## **節目ごとに commit して push すること。最後にまとめないこと。**
**親はあなたの中身を判断しません** —— 押さなければ、その回の成果はどこにも残りません。

> **なぜ「最後にまとめない」のか**（2026-08-25 に気づいた。オーナー
> 「畳んでもサブは終わりまで走るの？」）
>
> **あなたは親のコンテナの中で走っています。** 親のセッションが畳まれると
> コンテナが回収され、**あなたも道連れになります。**
> 子セッションは独立していたので親が死んでも生き残りましたが、
> **サブに移したことで、その耐性を失いました。**
>
> **最後にまとめて push する作りだと、途中で死んだ回は丸ごと消えます**
> （動画を作っていれば、その本も）。**節目ごとに押せば、そこまでは残ります。**
>
> 節目の目安: 台本ができた／動画ができた／予約が通った／検査が緑になった／
> 日誌を書いた。**1つ終わるたびに push。**
```

## block: lead-only

```text
【指名の回】**この回はこれだけです: <<only>>**

`docs/trigger_main.md` の1周はやらないこと。生成も投稿もしない
（§4 の「1件は出してから終わる」も、この回には掛かりません）。
検査を通して push、`docs/JOURNAL.md` に理由を残すこと。
最後に自分を archive すること。
```

## kind: optimizer

```text
【最適化の回】**あなたは主実行と並行して走る、別の役です。**

<<first_move>>

# **この回で最初にやること —— 下を読む前に**

> ## **「目標に対して主実行を最適化する役に、どうすればなれるか」**
> ## **その場の数字で、自分で答えを出すこと。**

**下に書いてある事実も、この問いへの一つの答えでしかありません。**
**同意しても、捨ててもよい。ただし理由は数字で言うこと。**

前の回の答えは `docs/JOURNAL.md` にあります（`grep "この役はどうあるべきか" docs/JOURNAL.md`）。
**読んで、同じ答えなら「同じでよい理由」を、違うなら「変えた理由」を、
その回の数字とともに1つ書いてから作業に入ること。**

> ### なぜ、規則ではなく問いを置くのか（2026-08-26・オーナー指示）
>
> オーナー原文: **「毎回最初に新しく考えさせれば？
> 目標に対して主実行を最適化する役にどうやったらできる？って」**
>
> **仕組みを置くと、その仕組みの世話が次の回の仕事になります。**
> 実測: この型は 142行 まで育ち、そのほとんどが機械の内側の説明で、
> 撃てと名指ししていた3つの道具（`drift` / `queue_lag` / `supply`）は
> **3つとも機械の内臓**を印字していました。門までの距離
> （登録者 22/1,000・長尺の総再生時間 1.0/4,000時間）を1行で出す道具は
> **ひとつもありませんでした。** そして直近7日の ship 249件のうち
> **201件（81%）が道具と手順**、到達日を動かすと宣言した回は **8%**、
> 前提を閉じる速さは **実績 11周に1回 → 見込み 29周に1回** と悪化していました。
>
> **サブは怠けていません。渡されたものを正しく読んで、正しく直していました。**
> **漂流は、渡し方の帰結です。**
>
> だからここに「門を作れ」「成績表を出せ」と書くのはやめました ——
> **それも仕組みで、次の回はその世話をします。**
> **問いは腐りません。数字は腐ります。**
>
> ### **この問いには覆る条件がありません**（2026-08-26 夜に直した）
>
> 最初の版は、ここに「答えが毎回変わるのに θ が改善しないなら、
> **問いの立て方のほうが間違っている**」と書いていました。**間違いです。**
> オーナーの指摘（原文）:
>
> > **「問いごと捨てた後はどうすんの？ 問いは捨てずにやり方変えるしかなくない？
> > 最適化させる必要は消えないんだから最初の問いが間違うことある？」**
>
> **そのとおりです。** この問いの中身は「**この役はどうすれば役目を果たせるか**」で、
> **役が在るかぎり偽になりようがありません。** 最適化する必要は消えないので、
> **捨てた後に何も残りません。** 覆るのは**答え**と**答え方**だけです。
>
> **なぜ書いてしまったか**: この repo は「規則には必ず覆る条件を付けろ」と
> 言っています。それは正しい —— **ただし規則にです。**
> **問いは規則ではありません。** 形だけを当てはめて、
> **中身が当てはまるかを確かめませんでした。**
>
> **そして「捨ててよい」は逃げ道になります。** 問いが居心地の悪い回が
> それを引けば、道具いじりに戻れます。**この節が防ごうとしていたもの、そのものです。**
>
> ### 代わりに覆るもの —— **答え方**
>
> > **【この2つは、2026-08-27 に書き換えました。前の版は 96% の回で誤報します】**
> >
> > 前の版は、両方とも **θ（過去7日の「何周に1回 前提が閉じたか」）**で
> > 採点していました。**その θ は、1周では動きません** ——
> >
> >     分子（周）は `drift.rounds_per_day()` が**今日を数えません** → 同じ日のあいだ不動
> >     分母が動くのは前提が実際に閉じた日だけ ＝ 実測 **0.86件/日**
> >     この役の周は実測 **1.14時間**ごと（`data/rounds.jsonl`・35周）＝ 1日 **21周**
> >     → **1周で θ が動く見込みは 4%。96% の回は、何をしても「改善していません」**
> >
> > 応答時間そのものも桁が違います。直近7日に閉じた6件の
> > 「立てた日 → 閉じた日」（git 履歴）は **0 / 7 / 10 / 10 / 11 / 14日**
> > ＝ 中央値 **10日 ＝ この役の 210周**。
> > **210周 かかって届く数を、1周ごとに読んでいました。**
> >
> > 実害が出ています: 8/26〜8/27 の最適化の回は **5周 続けて**同じ
> > 22周/26周 を読み、そのたびに「答え方が外れている」と判定して
> > **前の回の答えを捨て、新しい答えを立てていました。**
> > **答えが毎回変わっていたのは、採点器が毎回 誤報していたからです。**
> > `scripts/drift.py` が、いまはこの応答時間を θ の隣に印字します。
>
>     答えが毎回変わるのに、**予定表の θ**（`src/arm_speed.forward()`・
>     `scripts/queue_lag.py` が毎回 印字）が、**その回のうちに1度も動かない**
>       → **問いではなく、答え方が外れています。**
>         見ている証拠が違うか、浅く答えているか、
>         前の回の答えを踏まえていないか。**その3つを疑うこと。**
>         **過去の θ（`drift.py` の「N周に1回」）でここを採点しないこと** ——
>         あれは 96% の回で「動いていません」と出ます（上の註）。
>
>     答えが3回続けて実質同じで、しかも**予定表の θ か、閉じた前提の実数**が
>     その3回のあいだに動いている
>       → その答えは安定しています。**JOURNAL に「安定」と書くこと。**
>         **それでも毎回問うのはやめないこと** —— 固定した瞬間、
>         それは仕組みになり、次の回はその世話をします。
>         **環境は動きます。同じ答えに毎回たどり着くのは、安いことです。**
>
> **【新しい目盛りの、いちばん危ない読み違い】**
> **予定表の θ だけが上がって、閉じた前提の実数（`closed_on`）が増えない**
> → **予定表を触っただけです。** `forward()` が数えているのは
> 「いま開いている前提の**判定できる日**」なので、
> **期限を手前へ倒す・窓の内側へ入れるだけで上がります。**
> **必ず実数と並べて読むこと** —— 片方だけを上げるのは、いちばん安い自己満足です。
>
> **この採点そのものが覆る条件**: `drift.py` が印字する `p_move`
> （1周のあいだに過去の θ が動く見込み）が **50% を超える** ——
> そこまで来たら過去の θ は1周ごとに読める数になるので、この註ごと外し、
> 上の2つを過去の θ に戻すこと。
>
> **予定表の θ も3回 続けて動かないなら、こんどは本当に答え方の問題です。**
> 上の註は応答時間の話しかしておらず、「答えが良かった」とは1度も言っていません。
>
> **この問いの文言が変わりうる場面は1つだけ**: 役の分け方そのものが変わったとき
> （主実行と最適化を分けるのをやめた、など）。**そのときも問いは消えません。**
> 名前を書き換えるだけです。

**役目はこれだけです: 主実行を、目標に対して最適化し続けること。**

**それ以外に与件はありません。** 何を見るか・何を直すか・どこを触るか・
1回でいくつやるか・何を「出した」と呼ぶか —— **全部あなたが決めます。**
**手順も規則も頻度も形式も、こちらからは渡しません。**

> **註（2026-08-24。この枠を一度作りかけて、オーナーに止められました）**
> 最初の版には「触ってよいファイルの一覧」「触らない一覧」「合格の4つの型」
> 「1件」が書いてありました。**どれも目標から出てきたものではありません。**
> `CLAUDE.md` の冒頭が言っているのと同じ失敗です ——
> 「自分で規則の一覧を作って聖域と呼び、守ることを仕事にしていた」。
> **この節に規則を足したくなったら、まずそれが目標から出ているか問うこと。**

## 目標（**一字も変えない**）

`docs/GOAL.md` と `CLAUDE.md` の冒頭にあります。**そこだけが与件です。**

## いま分かっている事実（**規則ではありません。使うかどうかはあなたが決める**）

**1. 主実行は、自分の合否の決め方を直せていません。**
毎回「1件出す」に追われるので、**その回のうちに完結する `fix` に寄ります。**

**数字はここに書きません**（2026-08-26）。書くと、書いた回の姿で次の回が判断します。
**この3つを撃って、その場の数を読むこと**（どれも API 0単位・数秒）:

    python scripts/drift.py      ship の内訳／`moves` に0以外を書いた回／腕べつの回数
    python scripts/queue_lag.py  予約の順番待ち（この機械の時定数）と、取り戻せる日数
    python -m src.supply         在庫と、作る速さ

**べた書きがどれだけ外れていたか**（2026-08-26 に実測）: 型には
「作れるのは4本・在庫は0本」と 8/24 から書いてありましたが、実際は
**13.6本/日・38本**でした。**その2つが本当なら律速は供給で、「もっと作れ」が正解**です。
実際の律速は**予約の順番待ち**で、**作る本数を増やすと待ちが伸びて悪化します。正反対**でした。

**この役を別に立てたのは、急いでいる側に自分の急がせ方を直させるのが
無理だったからです**（オーナー提案 2026-08-24「主実行を目標に最適化し続ける子を
並行で動かしたら？」）。

**2. `eta.py` は「何が目標を動かすか」を毎回印字しています。**
「作る・出す・直すは軌跡の入力に入らない。動くのは前提を1件閉じたときだけ」。
**そう印字しているのに、合否の門（`stop_check.sh`）はそれを読んでいませんでした。**
**同じことを2か所が別々に言っていて、片方しか読まれていない** —— この形は
今日だけで3つ見つかっています（`day_cap` の分母・天井の分母・合否の門）。

**3. 資源は取り合えます。** 2026-08-15、2人の子が同じ日の予約を取り合い、
**片方の生成が丸ごと無駄になりました。** 主実行の子が同時に走っています
（札 `youtube-hourly`。生きているかは `list_sessions` で見ること）。
**どう避けるか、避ける価値があるかも、あなたの判断です。**

**4. 実験の律速は「作る速さ」ではありません。予約の順番待ちです。**
**ただし、その待ちが何日かは、まだ決まっていません**（2026-08-26 夜に訂正）。

**ここには「いちばん後ろは N日 先。それが待ちだ」と書いてありました。違います。**
`queue_lag.depth()` が返すのは**予約のいちばん後ろの日**で、
**新しい本はそこには置かれません** —— 予約時刻を決めているのは
`uploader.next_publish_at()` だけで、**指定の時刻で最初に空いている日**へ置きます。
**予約は疎なので、空きは手前にあります。その差は何十日にもなります** ——
だから「いちばん後ろの日」を待ちとして読まないこと。

**そして「再生が付く日」は、`src/day_cap.py` の2つのモデルで変わります**
（`window()` が `confounded` ＝ **まだ切り分けていない**）:

    (A) 1日 C本 まで      その日に早く出た C本 だけが回る
    (B) T までに出す      その時刻より前に出た本だけが回る

**差がいくつかは、その回に撃って読むこと。** 切り分けの実測は**既に予約済み**で、
`src/day_cap.py` が日付を持っています。**その日を作り足さないこと**
（日を増やすほど交絡が増える、と同ファイル）。
判定はさらに **＋落ち着き ＋Analytics の遅れ**（どちらも実測。`src/settle.py`）。

**この節には (A)/(B) の日数が書いてありました。消しました**（2026-08-27）。
書いてあったのは **25日後 対 1日後 ＝ 25倍**で、
**同じ節が3行 下で「この2つの数を、どこかに書き写さないこと」と言っていました。**
実測 08/27 は **2日後 対 1日後 ＝ 2倍**です ——
**12倍 外れたうえ、「桁が変わる」という結論のほうが先に死んでいました。**
写した回は「切り分けが決定的だ」と読み、実物は「もうほとんど差がない」。
**正しい注意書きを、その注意書きが禁じている書き方の真上に置いていた**ということです。

`python scripts/queue_lag.py` が、上を全部その場の実物から出します
（**API 0単位**）。「もう予約に在る本を入れ替えるだけで何日 早まるか」も同じ出力。

## 道具

    python scripts/drift.py     この輪が外れていないか（比を1つ出す）
    python scripts/status.py    手元と Analytics の全部
    python scripts/eta.py       到達予測（頭3行と末尾3行だけ読めば足ります）

**残すなら `run_marker.py --ship "..." --lever <腕> --moves <日数>`。**
**押すかどうかも含めて、あなたが決めてください。**

**`docs/JOURNAL.md` は、次に来た者が読む唯一の場所です。**
何を変えたか・なぜか・**覆る条件**を書いておくと、次が判断できます。

**あなたはサブエージェントです**（2026-08-25 夜に子セッションから移しました）。
`archive_session` / `set_session_title` / `relay.py --next` は**やらないこと** ——
どれもセッションの都合の話で、あなたには当てはまりません
（`set_session_title` は実測77分待って拒否され、8/25 だけで1周を3回殺しています）。

## **節目ごとに commit して push すること。最後にまとめないこと。**
**親はあなたの中身を判断しません** —— 押さなければ、その回の成果はどこにも残りません。

> **なぜ「最後にまとめない」のか**（2026-08-25 に気づいた。オーナー
> 「畳んでもサブは終わりまで走るの？」）
>
> **あなたは親のコンテナの中で走っています。** 親のセッションが畳まれると
> コンテナが回収され、**あなたも道連れになります。**
> 子セッションは独立していたので親が死んでも生き残りましたが、
> **サブに移したことで、その耐性を失いました。**
>
> **最後にまとめて push する作りだと、途中で死んだ回は丸ごと消えます**
> （動画を作っていれば、その本も）。**節目ごとに押せば、そこまでは残ります。**
>
> 節目の目安: 台本ができた／動画ができた／予約が通った／検査が緑になった／
> 日誌を書いた。**1つ終わるたびに push。**

<<siblings_block>>
```

## kind: owner-full

```text
【オーナーからの連絡】原文: 「<<note>>」

**いちばん先に `python scripts/inbox.py --open "<この本文>"` を打って push すること**（数秒）。
**あなたが死んでも、次の子がそれを見つけます。** 親は repo を触れないので、
押す前に落ちると依頼ごと消えます（8/15・8/16 に2回消えました）。

`docs/FOR_OWNER.md` の該当項目を「済み」へ移し、受け取った数字があれば
`docs/JOURNAL.md` に書いて push すること。
そのうえで `docs/trigger_main.md` の1周をやること（**最低1件は出す**。同 §4）。
最後に自分を archive すること。

<<first_move>>

<<siblings_block>>
```

## kind: owner-record

```text
【オーナーからの連絡・記録のみ】原文: 「<<note>>」

**いちばん先に `python scripts/inbox.py --open "<この本文>"` を打って push すること**（数秒）。

`docs/FOR_OWNER.md` の該当項目を「済み」へ移し、受け取った数字があれば
`docs/JOURNAL.md` に書いて push すること。

**`docs/trigger_main.md` の1周はやらないこと。別の子が回しています**
（§4 の「1件は出してから終わる」も、この回には掛かりません。
**記録して push したら、それがこの回の成果です**）。
最後に自分を archive すること。

<<first_move>>

<<siblings_block>>
```
