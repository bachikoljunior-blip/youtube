...
def print_budget() -> None:
    ...
    **律速は頻度ではなく、1回が長引くこと。** 通常の自動起動は10〜20応答で収まる。
    ...
    if _print_real_usage():
        ...
        print("  [!] **実測がまだ取れていません**（statusLine 経由）。")
        print("      設定は `.claude/settings.json` に入っているが、**起動時にしか読まれない。**")
        print("      自動起動は同じセッションに来るのでプロセスが再起動せず、有効にならない。")
        print("      **コンテナかセッションが再起動すれば効く。** それまで下の換算を使う。")
