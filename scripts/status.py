...
def print_hypotheses() -> None:
    ...
    **Analytics の日次はこの時点で 8/14 までしか届いていません**（3日遅れ）。
    ...
    for h in open_:
        ...
        # **腕の名前**（2026-08-26・最適化の回に足した）。
        #
        # `lever` は 8/20 から `src/arm_speed.py` が読んでいましたが、
        # `hypotheses.yaml` の書式一覧にも、ここにも**一度も出ていませんでした。**
        # 実測 2026-08-26: **開いた15件のうち 10件（67%）が空**で、
        # 閉じた21件だけが埋まっている ＝ **閉じるときに書き足す**運用。
        #
        # 空欄は黙って3つを壊します:
        #   `arm_speed.closed()`  `lever` の無い行を丸ごと飛ばす。**θ にも入らない**ので
        #                         `rate = p·log(g)·θ` で全部の腕の速さが同時に下がる
        #   `arm_speed.planned()` 「これから閉じる前提の配分」が数えられない
        #                         （軌跡は**過去**の配分で未来を解いている。実測 17日 差）
        #   `followup.open_on_lever()` 外れた前提の次の手が、同じ腕の開いた前提を
        #                         名指しできない（(1.8) の門が候補を出せない）
        #
        # **閉じてから付けると `share` は過去の写しにしかなりません** ——
        # どの腕に賭けるかを一度も選んでいないことになります（A14）。
        _lv = h.get("lever")
        if not _lv:
            print("        [!] **動かす腕（`lever`）が書かれていません。**"
                  " 閉じたときに `arm_speed.closed()` が**この行ごと飛ばし**、"
                  "θ（回転の速さ）が下がって**全部の腕の速さが同時に落ちます。**"
                  "\n            `falsified_if` が何を数えているかで選ぶこと ——"
                  " engaged／1本あたり再生＝`per_video`・1日の本数＝`density`・"
                  "登録＝`sub_rate`・露出の面／AdSense 外＝`rpm`・"
                  "どれも動かさない＝`none`")
    ...
    #
    # **5件の外れた前提が16日かけて同じ方向を指している**のに
    # （08/07・08/08・08/15・08/20・08/23。うち4件は「形式そのものを疑え」と名指し）、
    ...
    **`src/calc/` に表を足す以外に道が無い**ことを、ここで名指しします。
    ...
    if n_free == 0:
        ...
        print("  **尽きてから気づくと、その回の `upload` が空振りします。**"
              "**尽きる前のいまが、いちばん安く直せる時です。**")
        # **ここは長らく「先に `src/calc/` へ表を足すこと」の1行だけでした**
        # （2026-08-26 に直した）。**(A) しか名指ししていません。**
        # 0件になった側は 2026-08-17 に (B) を足して直っているのに、
        # **その手前で鳴るこちらは、直された事実を知らないまま残っていました。**
        # 実測（08/26 05:2x・この節が見つかった回）: n_free=3 でここに落ち、
        # 出た指示は「表を足せ」＝ **直近8回で7回いちばんの時間食い（20〜25分）**。
        # 同じ回が (B) を手で引き直したら **`shougai` に3節・12分**で済んでいます。
        # **早く鳴るほうが安い道を隠している**のが、この欠陥の形です。
        _print_deepening(_all)
    else:
...
def _channel_main(days: int = 7) -> int:
    ...
    #
    # `eta.py` の**実効RPMの天井**は、分子が **長尺の面（インプレッション・実測）**
    # で（`src/rpm_mix.surface_ceiling`）、同じ出力が「**長尺を出せば面が増え、
    # 次の回の測り直しでこの天井は上がります**」と書いています。つまり
    # **待ち行列に長尺が何本あるか**が、面の供給そのものです。
    #
    # ところが、それを出す計器がどこにもありませんでした。実測（この回）:
    # **予約 約300本のうち長尺は6本（2%）**、そして `status.py` は
    # 「**この数字に入っていない公開 40本／うち長尺 0本**」と印字していました。
    # 面を増やす手は「長尺を出す」しかないのに、
    # **その供給が 2% のまま40本進んでいた**わけです。
    #
    # 上の `schedule_gaps` は「**ショート**の空き」しか見ません（露出が
    # ショートにしか出ていなかった頃の正しい判断で、註もそう書いてあります）。
    # **その判断は、面が律速に出てきた時点で片肺になりました** ——
    ...
    if gaps:
        ...
        if _r.folded:
            ...
            print("    投稿が途切れるのが最大の損失。")
            print("    長尺が入っていても空きとみなす。露出が出ているのはショートだけだから。")
            print("    背後の生成はコンテナ再起動で消えるので、ログが残っていてもプロセスは死んでいる。")
            # **「生成を撃ち直すこと」だけを出していました**（2026-09-01 夜に直した）。
            #     実測 2026-09-01: 予約は **108本** 在り、うち 106本が 09/24〜10/09 に
            #     1日 7〜13本 で積んであって、**手前の 09/05〜09/23 の 19日が空**でした。
            #     **足りないのは本ではなく、置き場所です。** そこで「作れ」と言うと、
            #     規則（1日1本）の外側にある作り置きの山を、さらに高くします
            #     —— この山を作ったのが、まさにこの行です。
            #     **先に「もう在る本を前へ倒す」を出すこと。** 全体の姿は
            #     `python -m src.next_slot`（**API 0単位**・毎周いちばん最初に出ます）。
            print("    **まず、もう予約に在る本を前へ倒すこと**"
                  "（新しい本は1本も要りません。`python -m src.next_slot` が"
                  "暦の全体を出します・**API 0単位**）:")
            print("      python scripts/reschedule.py --compact          # 割り当てだけ・0単位")
            print("      python scripts/reschedule.py --compact --apply  # 1本 50単位")
            print("    **倒す本がもう無いときだけ、生成を撃ち直すこと。**"
                  "（`--compact` の `--per-day` は `src/house_rule.py` から 1 を読みます）")

    # **待ち行列の長尺の割合**（2026-08-24）。上の `schedule_gaps` の裏返し。
    # **長尺の面は、ここの本数からしか増えません**（`src/queue_mix.py` に理由）。
