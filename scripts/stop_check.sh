...
# --- (1.7) **期限の来た問いを置き去りにしたまま、直し続けさせない**（2026-08-24 に足した） ---
#
...
if [ $? -eq 2 ]; then
  ...
  if [ "$DN" -lt 3 ]; then
    echo $((DN + 1)) > "$DCOUNT"
    cat <<EOF >&2
【期限の来た問いを、置き去りにしています】（${DN}→$((DN + 1))回目・3回で通します）

${DRIFT}

**\`fix\` が悪いのではありません。** 壊れた計器で実験しても答えは出ないので、
直すこと自体は正しい。**ここで止めているのは「期限が来た問いを置き去りにしたまま
直し続けている」ことだけ**です。

**この回は \`verdict\` を出すこと**（\`config/hypotheses.yaml\` の期限が来た1件を、
実データで判定して \`verdict:\` を書く）。**外れてよい。**
外れた前提は、閉じた前提です —— いちばん価値のある判定です。

判定できないなら、**なぜできないかを \`docs/JOURNAL.md\` に書いてから**終わること
（データがまだ薄い・別の前提のほうが速い、は正しい理由になり得ます）。

- 背後で走っているもの: ${RUNNING}
- 未コミットの変更: ${DIRTY}
EOF
    printf '{"decision":"block","reason":"期限の来た前提を判定してから終わってください（config/hypotheses.yaml）"}\n'
    exit 0
  fi
  rm -f "$DCOUNT"
  cat <<EOF >&2
【3回問いました。通します】
**期限の来た問いを置き去りにしたまま終わる回になります。**
