...
親は自分の発火先が自分自身なので、**親からは通ります。**

だから本文の直し方はこうです（**人手は要りません**。親が毎時読む所に置いてある）:

    子   `docs/trigger_body.md` を直し、`--emit-body` で当てるべき姿を出す
    親   `docs/trigger_parent.md`「トリガー本文の正本」に従い、自分で当てる
...
def plan(spec: dict, seen: dict | None, root: Path = ROOT) -> str:
    ...
    if hit & set(PARENT_ONLY) or seen is None:
        out += ["## 本文（**子は撃てません。親だけ**）", "",
                ...
                "親がやること: 次の発火で `docs/trigger_parent.md`「トリガー本文の正本」",
                "              のとおり、自分で `update_trigger(prompt=…)` を当てる。",
                "",
                "**人手は要りません。** 親は自分の発火先が自分自身なので通ります。",
