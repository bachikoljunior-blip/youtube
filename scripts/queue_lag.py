...
def lag_lines(rows: list[dict], now: datetime | None = None) -> list[str]:
    ...
    **新しい本はいちばん後ろには置かれません**（`placement_days()` の註）。
    ...
    out: list[str] = []
    # **規則5（固定その4・2026-09-02）の下では、この節ぜんぶの向きが変わります。**
    #     ここは「予約の列に何本 並んでいるか／帯の空きへ何本 置けるか」を数えます ——
    #     つまり **先の日付に本が在ることを前提**にした道具です。
    #     オーナー原文「**現在の日付にしか予約しないってことだからね？**」の下では、
    #     **列の長さは 0 が正しく、帯の空きは埋める対象ではありません。**
    #     数え方は残します（過去のログと並べられる・規則が外れたら戻る）が、
    #     **読む向きを取り違えないよう、いちばん上で1回 言っておきます。**
    try:
        from src import house_rule as _hr                      # noqa: PLC0415
        if _hr.same_day_only():
            out += [
                "[!] **この節の数は、規則5（固定その4）の下では『埋める対象』では"
                "ありません** —— オーナー原文「**現在の日付にしか予約しない"
                "ってことだからね？**」。",
                "    **先の日付が空であることが正しい状態**なので、"
                "『列が短い』『帯が空いている』は欠陥ではありません。"
                "**先の日付に並んでいる本のほうを外します**"
                "（`python scripts/pool_drain.py --apply --keep 0`）。",
                f"    判定に要る本数も、暦に置いて稼ぐのではなく "
                f"**毎日 {_hr.planned_publishes_per_day()}本 出た結果として**増えます ——"
                "『いまの暦なら何日』という数え方は、この規則の下では意味を持ちません。",
                "",
            ]
    except Exception:                                          # noqa: BLE001
        pass
...
def relief(batch_build, need: int, due: date, cand: list[dict],
    ...
    **「材料を足しても、この床は期限内に埋まりません」は偽**でした。
    ...
    ## 値段（**この節を疑う回が、まずここを見るように**）

    二分探索なので `live_plan()` を **log2(候補数)+2 回** 歩きます。
    実測 2026-08-29（候補 145本・要 95本）: **1.0秒**。
    同じ回の `scripts/queue_lag.py` 全体は **61秒** なので、**1.6%** です ——
    **ここが遅いと思ったら、まず `answering()`（0.85秒）ではなく
    その外を測ること。**
        ...
        if walk is not None:
            ...
            if s_last > due:
                ...
                if rel is not None:
                    ...
                    # **見るのは旗ではなく、実物の暦です**（2026-09-05 03:1x に直した）。
                    #
                    # ここは長らく `_hr.same_day_only() and not _rows` でした。
                    # **オーナーが 09/04 17:3x に床を外した**（原文「目標以外全部外して良いよ」）
                    # 瞬間に `same_day_only()` が False になり、**この枝が丸ごと死んで**、
                    # 下の `else` の「**材料を足しても、この床は期限内に埋まりません**」が
                    # 復活しました（`tests/test_queue_lag_band_per_key.py` が赤で残っていた
                    # のがそれです）。**その断定は 2026-08-29 に実測で偽と分かって消した字**です。
                    #
                    # **この枝が正しいのは、旗のせいではありません** ——
                    # **予約が1本も無い**という**実物**のせいです。並べ替える対象が
                    # 0本 なのは同じでも、`cand` が空（＝ 帯は在るが死に枠が無い）と
                    # **帯そのものが空**は別の話で、後者は「材料を足しても埋まらない」
                    # ではなく「**日々の公開でしか埋まらない**」です。
                    # 旗はその**理由**を言うだけなので、下の文面の中で名指しします。
                    #
                    # **覆る条件**: 先の日付に予約が入るようになったら、この枝は
                    # 自分で通らなくなります（`_rows` が非空 ＝ `cand` の話に戻る）。
                    #
                    ...
                    per_day = _hr.cap()
                    _why_empty = ("規則5（固定その4「**現在の日付にしか予約しない**」）の下では"
                                  "**これが正しい状態**です"
                                  if _hr.same_day_only() else
                                  "規則5 は 2026-09-04 17:3x に**床から既定値へ**降りています"
                                  "（`house_rule.OWNER_FLOORS_LIFTED`）が、**実物の暦はいま空**です")
                    out.append(f"{bar}      **先の日付に予約が 1本もありません** ——"
                               f" {_why_empty}。"
                               f" この床は**並べ替えでは埋まりません** —— 1日 {per_day}本 の"
                               f"公開が {n_key}本 積むまで、およそ **{n_key // max(per_day, 1)}日**"
                               "（`src/house_rule.py`）。"
                               " **「材料を足しても埋まりません」ではありません** ——"
                               "足りないのは材料でも並べ替えでもなく、**日数**です。"
                               + ("" if not _hr.same_day_only() else
                                  " **`--compact` で先へ並べ直さないこと**（規則5 が禁じています）"))
                else:
                    # **どける先が1本も無い枝**（2026-08-29 14:2x に足した）。
                    # 24ebde3 が「材料を足しても埋まりません」を消したのは、
                    # **死に枠が在るのに『順番では消えない』と言っていた**からです。
                    # `cand` が空なら、その反例そのものが在りません ——
                    # **並べ替える対象が無いので、超過は枠の側で確定**します。
                    # ここで何も言わないと、上の `[!] 間に合いません` が
                    # **理由の無い断定**として残ります（`tests/test_queue_lag_supply.py`
                    # の `test_帯の最後の1本が期限を越えたら言う` が、そこを見ています）。
                    out.append(f"{bar}      **{due} までの帯に、どけられる死に枠が"
                               "1本もありません** —— **材料を足しても、この床は"
                               "期限内に埋まりません**（`relief()` が `None` かつ"
                               " `dead_slots()` が 0本 ＝ **並べ替える対象が無い**）。"
                               " **これは「この実験は無理だ」ではありません** ——"
                               "`falsified_if` は緩めず、動かすのは期限だけ"
                               "（`python scripts/deadline_check.py --extend`）")
...
def quota_lines(plan: Plan) -> tuple[list[str], bool]:
    ...
    **この窓で1回でも観測していたら、枠は本当に尽きています。**
    ...
    if ok:
        ...
        # **「手は消えません」を、「待っても損しません」と読ませないこと**
        # （2026-08-29 15:5x に足した。**この1行が、待つ側の背中を押していました**）。
        #   消えないのは**手**であって、**日付ではありません。**
        #   実測（`data/queue_lag.jsonl` の `before`）:
        #     08/27 07:11  opening_motion の判定 **09-29**
        #     08/27 07:47  同 **10-06**  ← **36分で +7日**（きょうだいが本を足した）
        #     08/29 15:2x  同 **10-07**
        #   **倒せる幅（30日）は変わりませんが、倒した先の絶対日は毎日 後ろへ動きます。**
        #   だから「次の窓で撃てばいい」は、**1窓ぶんタダではありません。**
        lines.append("      [!] **ただし『手は消えない』は『待っても損しない』では"
                     "ありません。** 消えないのは**手**で、**日付は動きます** ——"
                     " 実測（`data/queue_lag.jsonl` の `before`）: `opening_motion` の"
                     " 判定は 08/27 07:11 に **09-29**、その **36分後に 10-06**"
                     "（きょうだいが本を足した）、08/29 に **10-07**。"
                     " **倒せる幅は同じでも、倒した先が毎日 後ろへ下がります。**"
                     " **見送った窓のぶんは、戻ってきません。**")
    return lines, ok
...
def apply_moves(plan: Plan) -> int:
    ...
    **覆る条件**: `--plan` が失敗する本を組まなくなったら（当て先の実在を
    ...
    2026-08-28 に、ここだけ「1手 50単位」と書き換えて食い違わせました。）
...
def stuck_lines(plan: "Plan") -> tuple[list[str], bool]:
    ...
    **覆る条件**: 印字と実物が合うように直したら、この門は要りません
    ...
    return ([
        ...
        "実物が食い違っています。",
        "  **どちらが悪いかは、この行だけでは決まりません**（2026-08-28 に"
        "直した ―― ここは長らく **『直すのは印字の側です』**と言い切って"
        "いました）。実測で、少なくとも2つあります:",
        "    (1) **印字が広すぎる** ―― 組んだ日数に実際は届かない",
        "    (2) **手が当たっていない** ―― `apply_moves` は最初の失敗で"
        "止まるので、**幻の予約が1本あるだけで 0/16 になります**"
        "（実測 08/28: `--plan` の1手目が、控えでは 10/04 の予約・"
        "実物は 08-28 11:00 に公開済み）",
        "  **見分け方**: 上の `前の回: N手` は、2026-08-28 から"
        "**実際に当たった数**です（それ以前の行は*組んだ数*なので信用しない）。"
        " `skipped_public` に本が並んでいれば (2)。"
        "**0手 ならこの門はそもそも開きます。**",
