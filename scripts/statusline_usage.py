...
## **この経路は、いまの運用では効かない（2026-08-08 に確認）**

statusLine は**対話 UI の機能**で、**ヘッドレス（自動起動・API 経由）では
呼ばれない。** 別セッションを2つ立てて確かめた:

1. 1つ目 … リポジトリを渡し忘れて設定が読まれず失敗
2. 2つ目 … リポジトリを渡して正常起動。それでも
   「no new data in headless session」で `rate_limits` は来なかった

この常駐セッションも remote（ヘッドレス）なので、**再起動しても取れない。**
「起動時にしか読まれないから再起動すれば効く」と一度書いたが、**それも誤り**だった。

**このファイルは残す。** 人が対話で開いたときには効くし、害も無い。
ただし**当てにしないこと。** 使用量は `scripts/usage.py` の換算で見る。
"""
