...
def clarity_books() -> int:
    ...
    **1日1本 の規則で律速されます**（`_counts_videos()` がそう読みます）。

    **立てた回は逆を書いていました** ——「取る先はもう公開ずみの本の山なので
    律速されない」。**その日のうちに外れています**: `scripts/retention.py` を
    1度 撃つと 69 → 112本、**2度目は +1本**。井戸はそこで尽きました
    （`retention.videos()` が読む `data/scan.jsonl` の本を引ききった）。
    **ここから増えるのは、公開した本のぶんだけ**です。
    """
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
...
#: **取り直すと「凍結した下敷き」が動く計器。**
#:
#: `_METER_REFRESH` は「取り直す手」を名指しします。**そこまでは正しい。**
#: ところが `data/views.jsonl` は `scripts/zero_start.py` の §1 が
#: **凍結している前提で読んでいる下敷き**でもあります
#: （`tests/test_zero_start.py::test_実物の下敷き_旧データは凍結なので数は動かない`）。
#: ＝ **赤い門の言うとおりに撃つと、別の門が赤くなります。**
#:
#: **実測**（`scripts/snapshot.py` の註・2026-09-13 17:0x）: 行が 526行 入った回に
#: B群の最大が **275 → 382回** へ動き、初点の並びも変わって**その検査が赤**になりました。
#:
#: **なぜここに置くか**: 踏む相手は「`views.jsonl` が古い」と言われた回の次の一手です。
#: **註を読みに行く前に、コマンドを撃ちます**（`_BUILD_LEDGERS` を足したときと同じ理由 ——
#: 「撃てない手を名指しすると嘘になる」の、**もう半分**: **撃つと別の所が壊れる手**）。
#: **止めません**（取り直すべき回は実際に在ります）——**一言 添えるだけ**です。
#:
#: **覆る条件**: (1) `zero_start` が下敷きを凍結で読むのをやめたら、この註ごと畳むこと。
#: (2) 逆に「撃ったのに何も壊れなかった」回が 2度 出たら、実測のほうが古い ＝ 数を取り直すこと。
_FROZEN_UNDERLAYS: dict[str, str] = {
    "data/views.jsonl": (
        "  **ただしこれは『凍結した下敷き』でもあります** —— 取り直すと "
        "`scripts/zero_start.py` の §1 が黙って動き、"
        "`tests/test_zero_start.py::test_実物の下敷き_旧データは凍結なので数は動かない` が"
        "**赤**になります（実測: B群の最大が 275 → 382回）。"
        "**反射で撃たないこと。撃つなら下敷きを引き直す手と一緒に。**"
    ),
}
...
def _counts_videos(expr: str) -> bool:
    ...
    **見落としは「押さえない」側に倒れる**ので、日付が手前に出たら疑うこと。
    ...
    if "latest_views" in expr:
        return False
    # **`clarity_books()` は名前に `video_id` を持ちませんが、数えているのは本です**
    #   （2026-09-02 に、立てたその日に踏んだ）。
    #   あれは「維持率カーブと控えが**両方ある**本」の数で、立てた回は
    #   「もう公開ずみの山から引くので 1日1本 では律速されない」と書きました。
    #   **その日のうちに外れました** —— `scripts/retention.py` を1度 撃つと
    #   **69 → 112本**、2度目は **+1本**。**井戸はその回で尽きています**
    #   （`retention.videos()` が読む `data/scan.jsonl` の本を引ききった）。
    #   ところが伸び率は `have / (今日 - since)` なので **4.7本/日** を名乗り、
    #   `deadline_check` は「あと5日」＝ **09-07 に判定できる**と出しました。
    #   実際にそこから増えるのは**公開した本のぶんだけ**です。
    #   **押さえないと、`--shrink` が期限を毎周 手前へ動かし続けます**
    #   （`test_遅すぎる期限が残っていないこと` の「覆る条件」が言う churn そのもの）。
    #   **覆る条件**: 走査（`data/scan.jsonl`）が公開より速く増える作りになったら、
    #   ここから外すこと。そのときは井戸がまた湧いています。
    if "clarity_books(" in expr:
        return True
    return "video_id" in expr or "_arm(" in expr


#: **暦の日を数えている式か**（`_counts_videos` の隣。2026-09-04 22:4x に足した）。
#:
#: ## なぜ要るか（**実物で踏んだ**）
#:
#: `_counts_videos()` の押さえは **本を数える式にしか掛かりません**。
#: ところが台帳には「**帯に入った日が N日**」のように、本ではなく
#: **暦の日**を数える要件が在ります。**日は、1日に1つより速くは増えません** ——
#: 本より厳しい上限なのに、押さえが1つも掛かっていませんでした。
#:
#: 実測 2026-09-04 22:3x（前提「08-25 を境に落ちた1本あたり再生は…」）::
#:
#:     count_expr  `rule_per_video.staleness()['sample_days']`（＝ 帯に入った**日**の数）
#:     needs.what  「**規則の下では 1日1点**しか増えません」  ← 台帳が自分でそう書いている
#:     deadline_check  要 19 ／ いま 12（2日で **6.00/日**）→ **あと 2日**（09-06）
#:     earliest_pv_sample（`tests/test_hypothesis_deadline_reachable.py`）
#:                     1日1点 で数えて **09-11**
#:
#: **5日 ちがう日を、2つの道具が同時に印字していました**（`_counts_videos` の
#: docstring が 08-31 に書いた「25日 ちがう日」と、**同じ形の3つ目**です）。
#: 6.00/日 は「貯めを一度に引いた回が `since` の窓に入っている」ぶんで、
#: **これから先、暦の日が 1日に 6つ 進むことはありません。**
#:
#: **放っておくと何が起きるか。** `test_遅すぎる期限が残っていないこと` は
#: 「データは揃うのに期限が先」を赤にするので、この偽の `ready`（09-06）に合わせて
#: **期限が手前へ縮められます**。すると標本が 19日 に届かないまま判定に入り、
#: 「上回っていない＝外れ」で前提が倒れ、`src/arm_speed.forward()` が
#: `density` を「当たらなかった腕」として数えます —— **その検査自身の docstring
#: （75〜78行）が「検査が、台帳の禁じている行為を要求していた」と呼んでいる形**です。
#: そして 2026-09-04 09:0x の回は、この食い違いを
#: 「両方を満たす日はありません」と台帳の註に書いて**日付のほうを諦めていました**
#: （`config/hypotheses.yaml`）。**直すのは日付ではなく、この数え方のほうです。**
#:
#: ## 何を「日」と読むか
#:
#: **式だけを見ます**（地の文は読みません。`_counts_videos` と同じ約束）。
#:
#: ## 覆る条件
#:
#: - 1日に2つ以上の「日」が増える数え方（例: 過去へ遡って埋め直す走査）が出てきたら、
#:   この押さえは外すこと。**そのときは井戸がまた湧いています**
#:   （`clarity_books()` の註と同じ性質）。
#: - `PUBLISH_PER_DAY` とは無関係です —— 規則が 1日5本 になっても、
#:   **暦の日は 1日1つ**しか進みません。**本数をここに書き写さないこと。**
_DAY_COUNT_NAMES = ("sample_days", "_days']", '_days"]', "band_days", "days_in_band")


def _counts_days(expr: str) -> bool:
    """その `count_expr` が数えているのが **暦の日** か（`_DAY_COUNT_NAMES` の註）。"""
    return any(name in expr for name in _DAY_COUNT_NAMES)
...
def _stale_todo(need: dict) -> str:
    ...
    **覆る条件**: 1周ごとに全計器を取り直す作りにしたら、この門は毎回 黙るので
    ...
    return ("**待ち方が違います。足りないのは日ではなく、計器のほうです** —— "
            ...
            + _FROZEN_UNDERLAYS.get(src, "")   # **撃つと別の門が赤くなる計器**（`_FROZEN_UNDERLAYS` の註）
            + tail)
...
def _ans_accrual(need: dict, as_of: date) -> Answer:
    ...
    if _counts_videos(expr):
        ...
        if rate > cap:
            ...
            #   `clarity_books()` が入って、それが**その式には嘘**になりました ——
            #   あちらが速く見えるのは作り置きではなく、
            #   **貯めを一気に引いた回が `since` の窓に入っている**からです。
            #   **押さえる理由（公開は1日1本）は同じでも、速く見える理由は別**です。
            #   理由を1つに決め打つと、この repo でいちばん多い壊れ方
            #   （言っている所と、している所が別）をこの行が作ります。
            why = ("その速さは**作った本**の平均で、規則2 でその本は公開されません"
                   "——公開されない本は群の標本になりません"
                   if _BUILD_LEDGER in expr.replace('"', "'") else
            ...
            cap_note = (f"。**伸び率を規則（1日{house_rule.PUBLISH_PER_DAY}本）で"
                        ...
                        + cal_note)
            rate = cap
    elif _counts_days(expr) and rate > 1.0:
        # **暦の日は、1日に1つより速くは増えません**（`_counts_days` の註に実測）。
        # `_counts_videos` の押さえは本にしか掛からず、**日を数える式は素通り**でした。
        cap_note = (f"。**伸び率を暦で押さえています**（実測の平均は {rate:.2f}/日 ですが、"
                    f"この式が数えているのは**暦の日**で、**1日に1つより速くは増えません** ——"
                    f"速く見えるのは、貯めを一度に引いた回が `since` の窓に入っているぶんです）")
        rate = 1.0
...
def _ans_group_key(need: dict, as_of: date) -> Answer:
    ...
    **新しい A/B を足すときも、足す先はあちらです。**
    ...
    if ready is None:
        ...
        # **凍った群を名指しする**（2026-09-04 23:5x に足した）。
        #
        ...
        #
        # **日そのものは動かしていません。** `Answer(None, …)` を返すと、
        # `check()` が `_ready_at_rule`（足りぶん ÷ 規則の密度）へ落ちて**別の日**を出し、
        # `test_遅すぎる期限が残っていないこと` が「期限を4日 縮めろ」で赤くなります
        # （**その赤は検査の誤りではなく、守れない前提が実際に在るということ**。
        #   決め方の3択は `docs/JOURNAL.md` 2026-09-05 00:1x）。
        # **決めるまでは、日を書き換えずに理由だけ並べます** ——
        # 読む側が「上の行が実物・下の日は式の産物」と分かる形が、いちばん安い。
        #
        # **覆る条件**: 3択のどれかを決めた回が、ここを `Answer(None, …)` に変えて
        # `Floor.ready_at_rule` にも同じ門を入れること（`starved_arm_frozen`）。
        frozen_notes = []
        for g in sorted(floor.groups):
            if floor.nth[g] is not None:
                continue
            miss = n - len(floor.groups[g])
            note = SJ.starved_arm_reason(key, g, miss)
            if note:
                head = ("[!] **凍っています**" if SJ.starved_arm_frozen(key, g, miss)
                        else "[!] **在庫を回せば満ちます**")
                frozen_notes.append(f" → {head} {note}")
        body = body + "".join(frozen_notes)
