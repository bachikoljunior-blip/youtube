...
FILED_IDS: dict[str, str] = {
    "751f4947": ("2026-09-16 説明のパートごとにアニメーションとか画像でイメージしやすくしたらいいと思う"
                 ...
                 " ＝ **残る 9本 は 1枚 のまま**（`lint` が `[?]` で毎回 名指しする）"),
    "be77dee9": ("2026-09-16 40年を480か月に言い換えてるのね。分かりづらかった。…あと決まりでは、と言うのが連続してくると違和感ある"
                 " → 札: **2つ に割れます**。(1) 言い換えの分かりにくさ ＝ `docs/METHOD.md` §3 の"
                 " 分かりやすさの側（§4 (1) の輪が毎本 当てる）。(2) 「決まりでは、」の連 ＝ §3 の **7-b**"
                 "（09/16 14:3x に「毎コマ」→「本に 1度」へ変えた・門は `script.TAG_OPENERS` と"
                 " `script.OPENER_NEAR`（窓 2コマ）・直す口は `script.drop_repeated_openers()`）"
                 " ＝ **画面の札（`slides.TAG_COLORS`）が毎コマ 持つので、声は 1度 で対応が渡ります**"),
    ...
    "d5c7b271": ("2026-09-14 あとできないというな。そう思うならやり方が間違ってることを疑えよ"
                 ...
                 " ＝ 手渡しの話ではなく「その環境で立てろ」"),
    "8e695b8e": ("2026-09-14 適宜、期限内にできるか考え、できないと判断したならやり方が間違ってることを疑え"
                 " → 札: `docs/GOAL.md` の達成期限の節 (4-g)（09/14 13:4x・`hourly` の判定）"
                 " ＝ **疑う先は 尺（門の入り方: ショートは 10,000,000回 の扉だけ・4,000時間 の扉は 1/167）** → 長尺の枝を開けた"
                 "（ショートの枠は動かさない・型は `optimizer` が `studio/` に足す・1本目 は次に台本を書く回）"),
    ...
    "3aa5cecf": ("2026-09-14 最適化が立った時その役は期限内に達成できるか考え、"
                 "できないや分からないと判断したならやり方が間違ってることを疑え"
                 " → 札: `docs/METHOD.md` §7「いまの数」の **立った時の判断** の行"
                 "（09/14 21:0x・optimizer が 1度目を下した ＝ **できる**）・"
                 "口は `trend.rev_deadline_line` と `trend.sub_rate_line`（毎周 印字）"
                 " ＝ **疑った先は形ではなく、その数を作っていた口**"
                 "（`rev_deadline` が扉(a) だけを数えていた ＝ JOURNAL 09/14 21:0x〜21:5x）"),
    "b478751a": ("2026-09-14 最適化立った時その役はまず期限内に目標達成できるか考え、"
                 "できる以外の判断をしたならやり方が間違ってることを疑え"
                 " → 札: 同上（`3aa5cecf` の言い直し ＝ **2度 言われました**）"),
    "176da533": ("2026-09-14 サブ立てるのは今最適化の役やってるやつの分だけにして。"
                 "そいつはFable5.1のultracodeにして。目標と、（期限内に達成できるか考え・"
                 "できる以外なら やり方を疑え）だけ固定で、他の実行は全て自由にさせて"
                 " → 札: 親の側は `hourly` が 09/14 20:4x に入れた"
                 "（`next_round.ROLES`・`quota.ROLE_TIER`・`docs/spawn_prompt.md`・METHOD §5 の「1体」の段）。"
    ...
    "30c69fa9": ("2026-09-14 やってねえのかよ早くやれ"
                 " → 札: 同上（`67d63788` の催促 ＝ 口を確かめる番だった）"),
    "6df66dd7": ("2026-09-14 3分ごとに停止を確認していたら原因を全て潰してから再実行するようにして"
                 "出せるまでやって／停止を確認したら、ね"
                 " → 札: **`studio/stall.py`**（09/14 22:0x・optimizer-sub・Opus が新設）。"
                 "**停止の定義は数で決めた**: 口が開いた印（`channel`）が空いた窓 —— "
                 "分母 142窓 で 時間 2.0h（p90 0.93h の 2.15倍）か 周 2周 に当たるのは **3窓（2.1%）**で、"
                 "その 3つ が知っている停止の全部（09/11・09/13 の盲の周と、09/14 17:16〜20:07 の 2.85時間）。"
                 "ほかに `token_rejected`（実測 1行）・盲の周（127周 中 2周）・きょうの枠が空のまま 10:00 を過ぎた（8日 とも 0件）。"
                 "口は `cli.main()`（**`fn(a)` より前** ＝ 口が閉じた周でも読める）と"
                 "`next_round.decide()`（停止した周は床 91分 を待たず **3分**・`live == 0` のときだけ・**下げる側にしか動かない**）。"
                 "輪の出口は `stall.shippable()`（口が開いていて、次の公開の枠に本が在る ＝ 「出せる」）"
                 " ＝ **3分 を使うのは機械が潰せる原因が在る周だけ**（口だけの周は床のまま・訊きを置く）。"
                 "検査 `tests/test_next_round_stall_retry.py`（17件・陽性対照 3つ）・derivation は JOURNAL 09/14 22:xx"),
}
