...
**オーナーの実測にも幅がある。** 期間Aは本人が「1〜2%」と幅で言っており、
期間Bの「0.2%」も表示が1桁なら実際は 0.15〜0.25% かもしれない。
両方の幅を通すと **1% は 11.2M〜18.7M**。上下で1.7倍ある。

だから**この数字で運用の判断を左右させてはいけない。**
実際 2026-08-06 に、較正のずれだけで日次へ降格し、同日中に戻した。
**往復そのものが無駄で、しかもその間ずっと判断が鈍る。**

**判断は幅の外でだけ動かすこと。** `status.py` は安全側と楽観側の両方で
判定し、**結論が一致したときだけ**頻度を落とせと言う。食い違うなら、
それは「較正の幅に埋もれている＝動かす根拠が無い」ということ。

**本当に効く節約は、換算の精度ではなく出力を減らすこと。** そちらは較正が
どうであれ確実に効く。
