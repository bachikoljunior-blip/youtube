...
def money_owner(text: str, mod: str, mods: list[str],
    ...
    **検査は `tests/test_forge_money_owner.py`。**
    ...
    ## **`mods` を全 calc へ広げないこと**（2026-09-01 に測った。**負の結果**）

    候補は `mods` ＝ **この回で頼んだ calc だけ**です。だから
    **題の金額が「この回の外の calc」に在ると、この関数は黙ります。**

    実測 2026-09-01: `izoku-masshi-nenrei-kabefuka` は `calc: saishushoku`
    （就職促進給付）に貼られ、`calc_sections` も「90日の区分／330日の区分」——
    **まったく別の制度**でした。題の主役 15,300,000円 は `izoku` の表に在ります。
    **`sankyu` → `kafunenkin` の2件目**で、`tests/test_doc_numbers.py` が拾いました。

    **「では `mods` を全 calc に広げればよい」——測ったら、そうではありません。**
    全 71 calc を候補にして **topics.yaml 全 725件** に当てた結果（API 0単位）:

        動くのは **1件だけ**   `zangyo-sakanobori`   `zangyo` → `kokuho`（一致 1）

    **そしてそれは誤りです。** 題は「残業代 3年さかのぼると最大62万円戻る」で、
    620,000 は **`zangyo` が1つの数として印字しない導出値**（型1）。
    `kokuho` の「7割軽減の境目」に同じ 620,000 が在るだけです。

    **当たりも外れも「一致 1」なので、しきい値では分けられません**
    （上の `izoku` の件も、題の下限1000の金額は 15,300,000 の**1個だけ**）。
    **だから広げると、真陽性1件と偽陽性1件を同時に拾います。**

    **拾う側は `tests/test_doc_numbers.py::test_topics_yamlには掛けない` のままにします**
    —— あれは**止めずに名指しする**門で、型1 と貼り違いの見分け方を文面に持っています。
    **自動で貼り直すのは、この回で頼んだ calc の中だけ。**

    **本当に覆る条件**: 導出値（型1）を機械が見分けられるようになったとき
    —— 「題の金額が、その calc の**2つ以上の印字値の差か比**で出せるか」を
    当て直せば、`zangyo` の 620,000 は型1 と判り、`izoku` の 15,300,000 は
    どう組んでも出ないと判ります。**そこができれば、広げてよい。**
    """
