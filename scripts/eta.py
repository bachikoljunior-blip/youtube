...
def _escape_form(recs: dict | None = None) -> dict:
    ...
    **台帳が閉じた道を、画面の頭が毎周 名指しし続けていました。**
    ...
    for f, v in unsettled.items():
        if _top(v) >= top:
            form, top = f, _top(v)
    # **`comparable` は「比べられるか」だけを言います**（2026-09-02 に足した）。
    #     `escapes` の偽には**2つの意味**が混ざっていました ——
    #     (1) 比べて、逃げ先のほうが低かった  (2) **比べる相手が居ない**
    #     （伸びきった形が1つも無い ＝ `cap` が 0 ＝ `over` は `None`）。
    #     下の `_escape_lines()` は (2) を (1) として刷ろうとして落ちました。
    return {"form": form, "cap": cap, "top": top,
            "over": (top / cap) if cap else None,
            "comparable": bool(form and cap),
            "all_unsettled": sorted(unsettled),
            "escapes": bool(form and cap and top > cap)}


def _escape_lines(pl: dict, bar: str = "###") -> list[str]:
    """**天井の逃げ先の3行を、`headline()` の外で組む。**（2026-09-02 に切り出した）

    ## なぜ外へ出したか

    ここは `headline()` の中に埋まっており、**そのままでは検査から撃てません**でした。
    2026-09-02 16:3x、`scripts/eta.py` が**最初のコマンドで落ちました** ——

        TypeError: unsupported format string passed to NoneType.__format__

    `_escape_form()` の `over` は `cap`（＝**伸びきった形の記録の最大**）が
    0 のとき `None` を返します。この日、**ショートまで `settled: False` に落ちた**
    ので、伸びきった形が1つも無くなり、`cap` が 0 になりました。
    下の else 枝は `×{over:.2f}` を刷ろうとして落ちます。

    **手順（`docs/trigger_main.md` §2.6）は「最初の2手」でこれを撃たせます。**
    落ちると、その回は腕を選べません。**`headline()` の中にある限り、
    この枝は本物のデータでしか通らず、検査で押さえられませんでした。**

    ## 覆る条件

    `settle` がどれか1つの形を「伸びきった」と出したら `cap` が戻り、
    3つ目の枝（比べられない）は自分で黙ります。**定数を持ちません。**
    検査は `tests/test_eta_escape_form.py`。
    """
    out: list[str] = []
    # --- **`per_video` の天井から `rpm` へ逃がすとき、逃げ先が測れているか** ---
    #     （2026-08-31・最適化の回に足した。**API 0単位**）
    #
    #     `config/hypotheses.yaml` の `per_video` の天井は 1,891 で、`unit` が
    #     自分で「**24時間・ショート39本**の実測の最大」と書いています。
    #     `escape: rpm` は「形を替えれば外れる」という意味ですが、
    #     **その逃げ先（長尺）の1本あたり再生は、天井どころか測れていません。**
    #
    #     同じ `escape_note` が「**長尺の1本あたり再生はまだ測れていません**」と
    #     書いており、この回の実測（`settle.settles_at('長尺')`）はその理由を出しました
    #     —— **長尺は、この標本が届くどの地平でも伸びきりません**
    #     （地平 336h で 240h の伸びきり **0本** ／ ショートは地平 480h でも 48h で 100%）。
    #
    #     **つまり「per_video は天井、だから rpm」は、片方の形でしか成り立ちません。**
    #     ショートでは天井（1,891・実測）。長尺では**天井ではなく、未計数**です。
    #     腕を選ぶ3行がそこを言わないと、次の回は
    #     「per_video はもう伸びない」と読んで、**測れていないほうを測りに行きません。**
    #
...
def conversion_split(form: str = "長尺") -> dict | None:
    ...
    **いまのインプレッションのまま**、手本の CTR で回すと
    ...
    ## **その ×3.2 は「安い半分」であって、「効く半分」ではありません**

    最初の版はここに「**面は在ります。足りていないのは押された率のほう**」と
    書いていました。**絶対の量で見ると逆です**（撃って気づいて直した）::

        面は 1本あたり **239回**（5,012 ÷ 21本）
        **CTR を 100% にしても 239再生** —— 要る 3,333回 には **×14 届きません**
        手本の率のままで面を **×263** にして、はじめて届きます

    **サムネをいくら直しても、いまの面では目標に届きません。**
    CTR は「**いまの面の中で取りこぼしている ×3.2**」であって、
    **面の不足の代わりにはなりません。** 2つは足し算ではなく掛け算の別々の項です。
...
def report(m: dict, a: dict) -> list[str]:
    ...
    #
    #     `views_per_day` は Analytics の実測（直近28日）です。**測り方は正しい。**
    #     ところが**その28日に出した本数は 1日 7〜34本**（予約の消化）で、
    #     **規則は 1日1本**（`src/house_rule.py`・オーナーが固定・覆る条件なし）。
    #
    #     規則の下で続く再生／日 は `per_video_now × PUBLISH_PER_DAY` です。
    #     **同じ画面が、下の「公開の密度」の表でその数を出しています** ——
    #     実測 2026-08-31: 門1 は **1,140日（2029-10-15）** と
    #     **3,296日（2035-09-10）**。**2,156日 ちがう2つが、同じ報告の中に居ました。**
    #     そして頭の要約が引くのは上（1,140日）＝ **規則が禁じた本数で作られた側**です。
    #
    #     **数は直しません**（実測を模型で置き換えないこと）。**食い違いを言います。**
    #     どちらに転んでも 2,724回/日 は続きません ——
    #     予約が出続けるなら**規則に反しています**し、止めれば**再生が落ちます。**
    #     **覆る条件**: 直近28日の公開本数が規則に収まったとき、この行は消えます。
    for _line in rule_sustain_lines(a):
        P(_line)
    ...
    if _cs and _cs.get("headroom"):
        ...
        P(f"      → いまのインプレッションのまま、いちばん押された本の率で回すと"
          f" クリックは {_cs['clicks']:,.0f} → **{_cs['clicks_at_best']:,.0f}"
          f"（×{_cs['headroom']:.1f}）**。"
          f" `src/reach_split` の読み方: **面が少ない→題材・本数／CTR が低い→サムネと題**。")
        # --- **その ×3.2 は「安い半分」であって、「効く半分」ではありません**（2026-08-31）---
        #     最初の版はここに「**面は在ります。足りていないのは押された率のほう**」と
        #     印字していました。**絶対の量で見ると逆です。** 撃って気づいて直した:
        #         面は 1本あたり **239回**（5,012 ÷ 21本）。
        #         **CTR を 100% にしても 239再生**で、要る 3,333回 には**×14 届きません。**
        #     ＝ **サムネをいくら直しても、いまの面では目標に届きません。**
        #     CTR は「いまの面の中で取りこぼしている ×3.2」であって、
        #     **面の不足（×263）の代わりにはなりません。**
        _ipv = _cs["impressions"] / _cs["n"] if _cs["n"] else 0.0
        _need_pv = (a.get("per_video_needed") or {}).get(nearest) or 0.0
        if _ipv > 0 and _need_pv > 0:
            P(f"      [!] **その ×{_cs['headroom']:.1f} は『安い半分』であって"
              f"『効く半分』ではありません。** 面は1本あたり **{_ipv:,.0f}回**"
              f"（{_cs['impressions']:,.0f} ÷ {_cs['n']}本）。"
              f"**CTR を 100% にしても {_ipv:,.0f}再生**で、要る"
              f" {_need_pv:,.0f}回 には **×{_need_pv/_ipv:,.0f} 届きません。**")
            P(f"          ＝ **サムネだけでは、この面では目標に届きません。**"
              f" 面そのものを **×{_need_pv/(_ipv*_cs['ctr_best']):,.0f}**"
              f"（いちばん押された本の率のままで）増やす必要があります —— "
              f"**題材・本数・面の側**（`src/reach_split` の左の枝）。"
              f" CTR は『いまの面の中で取りこぼしているぶん』です。")
    ...
    if _gaps:
        ...
        #
        #     「届きません」を作っている定数は2つで、片方は `per_video` の天井 **1,891**
        #     ＝ **標本の最大**です。**標本の最大を「天井」と呼ぶのは、ふつうは誤り**
        #     （裾の重い分布では、最大は本数と一緒に伸び続ける）。
        #     **この機械は、その反論に答える数を持っていませんでした。**
        #
        #     `form_record._tested_by()` が、同じファイルから測ります ——
        #     記録が立ったのが何本目で、**そのあと何本が挑んで抜けなかったか**。
        #     実測 2026-08-31: ショート **139本**（17本目で立った）／長尺 **9本**。
        #     **形によって、天井の確からしさが桁で違う** ——
        #     そこが1行も出ていませんでした。
        _tested = []
        for _f in ("ショート", "長尺"):
            _r = _recs.get(_f) or {}
            if _r.get("tested_by") is None:
                continue
            _tested.append(
                f"{_f} 記録 {_r['best']:,}回 は **{_r['record_rank']}本目**で立ち、"
                f"そのあと **{_r['tested_by']}本** が抜けていません")
        if _tested:
            P("    --- **その天井は、何本の反証に耐えたか** ---"
              "（`src/form_record._tested_by`・同じ `data/views.jsonl`・API 0単位）")
            for _t in _tested:
                P(f"      {_t}")
            _sh = (_recs.get("ショート") or {}).get("tested_by")
            _lo = (_recs.get("長尺") or {}).get("tested_by")
            if _sh is not None and _lo is not None:
                P(f"      → **標本の最大を『天井』と呼ぶのは、ふつうは誤り**です"
                  "（裾が重ければ最大は本数と一緒に伸び続ける）。**だから数えます。**"
                  f" ショートの天井は **{_sh}本 の反証に耐えています** ——"
                  " この形に『まだ大きいのを引いていないだけ』は効きません。"
                  f" 長尺は **{_lo}本** だけで、**天井がまだ立っていません**。"
                  " **残りの隔たりを閉じられるとしたら長尺の側**です"
                  "（そして `¥2,000` の帯は長尺の側）。")
                P("      **これは「長尺なら届く」ではありません。**"
                  " 言っているのは1点だけ ——"
                  " **2つの『天井』は、確からしさが桁で違う。同じ重さで読まないこと。**")
                P("      **覆る条件**: 記録が更新されたら `tested_by` は **0 に戻ります**"
                  "（定数を持ちません）。戻った回は、"
                  "**その天井は一度も反証にかかっていない**ということです。"
                  " 公開の順は「いちばん古い観測の時刻 − そのときの年齢」の推定です。")
...
def observed_gate_note(remaining: int, a: dict) -> str:
    ...
    **覆る条件**: 観測の窓で動いた人数が増えて（目安 20人以上）、模型との比が
    ...
    # --- **この観測は、どの世界で測ったか**（2026-08-31・最適化の回）---
    #     すぐ下に `rule_sustain_lines()` が出ます（別の回が同じ日に足したもの）——
    #     「その再生／日 は**規則の外の本数**が作った数で、規則の下では 1/2.89」。
    #     **2つは別の軸です。掛けるか、片方を選ぶかで、答えが 3倍 変わります**::
    #
    #         こちら  模型 0.86 に対し **実際に返ったのは 0.59**（模型の当たり外れ）
    #         あちら  その 0.59 は **予約を消化していた日々**の中で出た数
    #                 → 規則に収めると再生が 1/2.89 になり、**さらに下がる**
    #
    #     **だから「どちらか正しいほう」ではなく、順に掛かります。**
    #     読む側がそこを迷わないように、こちらから下の行を名指しします。
    #     **掛けた数は出しません** —— 置くと、それ自体が新しい模型になります
    #     （この関数が「差し替えない」と言っている理由と同じ）。
    #
    #     **覆る条件**: 公開本数が規則に収まると `rule_sustain_lines()` は消えます。
    #     そのとき、この但し書きも一緒に消えること（下の `if` がそれを見ています）。
    try:
        _rule_note = bool(rule_sustain_lines(a))
    except Exception:                                          # noqa: BLE001
        _rule_note = False
    if _rule_note:
        note += ("　[!] **その観測は「予約を消化していた日々」の中の数です** ——"
                 " すぐ下の行が言うとおり、規則に収めると再生／日 はさらに落ちます。"
                 "**2つは別の軸なので、どちらかを選ぶのではなく順に掛かります**"
                 "（掛けた数はここに置きません —— 置くと、それが新しい模型になります）。")
    return note
        ...
        if ci:
            ...
            if need_ctr > hi:
                ...
                if per_pub > 0:
                    ...
                    # **要る本数は、いつも出すこと**（2026-08-31・最適化の回に直した）。
                    #
                    #     ここは `pubs > 0` のときだけ印字していました。ところが
                    #     **下の断りの条件は `need_pub > _cap_long`** で、
                    #     `pubs == 0` の回は**読む側に見えていない数で判定**しています。
                    #     実測 2026-08-31: `planned_pubs = 0`（規則が1日1本なので、
                    #     長尺の計画本数が 0 に落ちる回がある）→ 要る 3.08本/日 は
                    #     **一度も印字されず**、天井 6本/日 との比較だけが裏で走っていました。
                    #     この repo で名前の付いている「2か所が別々に言っている形」です。
                    #
                    #     **いまの本数が 0 でも、要る本数は決まります**（面 ÷ 公開1本あたり）。
                    #     0 のときは「いま 0本/日」と書いて、**同じ行に両方 出します。**
                    line += (f"、公開1本あたり {per_pub:,.1f}回 なので"
                             f" **長尺 {need_pub:,.1f}本/日**"
                             + (f"（いま {pubs:.2f}本/日）" if pubs > 0 else
                                "（**いま 0本/日** —— この回の計画に長尺が入っていません。"
                                "`planned_pubs` が 0 なので、"
                                "**この段は計画に長尺を1本 入れるまで動きません**）"))
    ...
    **上振れの本数と、下振れの1本あたりを、同時に当てていた**わけです。
    ...
    if sensitivity:
        ...
        if best and best[_key] > 0:
            ...
            #
            #     [!] **すぐ上の「向きが別なので掛かる」は、2026-09-01 に
            #     測ったら偽でした。** 1本ずつ抜いた比は `per_video` ×1.585 /
            #     `rpm` **×1.0000** / `sub_rate` **×1.0000** —— 3本の joint は
...
#: **上流が、その控えを作るまでにかかる時間**（2026-09-01 に実測して足した）。
#:
#: 値は `(中央値, 最大, 実測した日数, 何の待ちか)`。**時間・単位ともに「時間」。**
#:
#: ## なぜ齢の隣に要るか（**この回が 30分 使って気づいたこと**）
#:
#: `instrument_ages()` は 08-28 から齢を正しく出し、その隣で
#: 「**取り直す手は `scripts/reach.py` …日枠が尽きていても、前の3つは
#: 別の枠なので通ります**」と言っています。**枠の話は正しい。**
#: それでも **2026-09-01 の回が `scripts/reach.py` を撃って、新しい報告は 0本**
#: でした —— Reporting API は**その場で数字を返しません**。ジョブを作ると
...
#: 遡って埋まった分は除いています）:
#:
#:     最小 **20.8時間** ／ 中央値 **34.0時間** ／ 最大 **113.6時間**
#:     直近5日  29.5 ／ 113.6 ／ 46.3 ／ 61.3 ／ **48.6時間**
#:
#: **＝ `data/reach.jsonl` は、どれだけ撃っても 21時間 より新しくなりません。**
#: この回の齢は **67時間** で、**その帯の中**です（止まっているのではなく、
#: 上流がまだ作っていない）。08/24 に ¥184 → ¥287 を起こした止まりは **7日
#: ＝ 168時間** で、**この帯の外**でした。**齢だけでは、その2つが区別できません。**
#:
#: ## しきい値ではありません（`tests/test_eta_instrument_ages.py` の向き1）
#:
#: **何も黙らせません。** 齢は毎回そろって出ます。ここが足すのは
#: **隣に置くもう1つの事実**だけで、「古い／新しい」の判定はしません ——
#: 判定を機械に持たせると、こんどは「何時間で鳴らすか」を次の回が世話します。
#:
#: **覆る条件**: 撃った回数と、そのとき増えた行数を帳面に残せるようになったら、
#: この定数は要りません（実測から毎回 出し直せます）。そのときは
#: `scripts/reach.py` の側に「増えた行数」を書かせ、ここを消すこと。
_UPSTREAM_LAG: dict[str, tuple[float, float, int, str]] = {
    "data/reach.jsonl": (34.0, 113.6, 15, "Reporting API が CSV を作る間隔"),
}
...
def instrument_ages(now: datetime | None = None) -> list[str]:
    ...
    **一覧をここに写さないこと**（写した日から古くなります）。
    ...
    ## **通ることと、増えることは別です**（2026-09-01 に踏んだ）

    上の段落は枠の話だけをしていて、**「撃てば控えが進む」とは言っていません。**
    それでも 2026-09-01 の回は「面 **67時間**」を読んで `scripts/reach.py` を撃ち、
    **新しい報告 0本**でした —— Reporting API は**その場で数字を返さず**、
    YouTube が自分の都合で1日1回 CSV を置いていくからです。
    **だから齢の隣に、上流の作る間隔（`_UPSTREAM_LAG`）を事実として置きます。**
    帯の中なら待ち、帯の外なら止まり —— **齢だけでは区別が付きません**
    （67時間 は帯の中、08/24 の ¥184 を起こした 168時間 は帯の外）。
    検査は `tests/test_eta_upstream_lag.py`。
    ...
    **覆る条件**: 1周ごとに全部の控えを取り直す作りにしたら、この行は
    ...
    for label, rel, how in _CEILING_SOURCES:
        ...
        tail = "（日枠が尽きるまで取り直せません）" if blocked else ""
        lag = _UPSTREAM_LAG.get(rel)
        if lag is not None:
            med, mx, n, why = lag
            # **判定はしません。**（`tests/test_eta_instrument_ages.py` の向き1）
            # 齢の隣に、上流の作る間隔を**事実として**置くだけ。
            # 帯の中か外かは、その回が目で見て決めます。
            tail += (f"（上流の作る間隔 **中央値 {med:,.0f}時間・最大 {mx:,.0f}時間**"
                     f"・実測 {n}日 —— {why}。**これより新しくはなりません**）")
        parts.append(f"{label} **{hours:,.0f}時間**" + tail)
    if not parts:
        return []
    return ["   天井が乗っている控えの齢: " + " ／ ".join(parts)
            ...
            " —— ただし**通っても、上流がまだ作っていなければ 0行 です**"
            "（実測 2026-09-01: `scripts/reach.py` を撃って新しい報告 **0本**。"
            "隣の『上流の作る間隔』の帯**の中**なら、それが正常です）"]
...
def gate_lines(bar: str = "###", tr: dict | None = None) -> list[str]:
    ...
    **確率の数字を作りません。** 審査に出した実績が 0回 なので測れません
    ...
    if q.get("upcoming"):
        out.append(
            f"{bar} [!] **機械が1回も起きなくても、予約済み {q['upcoming']:,}本 は公開され続けます** ——"
            ...
            f"（混在の窓は予約が尽きるまで）。"
            # **2026-08-31 に直した**: ここには「引っ込める `reschedule.py` は
            #     `src/pause_guard` の対象で、この機械からは止められません」と
            #     書いてありました。**停止が解けたので、いまは撃てます。**
            #     出力に残った「できません」は、**やらない理由を毎周 配ります。**
            f"（引っ込めない、というのは**決めごと**です"
            f"（`docs/CONSTRAINTS.md` B14）—— `reschedule.py` は"
            f"いま撃てます。**撃たない理由は上の1行**で、"
            f"「機械が止まっているから」ではありません。）")
    ...
    # **閉じ切っている回に `--lever gate` を名指ししないこと**（2026-08-31 に直した）。
    #     ここは条件なしで「動かせるのは gate だけ」と印字していました。
    #     **残り 0件 の回にそう言うと、閉じるものが1つも無い腕を毎周 名指しします**
    #     —— しかも停止はもう解けているので、4本の腕はどれも引けます。
    #     **`levers.reconcile` から見ると、律速でない腕を宣言した回**になります。
    if opened:
        out.append(
            f"{bar} **だから、この回に到達日を動かすのは4本の腕ではありません** ——"
            f" 動かせるのは **審査に通る形を1つ決めること**（残り {opened}件）だけで、"
            f"それが `--lever gate` です（`src/levers.py`）。**腕は、その後でなければ効きません**"
            f" —— 塞がったまま本数を積んでも、審査に落ちれば**再生は1円にもなりません。**")
    else:
        out.append(
            f"{bar} **{total}件 とも閉じているので、この回に `--lever gate` で"
            f"閉じるものはありません。** 4本の腕（`per_video` / `rpm` / `density` /"
            f" `sub_rate`）は**どれも引けます** —— 停止は 2026-08-31 に解けています。"
            f" **閉じた根拠は上限であって、出来上がりの実測ではありません** ——"
            f"実測で当て直して外れたら、その件を**開き直して直すこと**"
            f"（`--open-gate`。**全体を止めないこと**）。")
    out.append(
        f"{bar} 閉じるときは `python scripts/eta.py --close-gate <番号> --evidence \"<どこに何を記録したか>\"`。"
...
def headline(pl: dict, prev: dict | None = None,
    ...
    **3回言われている ＝ まだ形になっていない。**
    ...
    #     **「その日 2本以下しか出さなかった日」の本だけ**で測った数です。帯を切る
    #     理由は正しい（弾力性 -0.733・t=-4.26）が、**帯に出口がありませんでした** ——
    #     実測 2026-09-02: 帯に入った日は **08-04〜08-18 の 12日**、08-19 以降の
    #     **12日は全部 帯の外**。**この数は 08-19 以降の本を1本も見ていません。**
    #
    #     そして**止まった数は下がりません。上がって見えることさえあります** ——
    #     `data/eta.jsonl` の `per_video_now` は 08-30 **603** → 08-31 **942**
    #     （帯を切る形に替えた日）。同じ窓で実物の1本あたりは **839回 → 121回**。
    #     **回が読む数だけが上がっていました。**
    #
    #     `data/runs.jsonl` 302件（08-29〜09-02）の内訳は `fix` **73%** ／
    #     `improve` **4%**。回はこの落ちを見る手を持っていなかったので、
    #     本文の真ん中の「名指しされた欠陥 36件」から1件 選び続けていました。
    #     **この3行は、その落ちを頭（実際に読まれる場所）へ出します。**
    #
    #     **覆る条件**: 規則（1日1本・`src/house_rule.py`）どおりに出た日は
    #     必ず帯に入るので、標本が動き出せば `stale` が False になり、
    #     この3行は**黙って消えます**（消えたことが、規則が効いた印です）。
    #     **腕の名前で条件を切らないこと。** 止まっているのは `per_video` の値ですが、
    #     天井の分子は全部そこから出るので、**どの腕が名指しされた回でも
    #     比べている相手が古い**まま進みます。`stale_lines()` は止まっていなければ
    #     **0行**を返すので、条件は要りません（**止まっていないことが条件です**）。
    try:
        from src import rule_per_video as _rule_pv
        for _sl in _rule_pv.stale_lines():
            out.append(f"{bar} {_sl}" if _sl.startswith("[!]") else _sl)
    except Exception as _e:          # pragma: no cover - 道具が欠けても頭は出す
        out.append(f"{bar} [!] `rule_per_video.stale_lines()` が撃てません: {_e}")
    ...
    # --- **来ない日を待っている前提**（2026-08-31 21:1x に足した） -------------
    #
    # ## なぜ見出しに出すか
    #
    ...
    # 出るのが 200行ある出力の末尾で、**この3行の見出しに入っていなかった**から。
    #
    # ## そこで何が起きるか
    #
    # すぐ上の行（`期日の来た前提があります`）は**期日**だけを見ます。
    # `needs` が「**来ない日**」（例: 09/10 に 16本 公開した日の読み）を
    # 待っている前提は、**期日が来ないので、この見出しに一生 出ません。**
    # そして同じ関数は「軌跡の腕が動くのは前提を1件 閉じたときだけ」と
    # 印字するので、**到達日はそこで止まったままになります。**
    #
    # ## 実測 2026-08-31（この行を足した回）
    #
    # この見出しは「**この回に閉じられる前提はありません**」と出していました。
    # **間違いです。** 同じ日に、期日 09-10 の前提
    # （「1日に再生が付く本数の上限（10本）は上がらない」）が
    # **公開ずみの8日ぶんのデータだけで、10日 早く閉じられました。**
    # `needs` が来ない日（09/10 の 16本）を名指ししていただけで、
    # **同じ形の日は 08/20〜08/30 にすでに 8日 在りました。**
    #
    ...
    _claims = [c[:40] for c in house_rule.unreachable_claims()]
    if _claims:
        out.append(
            f"{bar} **来ない日を待っている前提が {len(_claims)}件 あります**"
            f"（規則は 1日{house_rule.cap()}本。要件が名指しした日は**来ません**）→"
            " **その前提は、待っても永久に閉じません。**"
            " **公開ずみの日で代わりが立たないかを見ること** —— "
            + " ／ ".join(f"「{_c}…」" for _c in _claims[:3])
...
def _report_long_gate(m: dict, a: dict) -> list[str]:
    ...
    **その「届きません」は、長尺の実力ではなく「長尺を1本も出していない」ことの
    ...
    if lpv is None:
        ...
        # --- **同じ隔たりを、平均ではなく「これまでの最高」でも測る**
        #     （2026-08-31・最適化の回。`src/form_record.py`・API 0単位）
        #
        # 上の行は**平均 16.0回/本**で割っています。それは「登録者 23人 の
        # チャンネルに出した 21本」の平均で、M20 が「長尺の実力ではない」と
        # 書いている数そのものです。**ほぼ 0 の分母で割ると倍率は無限に出ます。**
        #
        # **この機械は、天井の帯では既に同じ直しを済ませています** ——
        # 記録で割ったら、いちばん近い帯が `ショート 高 ×117.9` から
        # `長尺 お金 高 ×21.4` へ**入れ替わりました**（5.5分の1）。
        # **門2a の側だけ、その直しが入っていませんでした。**
        #
        # そして**規則3 が追うのは1本です**（「次の投稿予定までにそこで投稿する
        # 動画を改善し続ける」）。1本が門を越えられるかを問うなら、
        # **分母は平均ではなく記録**のほうが単位が合います。
        #
        # **記録は下限です**（`settled` が偽 ＝ その形は伸びきっていない）。
        # 下限で割った隔たりは**上限**なので、**そう名乗ります。**
        try:
            _fr = form_record.per_video_best()
            _lg = (_fr or {}).get("長尺") or {}
            _best = float(_lg.get("best") or 0)
        except Exception:                                  # noqa: BLE001 — 回を止めない
            _best = 0.0
        if worst and _best > 0:
            _short_by, _label, _pd, _need = worst
            P(f"    --- **同じ隔たりを「これまでの最高」で測ると** ---"
              f"（`src/form_record.py`・API 0単位）")
            P(f"      長尺の記録 **{_best:,.0f}回/本**（n={_lg.get('n')}・`{_lg.get('id')}`）"
              f" 対 合格点 **{_need:,.0f}回**（{_label}・L={_pd}本/日）"
              f" → **×{_need / _best:,.2f}**")
            P(f"      **平均で割った ×{_short_by:,.1f} の "
              f"{_short_by / max(1e-9, _need / _best):,.1f}分の1** です。"
              " 天井の帯で `ショート 高 ×117.9` が `長尺 お金 高 ×21.4` に"
              "入れ替わったのと**同じ直し**を、門2a の側にも当てています。")
            if not _lg.get("settled"):
                P(f"      [!] **その記録は下限です** —— `settle.settles_at('長尺')` は"
                  " どの地平でも伸びきる年齢を返しません（`MATURE_HOURS_BY_FORM['長尺']"
                  " = 96` で打ち切った数）。**上の倍率は隔たりの『上限』**で、"
                  "実測の隔たりはこれより小さく出ます。")
            P(f"      **これは「開く」と言っているのではありません** —— 合格点は"
              f" **{_pd}本/日 を 365日 続けたときの、1本ごとの数**です。"
              f" 記録は **1本**の数（n={_lg.get('n')} のうちの最大）。"
              "**1本で出せたことと、365日 出し続けられることは別**です。"
              " ここで言えるのは1点だけ: **隔たりは桁ではありません。**")
...
def rule_sustain_lines(a: dict, prefix: str = "        ") -> list[str]:
    """**門1 の『再生／日』が、規則の外の本数で作られていないか。**（2026-08-31）

    ## 見つけ方（**この回に、同じ報告を上から下まで読んで気づいた**）

    同じ1つの出力の中に、門1 の日付が **2つ** あります::
    ...
    **測り方はどちらも正しい。** 食い違っているのは**本数**です ——
    直近28日に実際に出た本数は **1日 7〜34本**（予約の消化。控えは 359本）で、
    **規則は 1日1本**（`src/house_rule.py`・オーナーが固定・**覆る条件なし**）。

    ## なぜ「数を直す」のではなく「食い違いを言う」のか

    `views_per_day` は**実測**です。**実測を模型で置き換えないこと** ——
    置き換えた瞬間、この機械は「測った」と「そう置いた」の区別を失います。
    だからここは1行も数を変えず、**2つが在ることと、どちらが規則の側かだけ**を言います。

    **どちらに転んでも、いまの再生／日 は続きません**::

        予約が出続ける  → **規則に反しています**（1日1本ではない）
        予約を止める    → **再生／日 が落ちます**（2,724 → 942 ＝ 0.35倍）

    ## 覆る条件

    - **直近28日の公開本数が規則に収まったとき、この行は自動で消えます**
      （`views_per_day` が `per_video_now × PUBLISH_PER_DAY` の 1.2倍 を下回る）。
      **定数は、その 1.2 の1つだけ**です —— 実測の揺れで毎回 出ないための余白で、
      いまの倍率は **2.89倍** なので、この余白では隠れません
    - **`per_video_now` が規則と同じ密度の日で測られていること**が前提です
      （`src/rule_per_video.py`。混ぜた平均 499回 で解くと、この行は 1.9倍 甘く出ます）
    """
    try:
        rule_pd = float(house_rule.PUBLISH_PER_DAY)
        per_video = float(a.get("per_video_now") or 0.0)
        views = float(a.get("views_per_day") or 0.0)
        subs = float(a.get("subs_per_day") or 0.0)
    except (TypeError, ValueError):
        return []
    sustain = per_video * rule_pd
    #: **実測の揺れで毎回 出さないための余白。** この関数が持つ唯一の定数です。
    if sustain <= 0 or views <= sustain * 1.2:
        return []
    ratio = views / sustain
    slow = subs / ratio if ratio else 0.0
    return [
        f"{prefix}[!] **その {views:,.0f}回／日 は、規則の外の本数が作った数です。**"
        f" 規則は **{rule_pd:,.0f}本/日**（`src/house_rule.py`）で、"
        f" その下で続く再生／日 は **{sustain:,.0f}回**"
        f"（1本あたり {per_video:,.0f}回 × {rule_pd:,.0f}本）＝ いまの **{1 / ratio:,.2f}倍**。"
        " 差は**予約の在庫を消化しているぶん**です。",
        f"{prefix}→ **規則の下の速さは 1日 {slow:.2f}人**"
        f"（{subs:.2f} ÷ {ratio:,.2f}）。"
        "**下の「公開の密度を上げたら、門1はいつ通るか」の『規則』の行が、その数です** ——"
        " 同じ報告の中に、門1 の日付が**2つ**あります。"
        "**頭の要約が引くのは、この行の上のほう**（＝規則が禁じた本数で作られた側）です。",
        f"{prefix}**数は直していません**（実測を模型で置き換えないこと）。"
        "**言っているのは食い違いだけ**です —— どちらに転んでも いまの再生／日 は続きません。"
        "予約が出続けるなら**規則に反して**おり、止めれば**再生が落ちます。**",
    ]
