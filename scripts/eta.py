...
# --- オーナーが固定した床（`src/house_rule.py` が正本）---
try:
    from src.house_rule import PUBLISH_PER_DAY as HOUSE_PUBLISH_PER_DAY
except Exception:  # 単体で走らせたとき
    HOUSE_PUBLISH_PER_DAY = 1

# --- 公開の密度（1日に何本「公開」するか。投稿＝予約とは別物）---
#     いまの予約は 246本が39.5日に散って 1日6.4本。詰めれば25本（受け取り帳 3c7e12a3）
#
# 2026-08-31 追記: **下限が 4 だったので、オーナーの床（1本/日）を
# この表で表現できませんでした。** 与件は「1日一本・作り置きなし」で、
# `src/house_rule.PUBLISH_PER_DAY` が正本。床を場に出すために 1 を足しています。
# これを足すまで、`plan()` は**床の 25倍**の供給しか解いていませんでした。
PUBLISH_SCENARIOS = (HOUSE_PUBLISH_PER_DAY, 4, 10, 25, 92)

# --- いま計画している密度（受け取り帳 3c7e12a3 の詰め直しが着地する所）---
#     門2a の逆算は「門1 が通る日まで」で割るので、この1つを正本にします
#
# **注意（2026-08-31）**: この 25 は**オーナーの与件と矛盾しています**。
# 与件は 1本/日（`src/house_rule.PUBLISH_PER_DAY`）。ここを 25 のままにすると
# 到達日も段1〜4 も腕の順位も、**存在しない供給**の上で決まります。
# 落とすと 246本の検査と全部の印字が動くので、この回では**場に出すだけ**にして
# 落としていません。`scripts/house_gap.py` が差を数えます。**次の回が決めること。**
