...
- **広いほうが 30% を超える**（＝ `耳の負荷` の 13.4% を追い越し、
  ...
  **2026-09-02 に撃ち直しました（控え 694本・6,206コマ）**:

      狭い  1個以上 **478コマ（7.7%）／150本（21.6%）**
      広い  1個以上 **971コマ（15.6%）／234本（33.7%）**
      多い語（広い）: 棒 381・帯 369・この表 122・画面の 108・いちばん下 42

  **コマで見た広いほうは 15.7% → 15.6%。この合図は鳴っていません**（横ばい）。
  **本で見ると 33.7%** ですが、上の 30% の線は**コマの数**で書かれています
  （`耳の負荷 13.4%` と同じ物差しで並べるため）。**混ぜて鳴らさないこと。**

## **【2026-09-02】この (c) の前提が、オーナーの与件で1つ変わりました**

上の (c) は「**3つのうち、いちばん小さいから後回し**」＝ **腕の大きさの話**です。
2026-09-02、オーナーが**固定その3**を出しました（`CLAUDE.md` 冒頭・原文）:

> **「動画内の説明は人間にわかるようにして」**
> 「**分かるかどうかを機械が見る所を作ること。** どう測るかは、その回が決めてよい」

**これは `A14` の外側**です。つまり「分かること」は、
**ほかの腕と大きさを比べて後回しにしてよい対象ではなくなりました。**
上の (c) の理由（いちばん小さい）は、**そのままでは (c) を支えません。**

**次にここを読む回が決めること（この回は決めていません）**:

  - この道具を配線して「分かるかどうかを機械が見る所」にするのか
  - それとも、`_check_ear_load`／`slide_pace` の側を**その所**と数え、
    ここは (c) のままにするのか（**そう決めるなら、どこが「その所」かを
    ここに名前で書くこと** —— 書かなければ、次の回も同じ三択をやり直します）

**倒すときは、`tests/test_unwired_tools.py` の3件が (c) の目印を数えています。**
(a) へ倒したら、別の (c) へ差し替えるか、その検査ごと消すこと（各 docstring の覆る条件）。
