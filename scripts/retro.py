...
def main() -> int:
    ...
    if carried and r.folded:
        ...
        if zero:
            ...
            if open_zero:
                # **名前を出すこと。件数だけにしないこと**（2026-09-01 に踏んだ）。
                # ここは長らく「**この N件 から選ぶのが既定です**」とだけ印字して
                # いました。**直前に名前が並んでいるのは `suspect` のほう**で、
                # それは `_sinks()` で除いたはずの側です。実測（この回の出力）:
                #
                #     そのうち **2件** は、3周 以上 運ばれて…
                #         python scripts/pool_drain.py --apply   ← 枠で塞がっている
                #         [pool] [!]                             ← 枠で塞がっている
                #     …
                #     **この 2件 から選ぶのが既定です。**   ← 中身は waived / --claim
                #
                # **件数まで 2件 対 2件 で一致しています。** 読む側から見ると、
                # 「この2件」の指す先は直前に名前の出ている2件しかありません ——
                # **この回で 403 になると同じ出力が言っている側**です。
                # `_sinks()` の註（「この回で打てない手を勧めます」）は正しく、
                # 選び方も正しく、**印字だけが逆を指していました。**
                #
                # **覆る条件**: `suspect` の並びをこの下へ動かしたら、
                # 直前の名前は別物になります。そのときも名前は出すこと ——
                # 順で守るのは「たまたま当たっている」だけです。
