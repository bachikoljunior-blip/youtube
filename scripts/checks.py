...
PARENT_GLOBS = (
    ...
    # 2026-09-13 14:0x に足した（optimizer・Opus）。**同じ形の 4件目** ——
    # 置いた回に一緒に束へ入れました（`turf_run` / `method_growth` は置いてから
    # 1〜3日 のあいだ 1度も撃たれていなかった側）。**見張りを置く回が、束にも入れること。**
    "test_owner_words*.py",
    # 2026-09-14 02:5x に足した（`hourly`・Opus）。**置いた回に一緒に束へ入れた 2件目** ——
    # `owner_ask` は `owner_words` と同じ「オーナーの言葉の側の物差し」で、`studio` を
    # import しないので `studio` の束にも入りません。**見張りを置く回が、束にも入れること。**
    "test_owner_ask*.py",
)
