...
# ## 上限の出どころ（**定数を書かないこと**）
#
# `src.house_rule.PUBLISH_PER_DAY`。**この註に数を写さないこと**（2026-09-05）——
# ここは「＝ **1本/日**」と写しており、2026-09-04 17:3x にオーナーが床を外して
# （原文「目標以外全部外して良いよ」）**10本/日 になった日に、そのまま残りました。**
# 数は `house_rule` の側にしか在りません。理由と覆る条件はあちらの註。
#
# 下の原文は**オーナーが 2026-08-31 に固定した規則**です（`src/house_rule.py`・
# `CLAUDE.md` 冒頭・`docs/GOAL.md`。**消さないこと** —— 09/04 の
# 「目標以外全部外して良いよ」で**上書きされていますが、矛盾ではなく上書き**です）:
#
...
#
# **覆る条件**: **オーナーが自分の言葉で外したとき ―― 2026-09-04 17:3x に外れました。**
# （この行は長らく「**ありません**」でした。外れた日に、ここだけ残りました。）
# `density_verdict` の倍率が 0.5 以上に戻っても、**この上限は動きません** ——
# 帯が広がることと、規則が変わることは別ものです。**上限は `house_rule` の1か所だけ。**

#: **上限が読めなかった回の既定。** **わざと 1 のままにしてあります**（2026-09-05）——
#: `house_rule.PUBLISH_PER_DAY` は 10 になりましたが、ここは**読めなかった回**に
#: 落ちる先なので、**安全な側（少ないほう）へ倒します。**
#: **読めない回に無制限へ落ちないこと** —— 落ちると、計器が壊れた回だけ 22本/日 に戻ります。
#: **「規則と同じ数にすること」と書いてあった註は外しました** —— 同じ数にすると、
#: 計器が壊れた回が**規則いっぱいまで置きます**。ここが守るのは規則ではなく事故のほうです。
