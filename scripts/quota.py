...
#
# **訂正（同日 21:3x・オーナーが別のセッション経由で入れた原文。正本 `docs/OWNER_MODEL_BUDGET.md`）**:
#
# > **「Fableはそれ以外含めた全体週間使用量の50%だってことを言ってないなら直して。fableのみは100％使っていい」**
# > **「全モデルの50%っていう公式仕様だわ！」**
#
# ＝ 「50%」は公式の枠の構造（Fable に使えるのは通常の週間上限の 50% ぶん）で、
# 画面の「Fable のみ」はその 50% ぶんを 0〜100% で表す内訳の目盛り。
# **だから切り替え点は「Fable のみ」100%** であって 50% ではない（親は 50 で止めかけた ＝ 読み違い）。
# 親が撃つのは `scripts/next_round_owner.py`（この目盛りを 100% で門にする包み）。
# ここは同じ数に合わせておく（2か所が別の数を言わないため）。
...
def _births_between(rows: list[dict], start: datetime, end: datetime) -> int:
    ...
    **覆る条件**: `next_round.py` が `--record` を書かなくなったら、
    ...
    ## !! **【2026-09-20 02:1x】この註の「周」は、2026-09-08 17:0x から「親が起きた回」です**
    ##（optimizer・Opus 5・ultracode・1周 1体・**数えただけ・式は 1つ も動かしていません**）

    上の註は **2026-09-01** に書かれていて、そのとき `data/rounds.jsonl` に載るのは
    **GO で立てた周だけ**でした。**2026-09-08 17:0x にそれが変わりました** ——
    `next_round.record()` は **GO でも WAIT でも 1行** 書きます
    （`scripts/next_round.py` の `ROUNDS` の上の註・理由は「周が立たなかった時間の 3つ を
    見分けるため」で、正しい変更です）。**`_laps_between()` の側は直っていません。**

    **実測（2026-09-20 02:1x・`data/rounds.jsonl` を `round` で畳み、`data/model_choice.jsonl` と
    同じ窓で突き合わせた・API 0単位）**::

        直近 1日   畳んだ周 **37**   立てたサブ **11**   比 **0.30**
        直近 2日   畳んだ周  56       立てたサブ  22       比  0.39
        直近 3日   畳んだ周  68       立てたサブ  34       比  0.50
        直近 7日   畳んだ周 135       立てたサブ 151       比  1.12   ← 1周 2役 だった頃が混ざる

    ＝ **`births` が数えているのは「親が起きた回」**で、いま **サブ 1体 あたり 約3.4回** 在ります。
    **`tests/test_quota_fable_cost_per_sub.py` の「1周に 0.41体 ＝ 死んだ台帳」は、これのことです**
    —— 台帳は死んでいません（`model_choice.jsonl` は この周の行まで生きている）。
    **分母の単位が「サブ」から「起きた回」へ ずれただけ**です。

    **この回は式を 1つ も変えていません。** 理由は 2つ:
     (a) `per_lap` と `floor_min` が**同じ単位**なら輪は閉じます。`floor_min` が
         `decide()` の `passed >= floor` で **GO から GO** に当てられている以上、
         **単位が揃っているかは実測でしか言えません**（`forward_rate` に cap と carry が在り、
         それを読まずに 3.4 を掛けるのは「推測の数を測った数として残す」側です）。
     (b) 実測の GO から GO は **直近1日で 11体 / 24時間 ＝ 平均 131分**、
         同じ刻に親が印字していた床は **100分** ＝ **床は破られていません**
    ...
    **次に来た側が撃つ 1つ**（これで決まります・API 0単位）::
        ...
        1) `pace()` の `per_lap` を、分母 `births`（起きた回）と
           `_subs_from_choices()`（立てたサブ）の両方で出す
        2) その 2つ の `floor_min` を、実測の **GO から GO の区間**（`model_choice.jsonl` の差）と比べる
        3) 実測に合うほうが分母。**合わなければ `forward_rate` の cap と carry の側**

    **覆る条件**: (1) `next_round.record()` が WAIT を書かなくなったら、この段はまるごと消えます。
    (2) 1周 に 2体 以上 へ戻したら、比 1.12 の側（7日）が正しい顔になります ＝ 数え直すこと。
    (3) **GO から GO が床を 2周 続けて下回ったら**、そのとき初めて分母は壊れています ＝ (a) の輪が開いた。
    derivation は `docs/JOURNAL.md` 2026-09-20 02:1x。
    """
    seen = {}
    for r in rows:
...
def _choices_live_since(start: datetime, end: datetime) -> tuple[datetime, float, int]:
    ...
    **覆る条件**: (1) `record_model_choice` が落ちない口になったら（穴が出なくなったら）、
    ...
    (3) **穴の理由はまだ読めていません** —— 09/18 の周の印は
    「Fable は月の上限で断られるので opus」と書いており、`sub_model()` の例外の枝が
    台帳を飛ばした疑いが在りますが、**数えただけで、撃って確かめていません。**
    """
...
def short_words(reach_floor: float | None, reach_carry: float | None,
    ...
    if v["blind"]:
        # **この註にも §5 の指し先を置くこと**（2026-09-11 17:0x）—— 「短く終わる」に
        # 触れる字は、どれも §5 の決めを指すのが決まり（`tests/test_spawn_quota_block.py`
        # の `test_短く終わるかは_METHOD_5_の決めを指すこと`）。この行は
        # 親の【枠】の段にもそのまま載るので、指し先が無いと**そこだけ宙に浮きます**。
        line += ("\n      ＊**「いまの間隔のまま」では読まないこと**（§5 13:1x の決め）—— 区間の窓 "
...
def ceiling_rate(per_lap: float, lag_min: float = 0.0,
    ...
    **【2026-09-11 08:5x・optimizer・Opus】(1) の掃きを、切る前に 1度 やっておきました。
    切った周は、これと同じ数が出るなら掃き直さなくてよい（読むだけ）。**
    掃いた盤は 23:5x と同じ形（`per_lap` 0.15〜2.0 × 遅れ 0〜40分 の **144点**）で、
    **切る刻の状態**（`used` 80.9 / 残り 20.8時間）と **Fable が尽きる刻**（81.5 / 19.9）:

        いまの実際の組（`per_lap` 0.552・遅れ 1.74分）   **99.7%**（両方の刻とも・いまと同じ）
        `per_lap` 0.552 の行は 遅れ 0〜5分 で **99.7% 平ら**・遅れ 10分 で 99.1%
        余裕 1.3倍 まで落としても、`per_lap` ≦ 1.1 なら着地は **99.3% 以上**
        崩れるのは 余裕 **1.0倍 の下**（`per_lap` 0.3・遅れ 10分 で 96.2%）

    **そして、この掃きは (1) の言い方をひとつ狭めます —— 余裕は着地を順序づけません。**
    余裕 **3.06〜3.38倍** なのに着地 **97.2〜97.5%** の点が在り（`per_lap` 1.66〜2.0・遅れ 25〜33分）、
    余裕 1.3倍 で 99.3% の点（`per_lap` 0.5・遅れ 5分）より**悪い**。
    ＝ **着地を決めているのは `per_lap` の重さのほうで、余裕はその代理**です。
    代理が効くのは `per_lap` が軽い側（いまの 0.552）に居るあいだだけ。
    ＝ **いまの `per_lap` のまま 3.0倍 を切っても、それだけでは着地を疑う理由になりません。**

    **覆る条件**: (1) **余裕が 3.0倍 を切り、かつ `per_lap` が 1.0% を越えていたら、
