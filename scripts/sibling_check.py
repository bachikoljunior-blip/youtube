...
#:
#: ### **【2026-09-04】この線は、固定その4 の下では毎周 必ず当たります**
#:
#: `runway_days()` は「予約が何日先まで埋まっているか」です。**固定その4
#: （「現在の日付にしか予約しないってことだからね？」）の下では、その値は
#: 定義上 ≤1日**で、しかも**それが正しい状態**です（`CLAUDE.md` 固定その4:
#: 「先の日付が空であることが、正しい状態です」）。
#:
#: ＝ **`runway < 14日` は毎周 真**になり、**間隔の下限は毎周 外れます。**
#: オーナーの「今までの最高速度の**二分の一**の速度でやって」（2026-09-02 18:4x）を
#: 毎周 数え直している `quota.effective_floor_minutes()`（この回は 101分）は、
#: **印字されるだけで、1度も効いていませんでした。**
#: 実測 09/04 07:0x の `--phase spawn`:
#:
...
#:
#: **線の意味を、固定その4 に合わせて置き直しました** —— 危ないのは
#: 「先が空なこと」ではなく、**「きょうの枠がまだ空なこと」**です
#: （`today_slot_empty()`）。きょうの本が置かれていれば、その日の公開は続きます。
#: 置かれていなければ、**その日が丸ごと落ちます** —— そこだけが下限より重い。
#:
#: **覆る条件**: オーナーが「先の日付にも置いてよい」と言ったら、
#: `runway_days()` の側へ戻すこと（そのときは在庫が本当に在庫を意味します）。
...
def today_slot_empty(now: datetime | None = None) -> bool | None:
    """**きょうの枠が、まだ空か**（`True` ＝ きょうの本が1本も置かれていない）。読めなければ `None`。

    固定その4 の下で「在庫が尽きた」に当たるのは、**この1つだけ**です
    （`RUNWAY_FLOOR_DAYS` の註）。先の日付が空なのは正しい状態で、危険ではありません。
    **きょうの枠が空のまま日が暮れると、その日が丸ごと落ちます。**
...
def main() -> int:
    ...
    if args.phase == "spawn":
        ...
        #
        # **ただし「在庫が薄い」の測り方は、固定その4 で変わりました**（2026-09-04・
        #     `RUNWAY_FLOOR_DAYS` の註）。`runway_days()` は固定その4 の下では
        #     **定義上 ≤1日**で、しかもそれが正しい状態 ＝ **毎周 必ず下限が外れる。**
        #     オーナーの「二分の一の速度」を毎周 数え直している `effective_floor_minutes()` が、
        #     **印字されるだけで1度も効いていませんでした。**
        #     危ないのは「先が空なこと」ではなく **「きょうの枠がまだ空なこと」**です。
        empty = today_slot_empty(now)
        runway = runway_days(now)
        if runway is not None:
            print(f"在庫: 予約は **{runway:.1f}日先**まで"
                  "（固定その4 の下では ≤1日 が**正しい状態**。ここでは下限を外しません）")
        if empty is None:
            print("きょうの枠: **読めませんでした**（`src/next_slot.py`）。下限はそのまま効かせます")
        elif empty and floor:
            print(f"きょうの枠: **まだ空です** → **間隔の下限 {floor:.0f}分を外します。**"
                  "きょうの本が置かれないまま日が暮れると、その日が丸ごと落ちる。")
            floor = None
        else:
            print("きょうの枠: **埋まっています**（きょうの公開は続きます）"
                  f"→ 間隔の下限{f' {floor:.0f}分' if floor else ''}はそのまま効かせます")
