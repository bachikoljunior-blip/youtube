...
def wake_latency_minutes(rows: list[dict] | None = None,
    ...
    **覆る条件**: 遅れの中央値が 1分 を切ったら（＝ 起こしが頼んだとおり届くようになったら）、
    ...
    ~~逆に、周から周の中央値が `floor` を**下回る**回が続いたら、中央値ではなく 25% 点へ。~~

    **【2026-09-09 20:5x・optimizer・Opus。この後半が引かれ、直し方は 25% 点ではありませんでした】**

    実測（周から周 ÷ その周の GO の `floor_min`。直しの入った 10:05 UTC 以降の 3周）:

        10:05  54.1分 / 床 52.87  **+1.26**
        10:57  52.3分 / 床 52.87  **-0.58**   ← 下回った
        11:49  51.9分 / 床 52.89  **-0.98**   ← 下回った（2回 続けて・中央値 **-0.58**）

    ＝ 条件は引かれました。**ただし 25% 点（実測 1.45分）へ落としても、向きは直りません** ——
    定数が半分になるだけで、**下へ寄る形そのものが残ります**。原因は大きさではなく、
    **同じ遅れを 2度 引いていたこと**でした:

        (1) 起こしを `lat` 手前に置く   → 届く時刻をこちらで選べる ＝ **量子化が消える**
        (2) 狙い先を `floor - lat/2` に → 量子化が在る前提の**境目**を、狙い先に使っている

    (2) の `floor - lat/2` が正しいのは「**もう届いてしまった** `passed` を、いま出すか
    次の届きまで待つか」の場面だけです。**どこへ届かせるかを選べる場面では、狙い先は `floor`。**
    予測: 届く時刻は `floor + (実際の遅れ − 中央値)` に**中心が乗る**（両側・中央 0）ので、
    ...
    **【2026-09-12 09:4x・optimizer・Opus】その前提を見張る検査が、初めて落ちました。
    ただし落としたのは親ではなく、検査の「何回 届いたか」の数え方です。**
    実物（狙い先 2026-09-12 00:04Z を 23:43:50 と 00:00:04 の 2行 が指した）: 届いたとされた
    2行 は **00:05:46 の GO** と **その 25秒 後の 00:06:11 の WAIT**（`前の周の開始から 0分`）。
    **25秒 は `WAKE_SAME_SEC`（120秒 ＝ この file が置いている「同じ届きと見なす幅」）の中**で、
    上の 04:5x が既に名指ししている「**起こしと心拍の両方で起きる**」側そのものです。
    ＝ **検査だけが、親と違う幅で「届き」を数えていました**（門が 2か所・§5「門は 1か所」）。
    `tests/test_parent_wake_placed._deliveries()` が `WAKE_SAME_SEC` で畳むようにして、
    **幅の外の 2回目なら落ちる**ことを陽性対照で撃ってあります。
    ＝ **(3) はまだ取り消したままでよい。書き直す回は、幅の外の 2回目が出た回です。**
    derivation は JOURNAL 2026-09-12 09:4x。
...
def wake_missed(rows: list[dict] | None = None,
    ...
    **覆る条件**: (1) 15分 超が **置いた起こしの 20% を越えたら**（いま 4%）、
    ...
    (3) 心拍の**中央値**（`heartbeat_minutes`・いま 18.3分）が 30分 を越えたら、
    頭打ちが 2倍 になるので (1) の 20% を下げ直すこと。**最大の側（39.6分）で読まないこと**
    —— そちらは 1区間 でも外れ値で動きます。
    """
    got = rows if rows is not None else wake_rows()
...
def gap_ratios(limit: int = 10, got: list[dict] | None = None) -> list[float]:
    ...
    **§7 の覆る条件 (1) は「中央値が 1.25倍 を越えたら上限は丸めではない」**なので、
    ...
    （20:5x に `decide()` が書き始めた欄）。無い区間は
    **これまでどおり GO の `floor_min`** で割る ＝ **過去の数は 1つも動きません**

    **【2026-09-12 05:3x】「無い区間 ＝ `aim_min` より前の台帳」は偽でした**（optimizer・Opus）。
    `decide()` はこの欄を **`if idle:`（0体・起こしを置く枝）の中だけ**で書いており、
    `live >= 1` の WAIT は **38行 とも空**。**落ちていた区間 39/110 のうち 17 は
    `aim_min` が在る時代のもの**で、**§7 (d) の門を越えた窓 7つ は 7つ とも その側**でした
    ＝ **門が読む窓だけが、この 22:4x の直しを通らずに GO の床で割られていた。**
    いまは (1) `decide()` が両方の枝で書き、(2) 下の `aims` が **空欄の WAIT 行の `floor_min`**
    （＝ `aim = floor` と同じ数）を拾います。derivation と覆る条件は JOURNAL 05:3x・
    検査 `tests/test_next_round_aim_on_live_wait.py`
...
#:
#: **【2026-09-19 01:xx・optimizer・opus】その「穴」は閉じました。定数は動かしていません。**
#: `tests/test_next_round_beat_only.py` の 2件 が赤になったので撃ち直した実測
#: （窓を振って `landed` の gap を並べた・**API 0単位**）:
#:
#:     窓  8.0分   landed 197本   最大 **7.9分**   7.9 より上 **0本** ← 上の註が書かれた頃の姿
#:     窓 10.0分   landed 206本   最大 9.81分     8.0・8.09・8.11・8.74・9.07・9.33・9.77・9.79・9.81
#:     窓 16.0分   landed 216本   最大 15.74分    さらに 10.34〜15.74 が 11本
#:     窓 30.0分   landed 245本   最大 29.97分    **切れ目なし**
#:     窓 45.0分   landed 260本   最大 44.43分    **切れ目なし**
#:
#: ＝ **`7.9` の先の穴（この 8.0 を置いた根拠そのもの）はもう在りません。**
#: **それでも 8.0 を動かしませんでした。** 動かすなら「穴の中」へ置きたいのに、
#: **8分 から 44分 まで切れ目が 1つも無い** ＝ どこへ置いても**未測の帯の中段を選ぶ**ことになります
#: （2026-09-18 23:0x の回が `perf_need_rate` で踏んだのと同じ形）。
#: **しかも この数はまだ「遅い届き」と「無関係な次の起き」を分けられていません** ——
#: 上の数え方は、置いた起こしを**次に来た起き**へ貪欲に当てるので、窓を広げれば必ず増えます。
#: **＝ 分けられる測り方（`wake_at` と起きの id を突き合わせる側）を先に作ること。**
#:
#: **いま 8.0 のままで何が起きているか**: 8分 を越えて届いた起こしは `wake_missed` に数えられ、
#: **その起きは「新しい心拍」として数えられます** ＝ 心拍の数が実際より多く出ます。
#: **これは親の間隔の側に効きます**（`beat_only_gaps` を読む所）。**まだ値打ちを測っていません。**
#: **申し送りは `docs/JOURNAL.md` 2026-09-19 §3-c。**
WAKE_LAND_MIN = 8.0
...
def decide(now: datetime | None = None, live: int | None = None,
    ...
    **検査は `tests/test_next_round_live.py`。戻すには検査を消すしかありません。**
    ...
    if idle:
        ...
        # 「頼んだ時刻 ＋ 遅れ」なので、`aim` に届かせるには `aim - 遅れ` に頼む。
        # **`wake_at` は、実際に頼む分（`wake_min`）から作ります** —— 前の形は
        # `floor + 1.0` を別に組み立てており、送った起こしと台帳が最大 1分 ずれていました
        # （`send_later` は分の粒度なので、ずれる側は必ず「遅い側」）。
        #
        # **狙う先（`aim`）は `target` ではなく `floor` です**（2026-09-09 20:5x・optimizer・Opus）。
        # 18:2x は `wait = target - passed` のまま `- lat` していたので、**同じ遅れを 2度 引いて**
        # いました: 起こしを `lat` 手前に置けば届く時刻はこちらで選べる（量子化が消える）のに、
