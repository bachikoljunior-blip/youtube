...
#:
#: **[!] 「480/600/720 が同じ ×2.00 だから長尺は 480時間で平ら」は成り立ちません**
#: （2026-08-31・第2の回に撃って取り消した。**入れた回の註が、そう書いていました**）。
#:
#: `_nearest` の許容は `hours*0.35+12` で、**地平の間隔より広い**。だから
#: **最後の読みで終わる本は、3つの地平が同じ1点を読みます。**
#: 実測（各地平で、比の相手として実際に読んだ齢）::
#:
#:     t=336h  n=12  [246,246,247,247,248,249,249,330,334,335,336,336]
#:     t=480h  n= 5  [478,479,480,480,502]
#:     t=600h  n= 5  [481,505,529,577,599]
#:     t=720h  n= 5  [481,505,529,577,649]   ← **5本のうち 4本 が 600h と同じ点**
#:
#: **同じ点を3回 読めば、答えは3回とも同じです。** それは平らさの証拠ではありません。
#: `t=336h` はもっと悪く、**12本のうち 7本 が起点 246h と同じ点**を読んでいます
#: （＝ 比が定義上 ×1.00）。
#:
#: **逆を言っている道具が同じ repo に在ります**: `src.settle.settles_at('長尺')` は
#: 「**地平を延ばすと、伸びきる年齢が消える**」（480h で n=5・該当 0本）。
#: **そちらが正しい** —— あちらは許容を使わず、地平ごとに当て直しています。
#: **長尺は 480時間 でも平らになっていません。**
#:
#: つまり `censor_factor` が返す倍率は、**打ち切りの下限**でもあります
#: （まだ伸びている所で切った比なので）。`settle.SETTLE_HORIZONS` が 480 で
#: 終わっているのは変わらず本当で、そこは延ばす余地があります。
...
def censor_factor(form: str, *, views_path: Path | None = None,
    ...
    **[!] その ×2.00 を「実測に近い」と読まないこと**（2026-08-31・第2の回に取り消した）。
    **比の分母は [1, 1, 2, 3, 4] 再生で、掛ける先の記録は 156再生**です。
    中身は `1→2`・`2→4`・`4→8`・`3→8`・`1→4` —— **伸びではなく1桁の整数の刻み**で、
    **1再生 増えるだけで ×2.00 になります。** 同じ表のショートは分母
    **[312 … 1,741] 再生**で ×1.0000（最大 ×1.018）—— **あちらは記録と同じ桁、
    こちらは 2桁 下。同じ数として読めません。**

    だから `gaps()` の隔たりは **点ではなく帯**です ——
    **×10.7（補正した側）〜 ×21.4（補正しない側）。**
    `noisy` が真のときは `scripts/eta.py` がその帯を印字します。
    **どちらの端も、その帯を選ぶ理由にはなりません。**

    **それでも 1.0 に落とさないこと**: 長尺が伸び続けるのは別に実測できています
    （同じ本を追った比で **168h→480h 中央値 ×2.67**・n=5）。
    誤っていたのは倍率の**向き**ではなく、**測れていないのに点で印字したこと**です。
