...
#:
#: **`tests/test_reserve_hold_same_yardstick.py` は、いまも緑のままです。**
#: あの検査が守っているのは自分でこう書いています ——「**読みを数えるな**では
#: ありません。守っているのは**順番**です」。こちらは `floor`／`spent` の
#: 引き算に1文字も触らないので、順番を踏んでいません。
#: **あの検査を消さないこと** —— 下の門は今までどおり残っており、
#: 物差しが混ざる日が来たら、あちらが先に落ちて教えます。
#:
#: ## 覆る条件
#:
...
#: - **値段**: 1回 **7.2ms**（実測 2026-09-01・帳面 2,479行／窓の中 2,249行）。
#:   書き込みのたびに帳面を読み直します。**網の1往復（100〜500ms）の 2% 未満**なので
#:   いまは黙って払ってよい額です。`pool_drain` の 190本 で **1.4秒**。
