...
def day_quota(now: datetime | None = None) -> DayQuota:
    ...
    **読めなかったときは「開いている」と答えます。** 読めないことを
    ...
    # **窓が変わった直後の 403 は、前の窓の尻尾です**（2026-08-26・最適化の回に足した）。
    #
    # ここは「この窓で 403 を1回でも見たら、残り23時間ぜんぶ閉じる」でした。
    # **開け直す道がありません。** そして 403 で閉じる側は、**窓が開いた直後の
    # 1回でも latch します** —— Google 側の戻りには数分〜十数分の遅れがあり、
    # そこを踏んだ回が、**まだ丸ごと残っている窓を24時間 閉めてしまいます。**
    #
    # 実測（`data/day_quota.jsonl` 2946件・9つの窓。**窓が開いてから最初の403まで**）:
    #
    #     0.1h  0.2h │ 6.6h  7.4h  7.6h  8.1h  10.6h  12.2h  14.4h
    #     ↑2件        │ ↑7件
    #     **6.4時間の隔たりで、きれいに2つに割れています。**
    #
    # 左の2件（08/19 07:08 と 08/26 07:12）は、どちらも**直前の窓が
    # 重く尽きていた日**です（403 を 557回・706回 観測）。**新しい窓が
    # 12分で尽きた**と読むより、**前の窓の会計がまだ残っていた**と読むほうが
    # 実測に合います —— 実際 08/26 は、その窓で `videos.insert` が **0本**でした
    # （何も使っていない窓が尽きることはありません）。
    #
    # **試すのはタダです。** 403 は単位を消費しません（消費するのは通った呼び出し
    # だけ）。だから「開いていると思って撃って外す」損は失敗した1回ぶんの時間だけ、
    # 「閉じていると思って撃たない」損は**23.7時間ぶんの `videos.update` 全部**です。
    # **賭けが非対称なので、開ける側へ倒します。**
    #
    # **覆る条件**: 窓が開いて `GRACE_MIN` 以内の 403 のあとに、同じ窓で
    # **`GRACE_MIN` より後の 403 が続けて出る**なら、その窓は本当に尽きています ——
    # そのときは下の `late` が拾うので、ここは自動で閉じます。
    # 逆に「窓が開いた直後の403」だけで閉じていた回が無くなったら、この節は消してよい。
    head = window_start(now)
    late = [h for h in hits
            if (_parse(h.get("at")) or head) >= head + timedelta(minutes=GRACE_MIN)]
    if hits and not late:
        mins = (now - head).total_seconds() / 60
        return DayQuota(True, False, len(hits), tail,
                        f"日枠（単位）: この窓の 403 は **{len(hits)}回 とも"
                        f"窓が開いて {GRACE_MIN}分 以内**でした"
                        f"（前の窓の尻尾の可能性が高い。いま窓が開いて {mins:.0f}分）。"
                        " **閉じたと決めません** —— 403 は単位を使わないので、"
                        "撃って外れてもその1回ぶんだけです。")
    hits = late or hits
