...
# **行頭に置いてはいけない字**（行頭禁則。2026-08-15 に4字から広げた）。
#
# 実物で `給与収入のみ` ／ `・扶養なし` と折れました。中黒が行頭に来ると、
# **前の項目の続きではなく、新しい項目のように見えます。**
#
# 入れたのは「字の種類として単独で行を始められないもの」だけです:
#   句読点・区切り  、。，．・：；
#   閉じ括弧        ）」』】〕｝］》〉”’"'
#   感嘆・疑問      ！？!?
#   繰り返し・長音  ーゝゞ々〃
#   小書きのかな    ぁぃぅぇぉっゃゅょゎァィゥェォッャュョヮヵヶ
#   数につく記号    ％%℃‰
#
# **開き括弧（「（『【 など）は入れません。** あれが禁じられているのは
# **行末**であって行頭ではなく、混ぜると逆に「（」が前の行の末尾へ吸われます。
NO_LINE_START = (
    "、。，．・：；"
    "）」』】〕｝］》〉”’\"'"
    "！？!?"
...
# **行末に置いてはいけない字**（行末禁則。上の鏡像）。
# 同じ列で出ました —— `課税所得は` ／ `195万円以下（` ／ `税率5パーセント）`。
# **開き括弧が行末にぶら下がると、次の行と繋がって見えません。**
#
# **こちらは limit を見ます。上の行頭禁則とは判断が逆です。**
# 行頭禁則は字を**前へ**戻すので、伸びるのは既に確定した行のほうで、
# 図解側の `render()` が焼き直すときに吸収されます。
# 行末禁則は字を**後ろへ**送るので、**まだ limit いっぱいかもしれない行が伸びます。**
# 溢れるくらいなら、ぶら下がったまま残すほうがましです
# （下の「短すぎる余りを戻す」処理が長さを見ているのと同じ理由）。
NO_LINE_END = "（「『【〔｛［《〈“‘"

# **数のかたまりから、単位の1字だけがちぎれる**（2026-08-16 に実測して足した）。
# 実物 `E5i0YfIkmnA` の `全部配偶者 9589万3334円` が
# `9589万3334` ／ `円` と折れています（目視でも `年78.0万` ／ `円`）。
#
# `9589万3334` は**どこでも割れない数のかたまりで、ちょうど limit に等しい**ので、
# 「短すぎる行を隣に寄せる」も「短すぎる余りを戻す」も、**長さで断ります。**
# **`> limit` ではなく `== limit` なので、「もう溢れているなら断らない」では効きません**
# （前の回がそう書いて、動かないことを実測しています）。
#
# `％` と `℃` は `NO_LINE_START` に入っていますが、**`円` `人` `日` `年` は
# 普通の語としても出る**ので（「人が」「日に」「年の」）、無条件に足すと
# 関係のない行から字を奪います。**だから前の行が数で終わっているときだけ**戻します。
UNIT_AFTER_NUMBER = "円人日月年歳件回倍週杯枚"
# 数のかたまりの末尾になりうる字。半角・全角の数字と、位取りの漢数字。
NUMBER_TAIL = "0123456789０１２３４５６７８９万億兆千百.．"
...
_NUM_TOKEN = set("0123456789．."
                 ...
                 "条項号"
                 # **桁区切りのカンマ**。2026-08-16、実物 kLJ2Wsi3gQM の
                 # 箇条書きが **『上限額 9,』／『110円→7,830円』** と割れました
                 # （目視で発見。**数の真ん中で切れているので、もう数に読めません**）。
                 #
                 # 小数点（`．.`）は最初から入っているのに、**桁区切りだけ
                 # 抜けていた**。どちらも「数字のあいだに挟まる、数字でない字」で、
                 # 同じ理由で切ってはいけません。**数のかたまりは数字だけで
                 # できていない**、の5回目（元号・小数点・助数詞・カタカナの単位）。
                 #
                 # 全角の `，` も入れます（`NO_LINE_START` にはありますが、
                 # あれは**行頭**の規則で、割れ目は `，` の**後ろ**にできるので
                 # 一度も効きません）。
                 ",，")
    ...
    def ok(cut: int, lenient: bool = False) -> bool:
        ...
        **順位を下げるのと、禁じるのは別です。**
        ...
        if piece[cut - 1] in "かヶケ箇" and piece[cut] in "月年国所":
            return False
        # **2字でひとつの単位の途中でも割らない**（2026-08-16、実物の画面で見つけた）。
        #
        # `s-jikangai-futsu-6kagetsu-126` の注記が
        # **『45時』／『間以内で』** と割れました（目視で発見。**「45時」で
        # 行が終わるので、もう時間に読めません**）。
        #
        # なぜ上の規則を全部すり抜けたか。`時` は `_NUM_TOKEN` にあるが
        # **`間` は無い**ので「両側とも数字」に当たらない。漢字の連続を見る規則も、
        # 手前が数字（`45時`）なので **`num_end` の逃げ道**で通ります ——
        # あれは「千円／戻ります」を許すために置いたもので、
        # **`時` が単位の終わりだと決めつけていました。** `時間` は2字です。
        #
        # 実データ 497本・38,938組で **96組**（全部この形）。
        # 塞いだ結果、数のかたまりを割る形は1件も増えていません（下の実測）。
        if (piece[cut - 1] in "時年月日週秒" and piece[cut] == "間"
                and cut >= 2 and piece[cut - 2] in _NUM_TOKEN):
            return False
...
def _chunk(narration: str, limit: int = MAX_LINE_CHARS) -> list[str]:
    ...
    # 読点が次の行の先頭に来ると読みにくい。
    #
    # **2026-08-15 まで、ここは "、。！？" の4字しか見ていませんでした。**
    # 実物で `給与収入のみ` ／ `・扶養なし` と折れています（注記の前置き）。
    # **中黒は区切りなので、行頭に来ると「・扶養なし」という別の項目に見えます。**
    # 同じ関数が字幕・見出し・箇条書き・注記・ラベルの**全部**を折っているので
    # （`src/visuals.py` の `_wrap` が `_chunk` に委ねている）、
    # **1か所直せば画面の全部に効きます。**
    #
    # 足したのは、字の種類として**単独で行を始められないもの**だけです
    # （JIS X 4051 の行頭禁則のうち、この題材に出るもの）。
    # 開き括弧「（」は行頭に来てよいので入れません（あれは**行末**禁則）。
    #
    ...
    # **数から単位の1字だけがちぎれた行を、前へ戻す**（2026-08-16）。
    # 条件と理由は `UNIT_AFTER_NUMBER` の注記に書いてあります。
    #
    # **ここも limit を見ません。意図的です。** 下の行頭禁則と同じで、
    # 字を**前へ**戻すので伸びるのは既に確定した行のほうで、
    # 図解側は `render()` が文字数を詰めて焼き直すときに吸収します。
    # **単位が1字だけ映るほうが、1字ぶん溢れるより悪い。**
    #
    # **戻すのは、その行が単位と行頭禁則の字だけでできているときだけ**です
    # （`円` と `円。`）。`円です` のような続きのある行には触りません ——
    # そちらは単位が孤立しているのではなく、ただの文の続きなので、
    # 戻すと今度は `です` が取り残されます。
    # **この pass は行頭禁則より前に置くこと。** `円。` を割った残りの `。` は、
    # 下の行頭禁則がそのまま前へ戻します。
    pulled: list[str] = []
    for line in lines:
        if (pulled and line and line[0] in UNIT_AFTER_NUMBER
                and pulled[-1][-1:] in NUMBER_TAIL
                and all(c in UNIT_AFTER_NUMBER + NO_LINE_START for c in line)):
            pulled[-1] += line[0]
            line = line[1:]
        if line:
            pulled.append(line)
    ...
    fixed: list[str] = []
    for line in lines:
        # 空文字はどんな文字列の部分文字列にもなるので、必ず長さを見ること
        while fixed and line and line[0] in NO_LINE_START:
            fixed[-1] += line[0]
            line = line[1:]
        if line:
            fixed.append(line)
    ...
    # 行末にぶら下がった開き括弧を、次の行の頭へ送る。
    # **次の行が溢れるなら送らない**（上の注記の理由）。
    for i in range(len(lines) - 1):
        while (len(lines[i]) > 1 and lines[i][-1] in NO_LINE_END
               and len(lines[i + 1]) + 1 <= limit):
