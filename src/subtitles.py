...
def _best_cut(piece: str, limit: int, strict: bool = False) -> int:
    ...
    **ひらがなの次に漢字・カタカナ・数字が来る位置は、ほぼ語の頭**になる。
    ...
    for cut in range(floor - 1, 1, -1):
        if ok(cut, True):
            return cut
    # **それでも無いなら、限度の外へ出る**（2026-08-16 23:xx。**在庫が1件、
    # ここで詰まって2回とも作れませんでした**）。
    #
    # `s-zaishoku-1man-hanbun` は、台本を書き直しても**2回とも同じ所で落ちています**
    # —— `月給410,000円から440,000円へ` が限度6で
    # **『410,00』／『0円から』**。偶然ではありません。
    #
    # **1つの数のかたまり（9字）が、限度（6字）より長いからです。**
    # 前を向いて探すかぎり、`2`〜`limit` のどこを切っても数の途中で、
    # 上の行は全部 `ok()` に弾かれ、**最後の `return limit` だけが `ok()` を
    # 見ずに割ります。** `_UNIT_HEAD` の註が「逃げ場が無いときの最後の1行は
    # 規則を踏み越える」と書いているのは、まさにこの1行のことです。
    #
    # **短い行を作る道はもう尽きているので、残るのは長い行を作る道だけ。**
    # 数のかたまりの**外**まで出てから切ります。`410,000円` は9字で限度を
    # 3字はみ出しますが、**はみ出した行は読めます。割れた数は読めません。**
    #
    # **掛けるのは「限度で切ると数が割れるとき」だけ**にしてあります
    ...
    if limit < len(piece) and piece[limit - 1] in _NUM_TOKEN and piece[limit].isdigit():
        for cut in range(limit + 1, len(piece)):
            if ok(cut, True):
                return cut
        return len(piece)      # かたまりが行末まで続くなら、割らずに1行で出す
    return limit
