...
if __name__ == "__main__":
    ...
    # **住民税決定通知書を開けて、ふるさと納税が本当に引かれたかを読む節**（2026-09-05 13:3x）。
    #     外の帯のショート4位「How to check your hometown tax donation deductions.」
    #     （76,726回・99秒・`data/niche_corpus.jsonl`）の型 ＝ 届いた通知書の摘要欄を読む。
    #     **通知書に載るのは住民税の2階ぶんだけ**（基本分＋特例分）。所得税ぶんは
    #     確定申告なら還付で別に戻り、ワンストップ特例なら「申告特例控除」として住民税へ移る
    #     （上限の内側では申告特例控除 ＝ 所得税ぶん。式: 特例分 × r×1.021 ÷ (90% − r×1.021)）。
    #     **足し算が桁ごと合うように、各階を先に円で丸めてから足す**（`numbers_backed` は
    #     台本の数がこの印字に在るかを見るので、合計もここに刷る）。
    print("\n=== 住民税決定通知書の摘要欄に載る額（上限ちょうど寄付したとき）===")
    print("  通知書の「寄附金税額控除」の欄に載るのは**住民税の2階ぶんだけ**（基本分＋特例分）。")
    print("  所得税ぶんは、確定申告なら還付で別に戻り、ワンストップ特例なら住民税へ移ります（申告特例控除）。")
    print("   年収     寄付額(上限)   所得税ぶん   住民税基本   住民税特例   通知書(確定申告)   通知書(ワンストップ)   寄付額−2,000円")
    for _inc in (4_000_000, 6_000_000, 8_000_000):
        _pp = Person(income=_inc, social_rate=TYPICAL_SOCIAL_RATE)
        _oo = out_of_pocket(_pp, limit(_pp))
        _rb = int(round(_oo["住民税・基本分"]))
        _rs = int(round(_oo["住民税・特例分"]))
        # 上限の内側では3階の和 ＝ 寄付額−2,000円。**所得税ぶんは残りで取る**（各階を円で丸めたあと、
        # 足し算が 1円 も外れないように。`所得税から戻る` の丸めと最大 1円 ずれます）。
        _it = (_oo["寄付額"] - SELF_PAY) - _rb - _rs
        _notice_final = _rb + _rs
        _notice_onestop = _rb + _rs + _it
        print(f"  {_inc // 10_000:>5,}万  {_oo['寄付額']:>11,}円  {_it:>9,}円  {_rb:>9,}円  {_rs:>9,}円"
              f"  {_notice_final:>13,}円  {_notice_onestop:>16,}円  {_oo['寄付額'] - SELF_PAY:>11,}円")
    print("  **確定申告した人の通知書は、寄付額−2,000円 より所得税ぶんだけ小さく載ります。足りないのではありません。**")
    print("  ワンストップ特例の人は所得税ぶんも住民税へ移るので、通知書の額 ＝ 寄付額−2,000円 になります。")
    print("  どちらでもない（額が 0円・欄が無い）なら、申告が通っていません —— 確定申告（5年さかのぼれる）で取り戻せます。")
