...
def conserves(total: float, parts: Sequence[float], what: str,
              *, tol: float = 1.0, names: Sequence[str] | None = None) -> None:
    """**出ていった総額が、その内訳の合計と一致すること（保存則）。**

    2026-08-31 に足しました。**この形の検査が1つも無かったせいで、
    誤った金額が動画になりかけています。**

    `src/calc/hendo.py`（変動金利の未払利息）の `check_tables()` には
    検査が8件ありましたが、**8件とも「向き」しか見ていませんでした** ——
    上がった先が高いほど増えるか、遅く上がるほど減るか、行が重なっていないか。
    **向きの検査は、全部の行が同じ倍率でずれても通ります。**

    実際に通り抜けたずれ（3,500万円・35年・0.5%→4.0%）:

        払った総額   70,108,284円
        元金 ＋ 利息  68,783,950円
        差          **+1,324,334円**   ← 対応する債務がどこにも無い

    原因は、5年ごとの見直しが未払利息を月々へ割り付けたうえで、
    最終回の一括でも満額 請求していたこと（**同じものを2回**）。
    **向きは全部 正しいまま**でした。

    ## 使いどころ

    「**回ごとに積む**」計算 —— ローンの返済表、年金の受給、手取りの内訳、
    分割の按分。**払った側と、受け取った側の内訳**が別々に出るなら、
    ここで突き合わせること。

        _checks.conserves(got["total"], [元金, 利息の合計], "総支払額",
                          names=["元金", "利息"])

    `tol` の既定が 1円 なのは、利息を切り捨て・返済額を切り上げる計算だと
    **端数が最終回に寄る**ためです。率や比なら `close()` を使うこと。
    """
    got = sum(parts)
    if abs(total - got) <= tol:
        return
    if names and len(names) == len(parts):
        breakdown = " ＋ ".join(f"{n} {v:,}" for n, v in zip(names, parts))
    else:
        breakdown = " ＋ ".join(f"{v:,}" for v in parts)
    raise TableError(
        f"{what}: {total:,} が、内訳の合計 {breakdown} ＝ {got:,} と "
        f"{total - got:+,} ずれています。"
        "**払った額は、内訳のどれかにしかなりません。**"
        "同じものを2か所に乗せていないか見ること"
    )
...
#: **「年」のうしろに金額が続く書き方で、月の側が金額の頭を食っていました**
#: （2026-08-29 に踏んだ）。`1990年380,153円` の2つ目の枝は `月?` が任意なので
#: **`1990年38` まで食い**、残った `0,153円` を「表に無い数」として鳴らします。
#: 実測で `bunkatsu-2003-noritsu-sen` の angle が 3件（`0,153`・`0,697`・`6,751`）。
#: **どれも実物は表に在ります。** `(?![\d,])` を足して、月のうしろに数字が続くときは
#: この枝を当てないようにしてあります（`1990年` は3つ目の枝が拾います）。
#: **覆る条件**: `2026-08` のようなハイフンの書き方を拾えなくなったら、この行が原因。
#: 検査は `tests/test_doc_numbers.py` の「西暦のうしろに金額が続く」。
_DATE_RE = re.compile(
