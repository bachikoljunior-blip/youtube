...
WINDOWS: tuple[dict[str, str], ...] = (
    {
        ...
        #
        # **2026-08-28 09:2x に 08-27 → 08-28 へ（4件目）。** 支えている前提の期限は
        # **08-28**（`deadline_check` は「判定できるのは 08-28 の 16:00 JST 以降」）。
        # `until` は 08-27 のままだったので、**この窓は 08-28 の 00:00 に外れ**、
        # **答えが読める 16:00 JST の 16時間 前**から守らなくなっていました。
        # 上の註が「この形はまた起きます」と予言したとおりです ——
        # **同じ回に `claim` を足して、検査がその前提の期限そのものを見るようにしました。**
        "until": "2026-08-28",
        "claim": "1日に再生が付く本数の上限は「その日の本数（10本）」であって、時刻の窓ではない",
        "label": "day_cap",
    ...
    # 検査が「どの前提の期限か」まで見ていないので、この形はまた起きます。
    {
        ...
        # **2026-08-25 に、支えている前提の期限が 09-16 → 09-10 へ縮んでいました。**
        # `until` はそのまま 09-16 に残っていて、**この註のすぐ上に書いてある
        # 「期限を動かしたら `until` も一緒に動かすこと」が、また守られなかった**
        # 形です（3件目）。2026-08-26 に検査が落ちて見つけました。
        #
        # **`active()` は `today <= until` なので、09-10 は丸ごと守られ**、
        # 窓が外れるのは 09-11 です（測定日の最後の本は 16:00 JST 公開・
        # 6時間後は 22:00 JST なので、09/10 いっぱい要ります。08-27 の窓と同じ）。
        "until": "2026-09-10",
        "claim": "1日に再生が付く本数の上限（いま10本）は、チャンネルが育っても上がらない",
        "label": "M14",
        "why": ("上限（いま10本）を**上から**測り直す日（16本）。前提"
                "「1日に再生が付く本数の上限は、チャンネルが育っても上がらない」"
                "（期限 2026-09-10）の反証条件は**2026-09-11 までに 11本以上 出した日**"
    ...
    },
    {
        ...
        # 支えている前提は
        # 「engaged 比率は、その日に出した本数が増えると下がる」（期限 **2026-10-02**）。
        # `until` はその期限そのもの（上の註「期限を動かしたら `until` も一緒に」）。
        #
        # **2026-08-28 09:2x に 10-02 → 10-03 へ（5件目・上の 08-27 と同じ回に見つけた）。**
        # 支えている前提の期限は 10-02 から **10-03** へ動いていて、
        # `until` だけが残っていました。**この2件は、どちらも「たまたま通る」の
        # 反対側です** —— 検査は「開いている前提の**どれか**の期限」しか見ないので、
        # 一致しなくなったときだけ落ち、**しかも最初の1件で止まります。**
        # 08-27 の窓が先に落ちていたので、**この 10-02 は隠れていました。**
        "until": "2026-10-03",
        "claim": "engaged 比率は、その日に出した本数が増えると下がる（＝順に並べた窓の A/B は、全部これを測っている）",
        "label": "density_engaged",
        "why": ("**engaged を測る対照日です。1〜2本/日 のまま残すこと。**\n"
                "この2日は、いま予約が **1〜2本/日 しか入っていない**唯一の日で、"
                "`status.py` が毎回「**そこは詰められます**（`reschedule.py --compact`）」"
                "と鳴らしています。**詰めた瞬間に対照が消えます。**\n"
                "なぜ要るか: engaged を的にした前提が3件続けて外れており"
                "（30秒設計・冒頭22文字・実績の順番）、**3件とも「前の窓 対 後の窓」**です。"
                "同じ期間に1日の公開本数が **2本 → 25本** へ動いており、実測では"
                "**1〜2本/日 34.8% 対 9本以上/日 19.4%（片側 p=0.057）**。"
                "**処置群と高密度の日は同じ本**なので、順に並べた窓では分離できません"
                "（`src/density_confound.py`）。\n"
                "**低密度の日を新しく作るには `videos.update` が要ります**（日枠・JST 16:00 以降）。"
                "**この2日は、その手を使わずに残っている最後の対照**です。"),
    },
)
