...
def title_feature_line(next_row: dict | None, form: str = "ショート") -> list[str]:
    ...
    **覆る条件**: `title_features()` の上位が 3周 続けて同じで、しかも
    ...
    if open_:
        ...
        out.append("     [!] **その特徴が台本に無ければ、足さないこと。** 上の名指しは"
                   "「帯で効いている」しか言っていません —— **台本がその話をしていない題は"
                   "『誤解を与える内容』で、収益化されなければ再生が何回でも 0円 です**"
                   "（`CLAUDE.md`「YouTube のポリシーは制約ではなく事実」）。"
                   " 実測 2026-09-05: 09/05 の長尺は `場面`（×32.73・n=66対269）が空いていたが、"
                   "**台本に制度改正が1つも出てこない**ので足さなかった。**空いている＝打てる、ではありません。**")
