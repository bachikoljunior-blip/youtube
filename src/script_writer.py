...
- サムネの2行は「金額の差」と「判断の語」。**どの行も全角11文字以内**（黄色い箱＝ `thumbnail_kicker` も含む・外の上位3本の最大が 11文字・入らないなら前提を落とさず、行を割るか条件を line1 へ移すこと）。
"""
...
#: 入らないなら、行を割るか、条件のほうを `thumbnail_line1` へ移すこと。
#:
    ...
    for _name, _txt in (("thumbnail_kicker", _get("thumbnail_kicker")),
        ...
        if _txt and len(_txt) > OUTSIDE_THUMB_LINE_MAX:
            problems.append(
                ...
                "**前提を落として縮めないこと** —— 行を割るか、条件を line1 へ移すこと")
    return problems
