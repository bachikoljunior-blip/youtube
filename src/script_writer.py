...
def request_form(topic_id: str, share: float = MID_REQUEST_SHARE) -> str:
    ...
    `ROLE` は「長尺では登録の依頼も書かない（維持率が落ちる）」と書いています。
    だから `s-` で始まらないテーマIDには `"長尺"` を返し、
    `src/judgeable._members_by_split()` の `out` に無い群名なので**落ちます**。
    **`"終端のみ"` を返さないこと** —— 依頼そのものが無い本が対照群に混ざり、
    測っているものが「途中の依頼」から「依頼そのもの」にすり替わります。
    """
    if not str(topic_id).startswith("s-"):
        return "長尺"
    if share <= 0:
        return "終端のみ"
    if share >= 1:
