...
- **セグメントは5〜7個に割ること。** 1枚の絵が12秒を超えて止まると検査で落ちます。
  1つの計算結果を、結論→前提→内訳→自分の場合→次、と刻んで見せる。
- **最後の1セグメントは、次に何が分かるかを示すこと。** 長尺の登録依頼を書かない規則は
  ここには当てはめない。理由は、収益化の門が「登録者1,000人 かつ 総再生4,000時間」で、
  **遅いほうで決まり、いまの律速は登録者だから**。ショートは登録者を集めるために出している。
- ただし「チャンネル登録お願いします」とは書かない。**続きに何の数字があるかを言う。**
  「同じ条件を年齢べつに全部計算したものが別にあります」のように、
  次に得られるものを具体的に示すほうが、依頼より効くはず（これは推測。実績で確かめること）。
