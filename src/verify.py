...
def _check_ear_load(script: dict | None) -> list[str]:
    ...
    **2026-09-02 に撃ち直しました**（控え **694本・6,206コマ**。控えが 155本 増えています）:

        1コマあたりの数    中央値 2.0 ／ 平均 2.26 ／ 最大 16
        4個以上            1,380コマ（22.2%）
        **5個以上**        **610コマ（9.8%）** ／ 5個以上を1つでも持つ本 **106本（15.3%）**
        6個以上            451コマ（ 7.3%）

    **コマで見ると 13.4% → 9.8% に下がりました**（本では 14% → 15.3% で横ばい）。
    **これは「この検査が効いている」の証拠ではありません** —— 控えが増えており、
    書き直しの輪を通った本と通っていない本が混ざっています。
    **上の「覆る条件」の2つ目（歩留りが落ちないなら 4 へ下げる）を判定するには、
    歩留りのほうを並べること**（`scripts/status.py` の通過率）。
    4 へ下げたときに当たるコマは、いまの控えで **22.2%** です。
...
def _check_yomi(script: dict | None) -> list[str]:
    ...
    **かかる時間**（撃って測った・2026-09-02）: open-jtalk は **約 129文字/秒**
    （音声合成の側が律速で、`-s` を下げても変わりません）。読み上げ15行・約600字の
    台本1本で **約4.7秒**。`script_writer` の書き直しは3回までなので、
    **1本あたり 20秒 くらい足されます。** 作るのに分単位かかる工程なので通しました
    —— **これが重すぎると分かったら、行ごとの結果を憶えること**
    （同じ行は書き直しをまたいで変わらないので、そこがいちばん効きます）。
