...
def blocked(lever: str | None, state: dict) -> list[str]:
    ...
    **買うのは日付ではなく、日付を動かせる通貨のほう**です。
    ...
    # --- **実際に出された反論に、その場で答えること**（2026-09-02 12:4x）---
    #     同じ日の別の回が `--lever density` で ship し、**日誌に註を書いて**
    #     こう反論しました（commit a0538dcd）——「鳴っているのは『規則より上へ
    #     出せ』の話で、この回がやったのは **規則に届いていない実物
    #     （0.05本/日）を規則まで戻す手**だ」。**その反論は正しい。**
    #     **そして、腕の名前としては正しくありません** —— `data/runs.jsonl` の
    #     `lever: density` を数えるのは `drift.dead_arm_report` で、
    #     **日誌の註は台帳の行を数え直しません。** この repo が何度も踏んだ
    #     「言っている所と、している所が別」の形そのものです。
    #     だから断り文にこの道を書きます。**塞ぐのではなく、行き先を示す。**
    if lever == "density":
        out.append("         **「床に戻す手」なら、ここで詰まらないこと** ——"
                   " 暦を詰めて 0.05本/日 を規則の 1.00本/日 へ戻す手は、"
                   "**天井を上げていません。** それは `density` の腕ではなく、"
                   "**規則の日を増やして `at_rule_mean` の標本を育てる手**です"
                   "（`src/rule_per_video.py`）→ `--lever per_video`。")
        out.append("           日誌に註を書いても、`drift.dead_arm_report` は"
                   "**台帳の `lever:` しか数えません。** 註では数え直りません。")
