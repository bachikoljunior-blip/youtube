...
- **「円」がそろっていても、期間はそろっていません**（2026-08-28 に測って足した）。
  `unit_of` が見るのは**欄の名前の末尾だけ**なので、
  **月額の円と年額の円が、どちらも `円` として組になります。**
  実測 —— `比の形` の上位に出た `kogaku × kokuho`（90.3%〜95.1%）は

      A `kogaku.year_with_multi_hit(cost) → 多数回の額`  **44,400 ＝ 月額**
      B `kokuho.premium(members) → 保険料`               **415,415 ＝ 年額**

  で、**12倍の期間ちがいをそのまま割っています。** 割合そのものは算術として
  正しくても、**「保険料は家計の 9割」という読みは成り立ちません。**
  **これは `崖の近さ` にも同じだけ掛かります**（あちらも `unit` しか見ていない）。
  **節に起こす前に、両方の欄が月なのか年なのかを、関数の docstring で確かめること**
  （`inspect.getdoc` は数秒です）。
  **覆る条件**: 欄に期間の軸を持たせられたら、ここは `unit` の側で弾けます
