...
#
# 2026-08-04 はこの中身で出し始めた日。
# **ここに「前身のぶんが混ざらなくなる」と書いてあったが、混ざるものは無い**
# （2026-08-10 に実測。前身の動画は0本）。開始日を固定する理由は (3) の除去だけです。
...
def collect(start: date = ERA_START) -> dict:
    ...
    **開始日は固定**（`ERA_START` の説明を読むこと）。
    ...
    try:
        ...
        # **チャンネル単位の総計。全部こちらの成果です。**
        # 「前身の再生を含む」と書いていたが、2026-08-10 に uploads を全部引いて
        # **前身の動画が1本も無い**ことを確認した（35本すべてこの企画のもの）。
        # 動画ごとの合計より小さいのは無効再生の除外で、他人のぶんではない。
        out["チャンネル.総再生"] = int(st.get("viewCount", 0))
