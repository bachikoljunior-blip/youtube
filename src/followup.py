...
**3つの独立した実験が、8日かけて同じ結論に着きました。**

    2026-08-15  「1本あたりの再生が300〜1500なのは中身のせいで、天井ではない」→ **外れ**
                → next: **「この形式では桁が足りないと確定させる。中身の調整をやめる」**
    2026-08-20  「尺を30秒に縮めると engaged が上がる」→ **外れ**
                → next: **「静止画スライド＋合成音声という形式そのものを疑う」**
...
**算数も同じことを言っています。** `eta.py` の実測で:

    `per_video` の引き代   **×2.96**（腕を天井まで引いたときの倍率）
    20万に要る倍率         **×17.4**（最良の RPM 帯でも）

**5件が全部当たっても、2.96 < 17.4 で届きません。**
天井の算数と、3つの外れの verdict が、**別々に同じことを言っていて、
どちらも台帳に届いていない** —— このリポジトリで通算10回目の形です。
