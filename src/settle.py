...
#:
#: **2026-09-02 に 48 → 72 へ上げました**（最適化の回。理由は
#: `MATURE_HOURS_BY_FORM` の註）。この定数は `MATURE_HOURS_BY_FORM["ショート"]` と
#: **同じ数でなければなりません**（`tests/test_mature_hours_by_form.py` が門）。
MATURE_HOURS = 72
...
#: ---
#:
#: ## **2026-09-02: ショートを 48 → 72 に上げました**（最適化の回・撃って出た数）
#:
#: **この定数は、同じファイルの `settles_at()` と食い違っていました。**
#: `settles_at()` の docstring は 2026-08-31 の時点で
#: 「**ショートは、どの地平でも 72時間 で 100% です**」と書いており、
#: 関数もそう返します（`settles_at("ショート")["hours"] == 72.0`）。
#: **それでも定数は 48 のままでした** —— この repo がいちばんよく踏む形
#: （同じファイルの2か所が別のことを言い、片方しか読まれない）。
#:
#: 実測 2026-09-02（`views_curve(form="ショート", min_views=1.0)`・`share_settled`。
#: **門は `SETTLED_SHARE_FLOOR` = 0.95**）::
#:
#:     地平     24h     **48h**    **72h**   168h    n
#:     168h    0.642    0.815      0.908     1.000   119
#:     240h    0.566    0.767      0.822     0.878    90
#:     336h    0.722    **0.931**  **1.000** 1.000    29
#:     480h    0.500    **0.909**  **1.000** 1.000    11
#:
#: **いちばん長い2つの地平で、48h は門（0.95）を割っています**（0.931 / 0.909）。
#: 72h は 1.000。08-31 に 48 を置いたときの数は 96.2%（n=79・`min_views>30`）で、
#: **標本が増えて割りました。** ＝ `MATURE_HOURS` の覆る条件が発火した状態です。
#:
#: ## なぜ 24時間 の差が効くか（**ここが要点。上げるのは面倒な向きです**）
#:
#: `src/form_record._settled(form)` は `mature_hours_supported(form)` ＝
#: 「`settles_at()["hours"] <= mature_hours(form)`」です。
#: **72 ≤ 48 は偽**なので、**ショートは「伸びきっていない」側に落ちていました。**
#: そこから `scripts/eta._escape_form()` の `cap`（＝伸びきった形の記録の最大）が
#: **0** になり、毎周こう出ていました ——
#:
#:     **逃げ先（形を替える道）は、この回は比べられません**（`cap` が 0）
#:
#: 腕は3本とも天井まで引いても目標の 18.7%（`python -m src.joint_cap`）なので、
#: **「形を替えれば天井が上がるか」は、この機械に残っていた最後の未測の戸**でした。
#: それが**ショートの年齢の 24時間**で閉じていました。
#:
#: 上げたあと（同じ回に実測）: `cap` = 1,891（ショート）／逃げ先 長尺 525（打ち切り
#: 補正 ×2.75 込み）→ **`comparable: True`・`over` ×0.28** ＝
#: **「比べた。長尺は逃げ先ではない」**。「比べていない」ではなくなります。
#:
#: ## 代償（**隠さないこと**）
#:
#: 齢 48〜72時間 のショートが標本から落ちます。`drop_unripe` の「未熟」が
#: その2日ぶん増え、`ready`（前提の判定できる日）も 1日 後ろへ動きます
#: （`eta.ready_date()` が `ceil(MATURE_HOURS/24)` を足すため 2日 → 3日）。
#: **θ（腕の動く速さ）を直接は縛りません** —— あちらは `SETTLE_DAYS` /
#: `src/judgeable.py` の側で、この定数は「平均に入れてよい年齢」だけです。
#:
#: **覆る条件**:
#:
#: - `settles_at("ショート")["hours"]` がこの数と食い違ったら、**この数のほうを
#:   合わせること**（関数は定数を持たず毎回 測り直します。**逆はしない**）。
#: - `share_settled` が 72h でいちばん長い地平で 0.95 を割ったら、さらに上げる。
#: - **下げる向きに動かすときは、`_escape_form()` の `cap` が 0 に戻ることを
#:   先に確かめること** —— 戻るなら、それは上の戸をもう一度 閉めることです。
#: - 長尺の 96 は**下限**のまま（`settles_at("長尺")["hours"]` は None ＝
#:   この標本に長尺が伸びきる年齢はまだありません）。標本が 20本 を超えたら測り直す。
MATURE_HOURS_BY_FORM: dict[str, int] = {"ショート": 72, "長尺": 96}
...
def settles_at(form: str | None = None, *, min_views: float = 1.0,
    ...
    **分離できるようになる条件**: 150回 を超える長尺が 2本 出たとき。
    ...
    ## **この直しで隔たりは閉じません**（読み過ぎを止めるために、同じ回に測った）

    `scripts/eta.py` は ×21.4 を「隔たりの**上限**」と印字するようになりました。
    **それを「測り直せば届くかもしれない」と読まないこと。** 実測（2026-08-31）::

        長尺の記録            156回/本（`_Mz5rg6jQ_A`・齢 246時間 の観測）
        336h を基準にした曲線  240h の時点で **67%**（n=6）
        打ち切りを直した記録   ≒ **234回/本**（×1.50 の補正）

        目標に要る1本あたり（長尺・RPM ¥2,000・規則1本/日）  **3,333回**
          いまの隔たり             ×21.4
          打ち切りを直した隔たり   **×14.2**

    **打ち切りが説明するのは ×1.50 ぶんだけです。残り ×14.2 は別の所にあります。**
    だからこの直しの値打ちは「隔たりが縮む」ことではなく、
    **「×21.4 という数を、実測でない分だけ実測でないと言う」**ことです。
    **`per_video` を測り切っても、いまの構成では届きません** ——
    そこから先は構成の側（ニッチ・尺・形式・言語・収益の立て方）の話で、
    `CLAUDE.md` が「全部あなたが決めます」と書いている範囲です。

    **覆る条件**: 上の ×1.50 は n=6 の曲線から出ています。長尺の標本が増えて
    曲線が下がれば補正は大きくなりますが、**×14.2 を埋めるには 240h の割合が
    7% を切る必要**があり、いまの 67% からは桁がちがいます。
    """
