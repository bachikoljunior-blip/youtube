...
1. **停止が明けたら、この module は黙ります**（`is_paused()` が偽）。
   **2026-08-31 に明けています** —— いま止まるのは `.owner-pause` が
   オーナーの手で置かれたときだけ（`src/pause_guard`）。
   **条件の本文（`docs/RESUME_GATE.md`）は switch ではありません** ——
   あれが在っても、何も止まりません
2. 門を 6件 閉じても審査に落ちたら、**この6件が十分条件でなかった**ということ。
   そのときは `docs/RESUME_GATE.md` の側を書き換える（ここは写しているだけ）
