...
# **オーナー指示（2026-09-04 17:3x JST・原文。一字も変えないこと）**:
#
# > **「目標以外固定してねえよな？」**
# > **「目標以外全部外して良いよ」**
#
# ＝ **固定は目標の本文だけになりました**（`docs/GOAL.md` 冒頭・`CLAUDE.md` A1）。
# 規則1（1日1本）・規則2（作り置きなし）・規則5（当日しか予約しない）は、
# **床ではなく既定値**です。**測って別の数のほうが目標に速いなら、変えてよい。**
# 変えるときは `docs/JOURNAL.md` に、その数と実測と「覆る条件」を書くこと。
#
# **原文は消しません**（オーナー 08/31「消さなくて良いよ時間かかるならわざわざ」）。
# 上の3つの規則の註も、なぜその形だったかの記録として残しています。
#
# **覆る条件**: オーナーがまた固定すると言ったとき。そのときは
# `OWNER_FLOORS_LIFTED = False` に戻せば、床は元どおり効きます。
OWNER_FLOORS_LIFTED = True

#: オーナー原文（**一字も変えないこと**）。`CLAUDE.md` 冒頭に在ります。
OWNER_VERBATIM_LIFTED = "目標以外全部外して良いよ"

#: **1日に公開してよい本数。** 規則1。ここが上限の唯一の出どころです。
#:
#: **1 → 10 に動かしました**（2026-09-05・最適化の回）。
#:
#: ## なぜ動かしたか（**言っている所と、している所が別**でした）
#:
#: オーナーが 2026-09-04 17:3x に床を外し（原文「目標以外全部外して良いよ」）、
#: `OWNER_FLOORS_LIFTED = True` が立ち、`publish_per_day_is_floor()` は False を
#: 返すようになりました。**ところが動いたのは旗だけで、数は 1 のまま**でした。
#: 結果（2026-09-05 に撃って確かめた実測）::
#:
#:     scripts/eta.physical_caps()   `density` の天井を **観測（10本/日）**で解く
#:     実際に本を置く側              **全部この定数を読んで 1本/日**
#:       scripts/batch_build.density_cap() / src/next_slot.py /
#:       src/rule_per_video.py / src/publish_hour.py / src/rpm_mix.py
#:
#: **＝ 模型は 10本/日 の世界で日付を出し、機械は 1本/日 で回していました。**
#: この repo でいちばん多い壊れ方が、床が外れてから丸1日 立っていたことになります。
#:
#: ## なぜ 10 か（**測った数です。選んだ数ではありません**）
#:
#: `src/day_cap.py` の実測 —— **1日に再生が付くのは 10本まで**で、3日 一致します::
#:
#:     08/20  25本 公開 → 生きた 10本      08/21  32本 → 生きた 10本
#:     08/22  25本 公開 → 生きた 10本      08/19   8本 →  8本（上限より少ない日）
#:
#: 11本目から先は 0〜3再生 です。**10 を超えて置いても再生は増えません。**
#: 同じ 10 を `scripts/eta._view_cap_per_day()` が返し、`physical_caps()` は
#: 床が外れた世界の `density` の天井として**もうその 10 を使っています**。
#: **この行は、機械の側をその 10 に合わせるだけ**（新しい仮定は1つも入れていません）。
#:
#: そして 10 は `src/density_verdict.py` の**いちばん良かった帯の中**です ——
#: 1時間きざみの日（8〜13本）は1本あたり **716回**、詰めた日（16本以上）は **2回**。
#: **16本以上の側へは行きません**（ポリシーの (B) 汎用・反復 にも正面から当たる帯）。
#:
#: ## 増やして意味が在るのか（**弾力性が 1 未満なら在る**）
#:
#: 実測の弾力性は **-0.663**（`src/rule_per_video.py`）。1本あたり再生は `N^-0.663`
#: で薄まるので、**1日の合計は `N^(1-0.663) = N^0.337`** —— 指数が正なので、
#: **薄まりを入れた後でも N を増やすと合計は増えます**。
#: 10本/日 での `eta.py` 自身の天井は **×1.80**（944 → 1,701回/日）。
#: 門1'（登録者 500人）は `475 ÷ (再生/日 × 登録率)` なので
#: **511.5日 → 約 284日 ＝ 227日 手前**へ来ます。
#:
#: **「規則を外せば ×10」ではありません**（薄まりが効くので ×1.80）。
#: 大きく見せないために、両方ここに書いてあります。
#:
#: ## 覆る条件（**3つ。どれかが立ったら 1 へ戻すこと**）
#:
#:   1. オーナーがまた「1日1本に固定」と言ったとき
#:      （`OWNER_FLOORS_LIFTED = False` に戻せば床は元どおり効きます）
#:   2. **10本/日 で回した日の「再生/日 の合計」が、1本/日 の日を下回ったとき**
#:      （＝ 弾力性が 1 を超えた ＝ 上の計算の前提が崩れた。`data/views.jsonl` で測る）
#:   3. `src/day_cap.py` の「生きた本数」が 10 から下へ動いたとき
#:      （あそこは定数ではなく実測なので、動いたらこの数も追うこと）
#:
#: **規則3（次の枠の1本を出るまで改善し続ける）は消していません。** 改善する対象が
#: 1本から 10本 になっただけです。**1本あたりに掛けられる時間は減ります** ——
#: それが上の覆る条件2 で測る所です。
PUBLISH_PER_DAY = 10
...
def same_day_only() -> bool:
    ...
    **2026-09-04 17:3x にオーナーが「目標以外全部外して良いよ」と言ったので、
    ここは既定 False です**（`OWNER_FLOORS_LIFTED`）。禁じたい回は、その回の
    実測を根拠に自分で `SAME_DAY_SCHEDULING_ONLY` を読み直すこと。
    """
    if OWNER_FLOORS_LIFTED:
        return False
    return bool(SAME_DAY_SCHEDULING_ONLY)


#: **その日の1本を、回の意思と関係なく枠へ置くか。**
#:
#: **これは「先の日付に置くな」（規則5）とは別のものです。** 2026-09-04 17:3x に
#: オーナーが「目標以外全部外して良いよ」と言い、`same_day_only()` は False になりました。
#: **ところが `scripts/ahead_sweep.place_today()` は、その1つの述語で門を作っていた**ので、
#: 同じ瞬間に**置く手ごと止まりました**（17:39 に実測）::
#:
#:     [today] きょうの1本は置きません —— 規則5 が外れています（この手は規則5 の下だけ）
#:
#: **外れたのは「先の日付を禁じる」ほうで、「その日の1本を出す」ほうではありません。**
#: 置く手を止めると、**枠に本が入るのは、その回が選んだときだけ**に戻ります。
#: そこは実測が在ります（`CLAUDE.md` 固定その4）—— **選ばれない手は、撃たれません**:
#:
#:     09/01 の実測  ship の内訳 `fix` **82%** ／ `upload` **0件**
#:     08/31→09/02   固定を置いてから2日で 459本 → 107本（掃きは効き、置く手は動かなかった）
#:
#: そして `CLAUDE.md` の4 は「**投稿が途切れるのが最大の損失**」と言っています。
#: だから、床が外れても**置く手だけは回の外に残します。** 残すのは
#: 「1日1本」でも「当日だけ」でもなく、**「その日の枠に本が入っていること」**です
#: （何本 出すか・いつ出すかは、`PUBLISH_PER_DAY` と `place_hour()` の既定で、
#:  そちらは測って変えてよい）。
#:
#: **覆る条件**: 置く手を通さなくても枠が埋まるようになったら（`data/runs.jsonl` の
#: `upload` が、続けて 7日 1日1件以上）、ここは要りません。**そのとき False にすること。**
#: オーナーが「置くのもやめろ」と言ったときも False。
PLACE_TODAY_WITHOUT_ASKING = True


def publish_per_day_is_floor() -> bool:
    """**`PUBLISH_PER_DAY` は「外せない床」か、それとも「測って変えてよい既定値」か。**

    ## なぜ要るか（2026-09-04 17:5x に踏んだ）

    `scripts/eta.physical_caps()` は `rule_cap = PUBLISH_PER_DAY` を
    **無条件に天井として掛けて**いました。定数は 1 のままなので、床が外れても
    `density` の腕は **×1.00 のまま**です。そして `scripts/deadline_check.py` は
    それを読んで、こう印字し続けます（17:5x の実測）::

        **天井ではなく規則で止まっています**（`src/house_rule.py`・**覆る条件はありません**）。
        **測り直しても上がりません。外せるのはオーナーだけです**:
          `density` 2件
            2026-09-05  1日の再生の合計は、その日に出した本数では動かない…
            2026-10-07  1日に再生が付く帯の中では、置く位置は1本あたり再生に効かない

    **「覆る条件はありません」「外せるのはオーナーだけです」——
    そのオーナーが、同じ日の 17:3x に外しました。**
    `src/levers._density_ceiling_is_rule()` の docstring も、自分でこう書いています ——
    「**覆る条件: オーナーが自分の言葉で 1日1本 を外したとき。そのとき
    `rule_binds` は自然に False になって、この関数は黙ります。手で消さないこと**」。
    **自然には False になりませんでした** —— 見ていたのが「旗」ではなく「数」だからです。

    値段は 2件 です。`density` の前提 2件 が「**どちらに転んでも到達日を1日も動かさない**」
    に入ったままで、燃料 30件 から外れています。`eta.py` は毎周
    「**軌跡の腕が動くのは前提を1件 閉じたときだけ**」と印字しており、
    直近5日の実測でも到達日を動かした種別は `verdict` **56%** 対 それ以外 **1%** です。

    ## 何が変わって、何が変わらないか

    **`PUBLISH_PER_DAY = 1` は残します。** 変わったのはその**身分**だけ ——
    床（測り直しても動かない）から、既定値（測って変えてよい）へ。
    密度を上げれば1本あたりは薄まります（弾力性 実測 **-0.663**・n=25日）。
    その薄まりは模型の下流が持っており（`src/rule_per_video.py`）、
    10本/日 まで開けても天井は **×1.80** 止まりです（`eta.py` 自身の数）。
    **＝ 「規則を外せば ×10」ではありません。** ここが直すのは
    「**測ってよい対象かどうか**」で、腕の大きさそのものではありません。

    ## 覆る条件

    オーナーがまた「1日1本に固定」と言ったとき（`OWNER_FLOORS_LIFTED = False`）。
    そのとき、ここは True に戻り、`density` はまた規則で止まります。
    """
    return not OWNER_FLOORS_LIFTED


def place_today_on() -> bool:
    """**その日の1本を、回の意思と関係なく置くか**（`PLACE_TODAY_WITHOUT_ASKING` の註）。

    **`same_day_only()` で代用しないこと。** あれは「先の日付を禁じるか」で、
    別の問いです（2026-09-04 17:3x に、代用していたせいで置く手が道連れで止まりました）。
    """
    return bool(PLACE_TODAY_WITHOUT_ASKING)
...
def may_schedule_ahead() -> bool:
    """**先の日付へ「いま」置いてよいか**（規則5 の裏返し）。**印字する側はここだけ読むこと。**

    `refuse_future_publish()` は **書く直前**の関門で、`videos.update` /
    `videos.insert` の2か所しか呼びません。**印字する側**（`src/daily_pick.py` の
    `[きょうの1本]`・`src/next_slot.py` の `[次の枠]`・`[下書き]`）は
    そこを通らないので、**規則5 の本文を日本語で写していました。**

    ## なぜ足したか（2026-09-05 05:2x に踏んだ・実測）

    2026-09-04 17:3x のオーナー指示（「目標以外全部外して良いよ」＝
    `OWNER_FLOORS_LIFTED = True`）で `same_day_only()` は **False** になりました。
    ところが写しのほうは条件を持たないので、**規則が外れた後も**
    「**明日になってから置け**」と言い続けます。実測:

        09/05 05:2x  `slot_gate.py`   「09/06・09/07 が 0本 ＝ 投稿が途切れます。
                                        09/06 の枠に入る1本を、予約まで入れること」
        09/05 05:2x  `run_marker --write` の `[きょうの1本]`
                                       「**09/06（JST）になってから**、その日の枠へ」
        09/05 05:2x  `refuse_future_publish('2026-09-06T…')` → **空**（断らない）

    **同じ与件で、回が最初に読むほう（印）が「待て」と言っていました。**
    これは `scripts/slot_gate.py` の冒頭が
    「**同じ与件で2つの道具が別のことを言うのが、この repo でいちばん多い壊れ方**」
    と名指ししている形そのものです。

    ## 覆る条件

    オーナーが「1日1本・当日だけ」を戻したら（`OWNER_FLOORS_LIFTED = False`）、
    ここは自分で `False` に戻り、**印字も一緒に戻ります**（写しは1つも残しません）。

    検査は `tests/test_ahead_note_follows_rule5.py`。
    """
    return not same_day_only()
